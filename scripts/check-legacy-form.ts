 ...
 * заведён Задачей 21b.
 *
 * УЗКО, И ЭТО ЗАМЕР (m-1 задачи 5 среза Б-2, проба ревью). Прежняя форма засчитывала комментарием
 * любую строку, которая лишь НАЧИНАЕТСЯ с этих символов: метод-генератор (`*имя() {`), код после
 * блочного комментария, закрытого на той же строке, и префиксный декремент (`--имя`) проходили как
 * проза. Теперь `*` — только перед пробелом, `/` или концом строки (продолжение и хвост докблока),
 * и ни `*`, ни `/*` не снимают строку, на которой после закрытия блочного комментария идёт код
 * (хвост докблока с кодом на той же строке — дыра, найденная гейт-ревью задачи 18); `--` — только
 * перед пробелом или концом строки (SQL внутри литерала). Копия маски стоит в стороже вехи I
 * (`apps/server/test/gate-b2.test.ts`: импорт из тестов сервера сюда не дотягивается), и её
 * совпадение с этой пиннится тестом гейта по исходнику.
 */
export const COMMENT_ONLY_LINE =
  /^\s*(?:\/\/|(?!.*\*\/\s*\S)\*(?:\s|\/|$)|\/\*(?!.*\*\/\s*\S)|--(?:\s|$)).*$/;
...
export const LEGACY_MARKERS: ReadonlyArray<LegacyMarker> = [
  ...
  // (`check-legacy-form.test.ts`), а не только на синтетике: закрытая дверь, а не счётчик.
  {
    ...
    // Снимается строка, КОТОРАЯ ЦЕЛИКОМ комментарий, а не всякая, где комментарий встретился:
    // код с хвостовым `// …` остаётся под гейтом. Маска — общая `COMMENT_ONLY_LINE` (с задачи 18
    // среза Б-2; своя копия здесь была шире и пропускала метод-генератор и код после `/* … */`).
    exclude: [COMMENT_ONLY_LINE],
  },
  ...
  //
  // ПЯТЬ АЛЬТЕРНАТИВ. Первая редакция маркера имела две и требовала РОВНО ОДНУ точку в аргументе
  // парсера — ре-ревью показало на таблице форм, что мимо неё проходят восемь записей, включая
  // самую естественную прод-форму `parseGraphId(ctx.identity.actor)` (пара лежит в `ctx.identity`,
  // то есть точек всегда две). Ниже — по альтернативе на класс формы:
  //   1. `as`-приведение к бренду в любой записи: `as GraphId`, `as unknown as AccountId`,
  //      `as any as Identity`, `as string as GraphId` (средний `as <что-то> as` — опциональная
  //      группа).
  //   2. УГЛОВОЕ приведение `<GraphId>(x)` — вторая форма записи того же самого. Негативный
  //      lookbehind обязателен: без него маркер ловил бы `Promise<GraphId>`, `Set<Identity>` и
  //      всякий параметр типа, то есть краснел бы на законном коде и был бы снят первым же
  //      раздражённым читателем.
  //   3. `as never` / `as any` НА ПОЛЕ ПАРЫ или на переменной с именем актора/графа
  //      (`who.graph as never`, `someActor as any`): оба типа присваиваются ЧЕМУ УГОДНО, то есть
  //      это универсальное приведение к любому бренду. Голое `as never` маркер не ловит намеренно
  //      — в дереве 118 законных строк (`row.bodyDoc as never`, фикстуры разбора), и общий запрет
  //      был бы флудом.
  //   4. ОТМЫВКА бренда через границу: `parse*Id(<выражение с полем пары>)` — формально законный
  //      вызов парсера, фактически то же приведение. Аргумент проверяется ДО КОНЦА СТРОКИ, а не
  //      одной точкой, поэтому ловятся и `ctx.identity.actor`, и `String(who.actor)`, и
  //      `` `${who.graph}` ``.
  //   5. Она же в ОБРАТНОМ порядке на одной строке: `const v = who.actor; parseGraphId(v)`.
  //
  // ПОЧЕМУ `\.(?:actor|graph)\b`, а не `graph` вообще: `\b` разводит поле пары и КОЛОНКУ БД.
  // `row.graphId`, `r.graph_id`, `args.graphId` — законные источники (строка БД, аргумент CLI), и
  // они маркеру не видны: `graphId` и `graph_id` границы слова после `graph` не дают. Проверено
  // таблицей из 20 форм — 12 обязаны краснеть, 8 законных обязаны молчать, расхождений ноль.
  //
  // ЧЕГО МАРКЕР НЕ ДЕРЖИТ, И ЭТО НАЗВАНО ВСЛУХ: отмывку, РАЗНЕСЁННУЮ ПО СТРОКАМ
  // (`const v = who.actor;` здесь, `parseGraphId(v)` тремя строками ниже) — `git grep` построчен,
  // как и у `identity-pair`. Дно у такой формы одно: рантайм-барьер `isIdentity` на двух сайтах
  // записи (`withIdentity`, `assertHoldsOwnerGrant`) — до базы подделка не доедет, но ТИП в этом
  // месте уже обманут. Второй известный стык: строка БД, у которой алиас колонки буквально `actor`
  // или `graph` (так в `identitiesForScheduler`), — законное совпадение, снимается allowlist'ом
  // или переименованием алиаса.
  //
...
export const ALLOWLIST: ReadonlyArray<AllowEntry> = [
  ...
  {
    path: 'apps/server/src/llm/prompts/v7.test.ts',
    markers: ['bare-field'],
    reason:
      'гард смены линейки v7: перенесённый с v6 гард утверждает ОТКАЗ разбора голых имён ' +
  ...
  // --- brand-cast: единственное законное место приведений брендов ---------------------------
  {
    ...
    reason:
      ...
      'маркер; плюс резолвер тика, где `parse*` берёт строки БД под алиасами `actor`/`graph`',
  },
  {
    path: 'apps/server/src/oauth/grants.test.ts',
    markers: ['brand-cast', 'identity-pair'],
    reason:
      'сьют выдачи грантов: подделанная пара (литерал + `as unknown as Identity`) — ПРЕДМЕТ ' +
      'проверки рантайм-барьера `assertHoldsOwnerGrant`, законной пары из неё не выйдет',
  },
  {
  ...
  // --- Носители интервала Б-1→Б-2: второй сторож той же двери (класс 2 — предмет проверки) --
  {
