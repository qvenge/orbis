...
export const LEGACY_MARKERS: ReadonlyArray<LegacyMarker> = [
  ...
  // --- Шимы «дата смерти Б-1» (§С8-18, рамка Б1.16) ---------------------------------------
  // Шесть маркеров ниже описывают носители интервала А→Б-1: каждый объявил свою дату смерти в
  // собственном докблоке, и срез Б-1 её исполнил. С закрытия среза совпадение вне allowlist
  // означает воскресший шим, а не незаконченный перевод, — как у `aspects-legacy` после 0017.
  {
    id: 'shim-contract-ids',
    pattern: String.raw`CONTRACT_IDS_V1|ContractIdV1`,
    exclude: [COMMENT_ONLY_LINE],
  },
  // УТВЕРЖДЕНИЕ, А НЕ НОСИТЕЛЬ — и поэтому единственный из шести БЕЗ `COMMENT_ONLY_LINE`.
  // Шесть докблоков среза А называют условие П5 по имени: «таблицы контрактов и подписок
  // пусты, их сиды — первый акт среза Б-1, и начнётся он за этим гейтом». Условие снято
  // ревизией 3 спеки (§0.2-18), таблицы засеяны — и докблок, который его называет, врёт
  // следующему читателю. Снимать строку-комментарий тут значило бы разрешить ровно то,
  // против чего маркер написан.
  //
  // Слово «гейт» и имя условия стоят в ЭТОМ комментарии ПОРОЗНЬ, и это не стилистика.
  // `ALLOWLIST` снимает `scripts/check-legacy-form.ts` по ВСЕМ маркерам (запись без списка),
  // так что от собственной прозы `--gate` не покраснел бы; покраснела бы проба
  // по рабочему дереву (`check-legacy-form.test.ts`), где `allowed` перечислен поимённо, — и
  // держалась бы она тогда на формулировке комментария, а не на форме, которую маркер ищет.
  //
  // В паттерне `[а-яё]*`, а не фраза целиком, и это замер сразу по двум осям: имя условия
  // стоит в докблоках в разных падежах, а перед ним у пяти из шести — «после», у шестого
  // (`packages/shared/src/aspect-registry.test.ts`) — «раньше». Фраза, снятая с одного
  // докблока дословно, нашла бы пять из шести и молча объявила бы дерево чистым.
  //
  // `[Гг]` — третья ось того же замера: `git grep -P` идёт БЕЗ `-i`, а докблок вправе поставить
  // это слово ПЕРВЫМ в предложении, то есть с заглавной, — ровно так и было написано в
  // `db/seed-registries.ts` про снятое условие П5, и строчная форма паттерна проходила мимо.
  // Регистр остальных букв не варьируется: слово стоит либо в начале предложения, либо в
  // середине. Фразу целиком здесь не цитирую по правилу абзацем выше — образец заглавной формы
  // живёт в `SAMPLES` теста, где он предмет проверки.
  //
  // ОБЫЧНАЯ строка, а не `String.raw`, — единственный такой паттерн в файле, и это не стиль.
  // Транспайлер Bun 1.2.7 печатает не-ASCII внутри шаблонного литерала escape-последовательностями
  // (`charset: ascii`), а `String.raw` отдаёт именно СЫРОЙ текст — то есть `\u0433\u0435...`
  // вместо кириллицы. `git grep -P` на такой паттерн падает кодом 128 («PCRE2 does not support
  // \u»), гейт бросает исключение, и красным становится весь прогон. Спецсимволов регулярки в
  // паттерне нет, поэтому обычная строка описывает ровно ту же форму.
  { id: 'p5-gate-docblock', pattern: '[Гг]ейт[а-яё]* П5' },
  // Узел `class` Q-AST отвечал `CLASS_NOT_AVAILABLE`, пока контрактов не было; Задача 4 среза
  // Б-1 заменила его на `UNKNOWN_CONTRACT`/`UNKNOWN_SET` (§С8-3 «невыразимое — ошибка, а не
  // пустота»). Имя, вернувшееся в код, означает вернувшуюся заглушку.
  {
    id: 'class-not-available',
    pattern: String.raw`CLASS_NOT_AVAILABLE`,
    exclude: [COMMENT_ONLY_LINE],
  },
  // `sourceNotIn: {prop, values}` — «набор завершаемости прямой проверкой свойства»
  // (`packages/shared/src/query/ast.ts`: «ДАТА СМЕРТИ ПОЛЯ — срез Б-1»). Имя поля
  // ОСТАЛОСЬ, форма стала `{contract, set}`, поэтому маркер ловит форму, а не имя.
  // `\??` в паттерне — замер: два объявления типа пишут поле необязательным
  // (`query/normalize.ts`, `query/parse-ast.ts`), и без вопросительного знака маркер
  // находил 11 строк из 13 — те два и были бы молчаливой дырой.
  {
    id: 'source-not-in-prop',
    pattern: String.raw`sourceNotIn\??:\s*\{\s*prop`,
    exclude: [COMMENT_ONLY_LINE],
  },
  // Три боевых текста Agenda (`useAgenda.ts`) — три запроса вместо одной подписки
  // (§С8-17). В паттерне все ТРИ имени, хотя реестр интерфейсов назвал два: третий умирает тем
  // же коммитом, и гейт, зелёный при живом `AGENDA_DAYS_QUERY`, врал бы ровно про то, ради чего
  // написан. Корпус печати `AGENDA_QUERY_TEXTS` (`shared/query/ast-fixtures.ts`) живёт
  // дальше и под маркер не подпадает — у него другое имя, и это не совпадение, а Р-24 плана.
  {
    id: 'agenda-three-texts',
    pattern: String.raw`AGENDA_(DAYS|OVERDUE_DUE|OVERDUE_START)_QUERY`,
    exclude: [COMMENT_ONLY_LINE],
  },
  // Разворот сахара `excludeBlocked` через литералы статуса (`parse-ast.ts`): до
  // контрактов сослаться на набор `closed` было нечем. Задача 4 перевела разворот на
  // `{contract:'orbis/completable', set:'closed'}`; литералы обязаны давать ноль.
  {
  ...
  // identity-pair — ВТОРОЙ барьер вокруг пары «актор + текущий граф» (D44, спека Ш-2). Несущая
  // защита с Р-ИГ-12 — ЗАМОК ТИПА: `Identity` это псевдоним НЕэкспортируемого класса с приватным
  // полем (`apps/server/src/identity.ts`), поэтому собрать пару с нуля нельзя ни литералом (в
  // любой форме записи), ни своим классом, ни `satisfies`, ни функцией с объявленным возвратом,
  // и нельзя ПРОИЗВЕСТИ её спредом из готовой — приватное поле спред не переживает (`TS2741`).
  //
  // Зачем тогда маркер, и что он ловит СВЕРХ типа:
  //   1. `Object.assign({}, who, { graph: other })` — тип результата есть ПЕРЕСЕЧЕНИЕ аргументов,
  //      приватное поле в нём сохраняется, и компилятор такую подделку ПРОПУСКАЕТ (измерено
  //      ре-ревью Г-3). Здесь маркер — единственный барьер, а не дубль.
  //   2. Он краснеет РАНЬШЕ компилятора: `git grep` — секунды против полного `tsc`.
  //   3. Он переживёт попытку ослабить тип: снявший замок получит зелёный typecheck (директивы
  //      `@ts-expect-error` станут неиспользуемыми — TS2578, — но ослабить можно и их).
  //
  // ЧЕГО МАРКЕР НЕ ЛОВИТ, И ЭТО НАЗВАНО ВСЛУХ: `git grep` построчен, поэтому литерал, разложенный
  // на строки (`{\n  actor: …,\n  graph: …,\n}`), мимо него проходит — и biome его НЕ схлопывает
  // (измерено: `--write` → «No fixes applied», Ф-Г-50). Ровно эту форму закрывает замок типа.
  // Абсолютной гарантии нет ни у одного из двух барьеров; каждый закрывает то, что упускает другой.
  //
  ...
  {
    id: 'identity-pair',
    // Три альтернативы: (1) сборка литералом в любом порядке ключей; (2) ПРОИЗВОДНАЯ пара
    // спредом; (3) она же через `Object.assign` — единственная форма, которую тип не ловит.
...
const FROZEN_PROMPTS = [
  ...
  'v4',
  'v4.fixture',
  // v5 замораживается задачей 17 вместе с заведением линейки v6: его текст — снимок того,
  // что модель читала до выноса прозы Финансов в манифест.
  'v5',
  'v5.fixture',
  'routine-v1',
  'routine-v1.fixture',
  'routine-v2',
  'routine-v2.fixture',
  // `routine-v3` заморожен ТЕМ ЖЕ правилом (глобальное ограничение плана Б-1 и чек-лист
  // деплоя: «`v5.ts` и `routine-v3.ts` заморожены»), хотя канал рутины на нём и работает:
  // заморожен ТЕКСТ, а не использование, — и список обязан говорить об этом сам, иначе
...
export const ALLOWLIST: ReadonlyArray<AllowEntry> = [
  ...
  // --- identity-pair: сьюты, где литерал пары — ПРЕДМЕТ проверки ---------------------------
  // Записей ТРИ. Было семь: пять сьютов собирали пару руками и переведены на резолвер 2
  // (Р-ИГ-11), а сам `identity.ts` выпал из списка с Р-ИГ-12 — пару там делает `new`
  // закрытого класса, литерала не осталось вовсе, и поблажка ему больше не нужна (значит и
  // литерал, если он там заведётся, теперь краснеет). Осталось ровно то, что МОЖЕТ и ДОЛЖНО
  ...
  },
  {
    ...
    markers: ['identity-pair'],
    reason:
      'сверка того, ЧТО вернули резолверы, и пин замка типа: обе отвергаемые формы литерала ' +
      '(однострочная и многострочная) пишутся в нём дословно под `@ts-expect-error`',
  },
  {
    path: 'apps/server/test/graph-vs-account.test.ts',
    markers: ['identity-pair'],
    reason:
      'сьют «граф ≠ аккаунт» (гейт миграции 0021): пара с разными значениями названа дословно ' +
      'в ИМЕНАХ двух тестов — сама она собирается резолвером 2',
  },
