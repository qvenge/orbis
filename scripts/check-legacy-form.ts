 ...
 * блочный комментарий — только если на строке после его закрытия нет кода, `--` — только перед
 * пробелом или концом строки (SQL внутри литерала). Копия маски стоит в стороже вехи I
 ...
 */
export const COMMENT_ONLY_LINE = /^\s*(?:\/\/|\*(?:\s|\/|$)|\/\*(?!.*\*\/\s*\S)|--(?:\s|$)).*$/;
