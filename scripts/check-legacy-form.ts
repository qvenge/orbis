...
export const LEGACY_MARKERS: ReadonlyArray<LegacyMarker> = [
  ...
  // защита с Р-ИГ-11 — ЗАМОК ТИПА: у `Identity` есть поле с ключом-символом, объявленным в
  // `apps/server/src/identity.ts` и не экспортированным, поэтому собрать пару литералом снаружи
  // нельзя вовсе, в какой бы форме её ни записали.
  //
  // Зачем тогда маркер. Во-первых, он краснеет РАНЬШЕ компилятора и прямо называет правило —
  // на `git grep` это секунды против полного `tsc`. Во-вторых, он переживёт попытку ослабить тип:
  // снявший поле-бренд получит зелёный typecheck (директивы `@ts-expect-error` станут
  // неиспользуемыми — TS2578 — но ослабить можно и их), а гейт всё равно покажет литерал.
  //
  ...
  // (измерено ре-ревью Г-3: `--write` → «No fixes applied»). Ровно эту форму и закрывает замок типа;
  // маркер остаётся растяжкой против однострочного дрейфа, а не доказательством (Ф-Г-50).
  //
...
export const ALLOWLIST: ReadonlyArray<AllowEntry> = [
  ...
  // --- identity-pair: замок типа и сьюты, где литерал пары — ПРЕДМЕТ проверки --------------
  // После Р-ИГ-11 записей три, а не семь: пять сьютов собирали пару руками и переведены на
  // резолвер 2 (`identityOfGrant`) — собрать её иначе теперь нельзя. Осталось ровно то, что
  // МОЖЕТ и ДОЛЖНО содержать литерал.
  {
    path: 'apps/server/src/identity.ts',
    markers: ['identity-pair'],
    reason:
      'замок типа: единственная сборка пары (приватный помощник `pair`) и три резолвера над ней — ' +
      'ради этого места маркер и заведён',
  },
  {
