 ...
 * строках (ключи сортировки, строки :904-905 на момент написания). grep(1) читает файл
 * целиком и на NUL объявляет его двоичным: `grep -rnE 'aspects[[:space:]]*->'` по этому
 * файлу давал НОЛЬ строк вместо 51 — и молча, без пометки «Binary file matches», потому
 * что при отсутствии текстовых совпадений печатать нечего. То есть ровно тот файл, где
 * старой формы больше всего, выпадал из приёмки целиком.
 *
...
export const LEGACY_MARKERS: ReadonlyArray<LegacyMarker> = [
  ...
  // aspectsMap живёт с Задачи 4a до 13c: приёмка 13c (Шаг 3) требует отчёта именно по нему
  // (рулинг Р-П-4). Маркер `->>'` из §А12-2 НЕ заводится: после перевода `props->>'orbis/…'` —
  // законная форма доступа, а старую форму целиком покрывает `aspects(_legacy)?\s*->`.
  {
    id: 'aspects-legacy',
