...
//  - регэксп маркера: экранированные скобки (литерал `\{\{…`, строка для `new RegExp` с `\\{\\{…`,
//    квантификатор `\{{2}`), класс `[{]`, строка `new RegExp('{{…')` и НЕэкранированные `{{` в
//    регэкспе-литерале в любом его месте (`/{{`, `/^{{`, `/^\s*{{query:/`);
//  - поиск маркера строкой: `indexOf`/`startsWith`/`includes`/`lastIndexOf`/`split` с аргументом `'{{…`;
//  - строка-маркер в `'`/`"` в ЛЮБОМ месте строки кода: в сравнении и константе (`= '{{…`,
//    `case '{{…` — так ищет блок данных сама единственная копия, `QUERY_OPEN` + `startsWith`), в
//    массиве и множестве (`new Set(['{{title}}', …])`, `['{{body}}'].includes(…)`), аргументом
//    (`isMarker(line, '{{title}}')`). Данные в `'`/`"` с `{{` не начинаются: сид и тела пишут шаблонными
//    строками. В обратных кавычках — только голый маркер без содержимого на месте значения
//    (`` = `{{title}}` ``, `` [`{{query:` ``).
// Строки сида (`` `{{query:aspect=…}}` ``), печать маркера с интерполяцией (`{{${name}}}`), объекты
// JSX (`style={{…}}`) сюда не попадают; строки-комментарии (начинаются с `*`, `//`, `/*`) не
...
// с интерполяцией рядом со сравнением (печать и сравнение различимы только разбором кода);
// регэксп-литерал, стоящий после `)` или имени (там `/` — деление, и без разбора кода их не
// различить: `x = f() /{{…/` страж пропустит), и регэксп, начатый на одной строке и продолженный на
// другой; разбор посимвольным циклом; копию правил в `scripts/` и в тестах — они вне охвата.
//
...
/**
 * Приметы образцами (`git grep -P`). Один образец и для `git grep -P`, и для контроля примет в самом
 * тесте — чтобы проверялось ровно то, что ищется:
 *  - строка-маркер в `'`/`"` в любом месте строки (данные такими не пишут: сид — шаблонными строками);
 *  - голый маркер `{{имя}}`, `{{/имя}}`, `{{имя:` в обратных кавычках там, где стоит значение (после
 *    `=`, `:`, `(`, `,`, `[`, `!`, `case`, `return`): с содержимым — тела сида, а голый маркер в
 *    обратных кавычках посреди текста — слово многострочного комментария JSX, не код;
 *  - неэкранированные `{{` внутри регэкспа-литерала в любом его месте. Литерал узнаётся по `/` после
 *    начала строки, `(`, `=`, `,`, `:`, `[`, `!`, `&`, `|`, `?`, `;`, `>` (стрелка `=>`) или `return` —
 *    там `/` не бывает делением; тело литерала — до первого неэкранированного `/`, классы `[…]` проходятся целиком.
 *    Скобки `{`/`}` в число начал не входят: `{{/tab}}` в тексте отказа иначе читался бы литералом.
 */
const OPENER_SIGNS: readonly RegExp[] = [
  /['"]\{\{/,
  /(?:[=:(,[!]|\bcase|\breturn)\s*`\{\{\/?[a-z_]+(?::|\}\})`/,
  /(?:^|[(=,:[!&|?;>]|\breturn)\s*\/(?![/*])(?:[^/\\\n[]|\\.|\[(?:[^\]\\\n]|\\.)*\])*?(?<!\\)\{\{/,
];
...
const ALLOWLIST: Record<string, { readonly count: number; readonly reason: string }> = {
  ...
  'packages/shared/src/doc/nodes/layout.ts': {
    count: 1,
    reason:
...
describe('маркеры тела распознаёт одна копия правил (спека страниц 1а §5.9, РП-6)', () => {
  ...
  test('приметы ловят все роды разборщика и не ловят данные', () => {
    ...
    const parsers = [
      ...
      'const OPEN = `{{title}}`;',
      'const Q = `{{query:`;',
      // Финальное ревью, MUT-I1: маркер в массиве, множестве, аргументом; `{{` в середине регэкспа.
      "const RECORD_LINES = new Set(['{{title}}', '{{tags}}', '{{body}}']);",
      "['{{title}}', '{{tags}}', '{{body}}'].includes(l.trim())",
      'const L = ["{{body}}"];',
      "isMarker(line, '{{title}}')",
      'const OPEN = [`{{title}}`];',
      '/^\\s*{{query:/.test(l)',
      'export const isQueryLine = (l: string) => /^\\s*{{query:/.test(l);',
      'if (/^\\s*{{(title|tags)}}\\s*$/.test(l)) return;',
      'const ok = cond && /[a-z]+{{/.test(l);',
    ];
    ...
    const data = [
      ...
      'const box = <div style={{ width: 1 }} />;',
      'const ratio = total / count; const o = {{}};',
