 ...
 * (`BUILTIN_PROPERTY_META`, 79 строк) и воспроизводятся тестом, а не переписаны из чужого
 * отчёта.
 */
export const HEURISTIC_ACCURACY_NOTE = [
  'Мерка сходства — ЭВРИСТИКА, и её точность измерена, а не предположена: на встроенном',
  'реестре (79 свойств, 3081 пара) порог 0.5 пересекают 18 пар, и ВЕРНЫХ среди них 0 —',
 ...
 * реестру (79 свойств, 3081 пара): порог 0.5 пересекают 18 пар, ВЕРНЫХ СРЕДИ НИХ 0. Примеры
 * ложных: «Иконка ~ Цвет», «Вид текста ~ Заголовок», «Создана ~ Изменена», «Предложение ~
 * Режим», «Заголовок ~ Шаблон для». Семнадцать из восемнадцати дают РОВНО 0.50 — вырождение
 * знаменателя `min(|A|,|B|)`: на короткой подписи одной общей основы хватает, чтобы пересечь
 * порог.
