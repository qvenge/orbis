    ...
    if (!who.bypassRls) {
      console.error(
        `\nВНИМАНИЕ: роль ${who.role} НЕ несёт BYPASSRLS, а на entities включён FORCE RLS` +
          ...
          '\nи нули выше означают «корпус НЕ ВИДЕН», а НЕ «корпус здоров».' +
 ...
 *
 * Числа — владельцу ДО прода: сколько тел без документа, где строка `{{…}}` станет блоком
 * обвязки, карточкой или контейнером; сколько тел с документом несут такой маркер в `body` (его
 * увидят MCP, откат и первый кадр); сколько получат плашку ошибки разбора и сколько блоков данных
 * начнут рисоваться таблицей или списком. Это не гейт: текст ни в одном случае не теряется.
 *
 * Счёт и порционный цикл — в `apps/server/src/db/census-v3.ts` под тестом без базы; здесь только
 * обвязка. Тела не печатаются: SELECT берёт `body` и `body_doc` ради разбора (формы показа
 * считаются по документу, который покажет чтение), а наружу выходят числа и id.
...
async function censusV3Op(): Promise<number> {
  return withDb(async (sql) => {
    ...
    const r = await censusV3({
      selectBatch: (limit, afterId) =>
        sql<CensusV3Row[]>`SELECT id, body, body_doc AS "bodyDoc"
                           FROM entities
                           WHERE id > ${afterId}::uuid ORDER BY id LIMIT ${limit}`,
    });
