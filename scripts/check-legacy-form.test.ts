...
const SAMPLES: ReadonlyArray<{
  ...
  {
    id: 'p5-gate-docblock',
    // Две формы фразы: «после гейта П5» и «раньше гейта П5» — обе жили в докблоках среза А.
    // Третья строка — заглавная буква: докблок вправе начать предложение с имени условия
    // («Гейт П5 ревизией 3 снят»), а `git grep -P` идёт без `-i`; без этого образца
    // альтернатива `[Гг]` выпала бы из паттерна незаметно.
    lines: [
      'const a = "первый акт среза Б-1 после гейта П5";',
      'const b = "положенная раньше гейта П5";',
      'const c = "Гейт П5 ревизией 3 снят";',
    ],
  },
...
/**
 * ЗАМОРОЖЕННЫЕ ПРОМПТЫ — ПИН СОСТАВА. Обещание живёт в двух местах сразу (глобальное
