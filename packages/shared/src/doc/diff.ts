 ...
 * (у пары Дайс 0.30 при вложении 1.0, у соседа 0.35 без вложения).
 *
 * Прежняя формулировка «отбор может только ДОБАВИТЬ спаривание, ни одного не отнимая» ВЕРНА
 * УЖЕ НЕ БЫЛА — её отменила монотонность по `j` (см. `takenUpTo` ниже). Жадный проход идёт
 * слева направо и, взяв блок правее, закрывает всё левее него для следующих удалённых: пара,
 * которую при другом выборе получил бы следующий removed, у него отнимается. То есть отбор не
 * оптимален глобально и не обязан быть — он обязан не скрещивать пары и не врать про порядок.
 */
