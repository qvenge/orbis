 ...
 *
 * ГРАНИЦА: вставка из ОДНИХ ПРОБЕЛОВ не выживает. Менеджер выносит наружу ВСЕ её пробелы,
 * содержимого не остаётся, и разделители становятся обычным текстом (`` ` ` `` → `` `` `` →
 * канон `` \`\` ``). Пре-существующее и отсюда неисправимое — обёртку спрашивают уже после
 * выноса пробелов. Аудит корпуса этот случай ЛОВИТ краном «канон неустойчив» (проверено на
 * шести формах), так что на прод он молча не проходит.
 */
 ...
 *
 * ГРАНИЦА: обёртка считается по ОДНОМУ ТЕКСТОВОМУ УЗЛУ, а не по всему прогону марки. Для
 * документа из редактора это одно и то же — `Code` объявлен `excludes: "_"`
 * (@tiptap/extension-code), поэтому смежные узлы с этой маркой ProseMirror всегда сливает
 * в один, и прогон длиннее узла не бывает. Рукописный JSON такое собрать может, и тогда каждый
 * кусок получит СВОЮ обёртку; текст при этом цел, а страховка записи всё равно сверит потерю.
 * (Раньше это жило только в отчёте — а отчёт при правке файла никто не открывает.)
 */
