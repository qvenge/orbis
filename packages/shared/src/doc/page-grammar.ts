 ...
 * (задача 8). `{{query:…}}` пока распознают ещё токенайзер `nodes/query-block.ts` — внутри
 * текстовых кусков, где он держит блок в пункте списка, — и первый кадр `bodySegments` (web);
 * первый кадр переходит сюда в задаче 11, сторож «одной копии» — задача 17. Зачем одна копия:
