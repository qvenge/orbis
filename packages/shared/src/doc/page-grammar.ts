 ...
 * (`nodes/query-block.ts`) ВНУТРИ текстовых кусков, где он держит блок с отступом в пункте списка,
 * в цитате и второй блок на строке — прежнее поведение редактора; первый кадр таких блоков не видит
 * (закреплено тестом `apps/web/src/features/browser/query.test.ts`). Зачем одна копия:
