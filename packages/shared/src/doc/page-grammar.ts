 ...
 * единственное место правил контейнеров и блоков обвязки, и `parseBody` идёт поверх него
 * (задача 8). Первый кадр тела в web читает тело этим модулем (задача 11; прежний регэксп
 * `bodySegments` снят). `{{query:…}}` пока распознаёт ещё токенайзер `nodes/query-block.ts` —
 * внутри текстовых кусков, где он держит блок в пункте списка и в цитате; сторож «одной копии» —
 * задача 17. Зачем одна копия:
  ...
  while (pos < src.length) {
    ...
    if (body.startsWith(QUERY_OPEN)) {
      ...
      if (close !== -1) {
        ...
        // строки, и вторая обёртка на той же строке блоком не считается (как у прежнего регэкспа
        // первого кадра, снятого задачей 11).
