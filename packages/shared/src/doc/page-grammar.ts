 ...
 *
 * Одна копия правил маркеров (РП-6). Семейства `{{…}}` распознаются ТОЛЬКО здесь: этим модулем
 * читают тело `parseBody`, первый кадр редактора и рендерер экрана записи. Вторая копия регэкспов
 * рано или поздно разъехалась бы с этой, и человек видел бы на первом кадре один разбор, а в
 * редакторе через мгновение — другой.
 *
...
function lex(src: string): Item[] {
  ...
  while (pos < src.length) {
    ...
    if (body.startsWith(QUERY_OPEN)) {
      const close = src.indexOf(QUERY_CLOSE, pos + QUERY_OPEN.length);
      if (close !== -1) {
