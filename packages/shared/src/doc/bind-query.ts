 ...
 *
 * ЭКСПОРТИРУЕТСЯ РАДИ ОДНОГО ПОТРЕБИТЕЛЯ — редактора блока в web (`QueryWidget.save`).
 * Собирать пару `{ast, text}` он обязан ЭТОЙ функцией, а не своей: решение «что кладём в
 * атрибуты» — одно на обе стороны, и второй его экземпляр (как было до Р-21-8-фикса)
 * означал бы, что гард, поставленный на сервере, клиент обходит собственной сборкой.
 */
export function bindAttrs(attrs: Record<string, unknown>, reg: ParseRegistry): QueryBlockAttrs {
  ...
  // Р-21-8, ВХОД-ДЕРЕВА: печать БЕЗУСЛОВНОГО дерева пуста, то есть от пустого блока оно
  // неотличимо — а значит и смысл у него обязан быть тот же, «блок не настроен», а не «весь
  // корпус владельца». Дореформенный `min(1)` отвергал ровно это состояние; текстовый вход
  // его уже не пропускал (ветка выше), а вход-дерева проезжал: стереть текст в строковом
  // редакторе или снять последний аспект в форме давало `{ast:{filter:null}, text:''}`, и
  // виджет рисовал «Совпадений: N», а `body_doc` хранил это НАВСЕГДА.
  //
  // Предикат именно «печать пуста», а не `filter === null`: заготовка `/`-меню
  // (`{filter:null, sortBy, limit:10, title}`) законна и печатается непустой строкой —
  // сравнение по `filter` убило бы её.
  if (printed === '') return { ast: null, text: '' };
