...
describe('query-блок в диффе: текст из `text`, ключ по `ast`', () => {
  ...
  // Свойство «переименование подписи блок НЕ трогает» проверяется НЕ здесь: реестра у
  // листового `diff.ts` нет, и сверка двух одинаковых деревьев была бы тавтологией — `same`
  // на них вернёт любая реализация. Живой тест — в `bind-query.test.ts`: там один и тот же
