 ...
 * id ссылки. Пробелы выброшены — markdown вправе их переставлять (перевод строки в конце блока,
 * выравнивание таблицы, схлопывание пустых строк), и считать это пропажей нельзя.
 *
 * ПОДПИСИ ссылки здесь НЕТ намеренно. Она не авторский текст, а вмороженный кеш заголовка
 * (EntityChip показывает живой), и по решению находки 2 сериализатор ОПУСКАЕТ подпись со
 * скобкой — то есть считать её пропажей значило бы объявить поломкой собственную починку.
 * Проверено пробой: без этой оговорки чип с подписью «Задача ] хвост» уводил весь документ
 * в raw. Сам факт связи при этом не теряется — за ним следит сверка ссылок ниже.
 */
function writtenText(node: JSONContent | undefined, out: string[] = []): string[] {
  ...
  if (node.type === 'entityRef' && typeof attrs.entityId === 'string') out.push(attrs.entityId);
