...
/**
 * Текстоподобные листья лексера. Разбор склеивает соседние такие токены в ОДИН текстовый узел
 * (`\_курсив\_` — три `escape` и три `text` подряд дают один узел), поэтому в подсчёте они
 * схлопываются — но только В ПРЕДЕЛАХ ОДНОГО контейнера.
 */
 ...
 */
function leafTokenCount(toks: Tok[] | undefined, out: { n: number } = { n: 0 }): number {
  let prevTextish = false;
  for (const t of toks ?? []) {
    if (t.type === 'space') continue;
    const containers: Tok[][] = [];
    if (t.tokens?.length) containers.push(t.tokens);
    if (t.items?.length) containers.push(t.items);
    if (t.nestedTokens?.length) containers.push(t.nestedTokens);
    ...
    if (containers.length === 0) {
      ...
      prevTextish = false;
      for (const c of containers) leafTokenCount(c, out);
    }
  }
  return out.n;
}
...
/**
 * СТРУКТУРНЫЙ критерий: донёс ли разбор столько же содержательных листьев, сколько увидел
 * лексер. Общий приём против целого семейства «разбор молча выбросил узел».
 *
 * ПОЧЕМУ НЕ ПО СЛОВАМ, как было в раунде 6. Словесная проверка строилась как шумонезависимый
 * СЧЁТЧИК ДЛЯ АУДИТА, где размен в сторону молчания оправдан, и была перенесена на роль
 * СТРАХОВКИ ОТ ПОРЧИ, где молчание и есть потеря. Отсюда слепота: выброшенный узел невидим,
 * если его слова уже встречаются в том же блоке. Замерено на бытовом рантбуке:
 *
 *   "1. Установка\n\n   - [ ] выполнить `npm install`\n\n     ```\n     npm install\n     ```"
 *   → вставка удалена ЦЕЛИКОМ, все пять кранов в нуле; « ```sh » спасалась лишь тем, что «sh»
 *     не встретилось в тексте пункта, а одиночные буквы, цифры, эмодзи и иероглиф терялись все.
 *
 * Структурный счёт от состава текста НЕ ЗАВИСИТ и снимает класс разом. Проверено на 65 здоровых
 * телах (заголовки, списки всех видов и вложенностей до четырёх уровней, таблицы с пустыми
 * ячейками, цитаты, ограды, сущности, ссылки, эмодзи, setext, отступный код, экранирования) —
 * ложных срабатываний НОЛЬ; на пяти сломанных формах срабатывает на всех.
 *
 * Чего он не видит: потерю ВНУТРИ сохранённого листа (рваная строка таблицы — лексер обрезает
 * лишние ячейки ещё до нас, и лист один и тот же). Этот класс закрывают отдельные правила
 * (`hasOverwideRow`) и словесный кран аудита, который остаётся ВТОРЫМ, независимым замером.
 */
function blockKeepsItsShape(token: Tok, parsed: JSONContent[]): boolean {
  return leafTokenCount([token]) === leafNodeCount(parsed);
}
 ...
 */
/**
 * Кого АДРЕСУЮТ query-блоки тела: id свойств (`prop`, `has`, `sortBy[].field`), аспектов
 * (`aspect`), ролей (`rel.via`) и сущностей (`rel.of`). Обратный индекс колонки
