...
describe('разбор обязан воспроизвести свой текст — ОБЩИЙ приём (ре-ревью раунда 6)', () => {
  ...
  test('список с ПУСТЫМ пунктом остаётся списком (ре-ревью раунда 8, жадность приёма)', () => {
    // Моя же жадность, которую замер раунда 7 не увидел: нормализация «пустой абзац не лист»
    // закрыла узловую сторону и не закрыла токенную, и любой список с пустым пунктом уезжал
    // в дословный блок. «Чеклист, где последний пункт ещё не набран» — бытовейшая форма;
    // текст оставался цел, но заметка переставала быть чеклистом.
    const widget = (md: string) => types(md).join();
    expect(`1: ${widget('- [ ] набрано\n- [ ] ')}`).toBe('1: taskList');
    expect(`2: ${widget('- [x] сделано\n- [ ] ')}`).toBe('2: taskList');
    expect(`3: ${widget('- [ ] раз\n- [ ] \n- [ ] три')}`).toBe('3: taskList');
    expect(`4: ${widget('- раз\n- ')}`).toBe('4: bulletList');
    expect(`5: ${widget('- раз\n- два\n- ')}`).toBe('5: bulletList');
    // А пустой пункт НУМЕРОВАННОГО списка остаётся дословным блоком — но по другой, давней
    // причине: marked строит для него `listItem` с пустым `content`, документ непригоден по
    // схеме, и его ловит `fitsSchema` (находка A раунда 1, там же и крэш). К структурному
    // критерию это отношения не имеет — проверено снятием критерия.
    expect(`6: ${widget('1. раз\n2. ')}`).toBe('6: rawBlock');
    expect(canonicalizeBody('1. раз\n2. ').body).toBe('1. раз\n2. ');
  });
