...
/**
 * Предикат к manualBucket — «этот прогон запустил владелец кнопкой, а не расписание».
 * Рядом с конструктором, а не литералом `startsWith('manual:')` по вызывающим: префикс
 * объявлен один раз, и все, кто по нему ветвится (стоп-кран, история, ретрай), читают
 * одно и то же место.
 */
export function isManualBucket(bucket: string): boolean {
