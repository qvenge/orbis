...
describe('orbis/agent-run V1 (V1.4)', () => {
  ...
  test('orbis/entity НЕ аспект: имя зарезервировано под колонки строки (D42 ОЧ.13)', () => {
    // Не формальность, а защита от будущей коллизии. `orbis/entity` — псевдо-аспект
    // предусловий (executor.ts, ENTITY_PSEUDO_ASPECT): под ним предусловие адресует КОЛОНКИ
    // строки сущности, а не карту аспектов. Реестр закрыт, и пока этого id в нём нет,
    // ветка по точному имени однозначна. Если однажды кто-то заведёт настоящий аспект
    // `orbis/entity`, предусловия по нему станут молча читать колонку вместо данных
    // аспекта — расхождение вылезло бы не здесь, а в чужом отказе «не выполнено» без
    // видимой причины. Падение этого теста означает: сначала переименовать псевдо-аспект.
    expect(Object.keys(ASPECT_SCHEMAS)).not.toContain('orbis/entity');
  });
});
