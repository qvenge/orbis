  ...
  .object({
    ...
    proposal: z
      .object({
        ...
        //
        // ДВЕ формы, и обе законны в срезе А реформы. Единицей расхождения стало СВОЙСТВО
        // (§А7-3/§А7-4), и реестр свойств принимает у `orbis/run_proposal` только её —
        // значит новый писатель обязан говорить `{property, note}`. Пара «аспект + поле»
        // остаётся в союзе, потому что она лежит в уже записанных прогонах: сузить схему
        // до новой формы значило бы объявить существующие данные невалидными, а обратной
        // функции у перевода нет (у слитых свойств и псевдо-аспекта `orbis/entity`
        // несколько прообразов). Старая половина уходит вместе с модулем перехода.
        mismatches: z
          .array(
            z.union([
              z.object({ property: z.string(), note: z.string().max(500) }).strict(),
              z
                .object({ aspect: z.string(), field: z.string(), note: z.string().max(500) })
                .strict(),
            ]),
          )
          .max(50)
