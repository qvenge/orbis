...
describe('hasValidCalendar: существует ли момент или день (Р-9b-5, I-1)', () => {
  ...
  test('у момента существует и время суток: часы, минуты, секунды', () => {
    ...
    // которого эта проверка есть (`query/parse-ast.ts`, `hasValidCalendar`). Такой
  ...
  test('toParts судит о годе тем же правилом — иначе импорт и запись разошлись бы', () => {
    // `toParts` — вторая дверь того же вопроса «существует ли день»: через неё дату
    // проверяет граница импорта выписки (`contracts/import.ts`). Разъедься она с
    // `hasValidCalendar` на эре, и `0000-01-01` проезжал бы review, чтобы упасть на
    // коммите — то есть отказ пришёл бы позже и не там (находка ре-ревью).
    expect(() => toParts('0000-01-01')).toThrow(RangeError);
    expect(() => toParts('0000-12-31')).toThrow(RangeError);
    expect(toParts('0001-01-01')).toEqual({ y: 1, m: 1, d: 1 });
    // Прочие грани не задеты: форма, месяц, день — как были.
    expect(() => toParts('2026-02-30')).toThrow(RangeError);
    expect(toParts('2028-02-29')).toEqual({ y: 2028, m: 2, d: 29 });
  });

  test('нулевого года не бывает: 0000 отвергается, 0001 и 9999 принимаются (I-5)', () => {
    // Ни григорианский календарь, ни Postgres нулевого года не знают («1 BC → 1 AD»).
    // Проверено на живой базе: '0000-01-01'::date → 22008, '0001-01-01' и '9999-12-31' → ок.
    expect(hasValidCalendar('0000-01-01')).toBe(false);
    expect(hasValidCalendar('0000-12-31')).toBe(false);
    expect(hasValidCalendar('0000-06-15T12:00:00Z')).toBe(false);
    expect(hasValidCalendar('0001-01-01')).toBe(true);
    expect(hasValidCalendar('0001-01-01T00:00:00Z')).toBe(true);
    // Верхней границы нет намеренно: год в паттернах — ровно `\d{4}`, а Postgres берёт
    // до 5874897 AD, так что охранять там нечего.
    expect(hasValidCalendar('9999-12-31')).toBe(true);
  });
