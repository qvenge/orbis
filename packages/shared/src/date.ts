...
/**
 * Минимальный год, который вообще БЫВАЕТ. Единица, потому что нулевого года нет: и
 * григорианский календарь, и Postgres нумеруют эры «1 BC → 1 AD» без нуля между ними.
 * Проверено исполнением на живой базе: `'0000-01-01'::date` → `22008 date/time field value
 * out of range`, `'0001-01-01'::date` → ок, `'9999-12-31'::date` → ок (тем же прогоном).
 *
 * Верхней границы здесь нет и не нужно: год в наших паттернах — ровно `\d{4}`, то есть
 * потолок 9999, а Postgres берёт до 5874897 AD. Появится пятизначный год — появится и
 * граница, но заводить её раньше значило бы охранять то, чего форма не пропускает.
 */
 ...
 *
 * «Существует» включает и ЭРУ: `0000-01-01` проходит форму `^\d{4}-\d{2}-\d{2}$` и
 * календарь месяцев-дней, но нулевого года не бывает — Postgres отвечает на него тем же
 * `22008`, что и на 30 февраля (см. `MIN_CALENDAR_YEAR`).
 *
 ...
 *
 * У парсера §6.1 копия была своя (приватные `isLeapYear`/`daysInMonth`), и трогать её было
 * нельзя (РП-11) — до тех пор календарей в монорепо было ДВА. Задача 21b снесла `parse.ts`
 * целиком вместе с копией: **календарь остался один, этот**. Второго потребителя, который
 * мог бы с ним разойтись, больше нет.
 *
