 ...
 * уже переведён: старую карту шлют web и ещё не переведённые серверные пути (Задачи 13c и
 * 18), новую — всё остальное. ТУЛЫ в число отправителей старой карты НЕ входят с Задачи 12:
 * их контракт (`entityCreateInput` выше) `.strict()` и списка форм не знает вовсе — карта в
 * нём отклоняется разбором.
 *
