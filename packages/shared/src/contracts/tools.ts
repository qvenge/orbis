 ...
 *
 * СТАРОЙ КАРТЫ `aspects` ЗДЕСЬ БОЛЬШЕ НЕТ (Задача 13c): последний её отправитель через
 * роутер — форма правки web — переведён на `props`/`unset`/`aspects.{attach,detach}`, и
...
/**
 * Вход tRPC-роутера `entity.create`.
 *
 * Совпадает с контрактом тула ПОБАЙТОВО с Задачи 13c — старая карта аспектов из него снята
 * вместе с последним web-отправителем (`MemoryRuleCard`). Отдельным именем схема оставлена
 * не по инерции: она отвечает на другой вопрос («что принимает роутер владельца»), и первая
 * же правка одного контракта не обязана быть правкой второго. Ровно так они и разошлись
 * сейчас — у роутера входов на одну форму меньше, чем у исполнителя.
 */
 ...
 * всё-таки объявлено отдельно: они отвечают на разные вопросы («что принимает роутер
 * владельца» и «что умеет исполнитель»), и первое же их расхождение — например, серверный
 * путь с формой, которой у роутера нет, — иначе тянуло бы правку одного контракта во второй
 * молча.
 *
 * СТАРОЙ КАРТЫ `aspects` ЗДЕСЬ БОЛЬШЕ НЕТ (Задача 23, «Пересев мира»): она была последним
 * входом старой формы во всём сервере и жила ровно до перевода серверных путей. Карта
 * объектов на входе теперь не «вторая форма», а нераспознанное значение — `.strict()` и
 * тип `string[]` отвечают на неё `VALIDATION`, и это проверяет приёмочный тест
 * (`executor/legacy-input-rejected.test.ts`).
 */
export const entityCreateExecInput = entityCreateInput.strict();
 ...
 *
 * `aspects` здесь — ровно `{attach, detach}` новой формы, как и у контракта тула: union со
 * старой картой снят Задачей 23 вместе с последними её отправителями внутри сервера.
 */
 ...
 * они разошлись — но НЕ здесь: поле `meta` восстанавливаемой связи несёт ВНУТРЕННЯЯ форма
 * создания (`relationCreateInternalInput`, `executor/executor.ts`), которой в контрактах нет
 * и не будет (модель `meta` не пишет). Общий объект тянул бы правку одного контракта во
 * второй молча — и первая же такая правка прошла бы мимо ревью.
 */
...
export const batchExecuteInput = z
  .object({
    ...
    operations: z
      ...
      .min(1)
      // В-9: предел длины = константа `batch_cap` действий — ОДНА на оба (Р-11). Внутренние
      // пачки (импорт, материализация, undo) идут в `execute` операциями с `batchId`, мимо этого
      // конверта, и капом не задеты: кап стоит на КОНВЕРТЕ ТУЛА, то есть на том, что присылает
      // модель, — и на сохранённой карточке подтверждения, которую `approvePending` разбирает
      // этой же схемой (`toOperations`).
