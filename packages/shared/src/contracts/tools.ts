...
// ---------------------------------------------------------------------------
// Переходная форма `aspects` (РП-3): старая карта «аспект → поля» осталась ТОЛЬКО во
// ВНУТРЕННЕМ надмножестве исполнителя — до «Пересева мира» (Задача 23). Из входов, которые
// принимает роутер владельца, она снята Задачей 13c: все web-отправители переведены на
// новую форму (`props` + `aspects` списком/`{attach,detach}`), включая последний —
// `MemoryRuleCard`.
//
// Объявлена ЗДЕСЬ, до первого потребителя: `const` в TDZ до своей инициализации, и
...
/**
 * `aspects` надмножества создания ДЛЯ ИСПОЛНИТЕЛЯ: новый список ИЛИ старая карта.
 *
 ...
 * против карты объектов), поэтому разбор однозначен.
 *
 * ОСТАВШИЕСЯ отправители старой карты — только серверные пути мимо тулов, и ровно поэтому
 * union живёт в exec-надмножестве, а не в UI: у роутера владельца отправителей карты
 * больше нет (см. шапку блока), и принимать её там значило бы держать открытым вход,
 * которым никто не пользуется, — то есть форму, за которой не следит ни один тест.
 */
