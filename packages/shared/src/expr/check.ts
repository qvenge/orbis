...
export interface ExprScope {
   ...
   * не этим флагом, а `derefDenied` ниже — со своим кодом.
   */
  allowDeref?: boolean;
  /**
   * ОБЛАСТЬ ПРАВИЛА ЗАПИСИ (§Б3-3, Р-И-10, Р-К-13): правило читает СВОЮ запись и ничего больше — `deref`
   * здесь `DEREF_IN_CONSTRAINT` (свой код §С1-2), `agg_via` — `EXPR_TYPE`. Второго флага под `agg_via` нет:
   * запрет один и довод один — чтение соседа делает вердикт записи зависимым от того, кого автор правила
   * не видит. Флаг СИЛЬНЕЕ `allowDeref` и ставится областью ЛЮБОГО правила записи — и C, и T
...
function derefType(
  ...
  // §Б3-3 запрещает `deref` в правилах записи, и код у запрета СВОЙ — `DEREF_IN_CONSTRAINT`
  // (§С1-2, приёмка §С8-29): «правило записи не читает чужую запись» — устройство области, а не
  // промах типа. Ветка стоит ПЕРВОЙ, до `allowDeref`: флаг области правила сильнее разрешения.
  if (s.derefDenied === true) {
    return bad(
      DEREF_IN_CONSTRAINT,
      path,
      'область без разыменования (C-правило записи, §Б3-3)',
      'deref',
    );
  }
  // `allowDeref` остаётся разделителем D/V и прочих областей (предикат набора, формула
  // ведомости): там разыменование не читается — отказ по типу, как и до правил записи.
...
function typeOf(
  ...
  if ('agg_via' in node) {
    // Область правила записи (§Б3-3, Р-И-10): величина СОСЕДА — тот же чёрный ход к чужой
    // сущности, что `deref`; своего кода спека ему не заводит, отказ — по типу.
    if (s.derefDenied === true) {
