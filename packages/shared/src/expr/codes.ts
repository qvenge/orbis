 ...
 * же правилом одной таблицы; бросает его `derefType` чекера по флагу области `derefDenied`.
 */
...
/**
 * §Б3-3/§С1-2 (строка 20): `deref` в области правила записи. Отдельный код, а не прежний `EXPR_TYPE`
 * (им по-прежнему отвечают области без `allowDeref` — предикат набора, формула ведомости): «правило
 * записи не читает чужую запись» — запрет по УСТРОЙСТВУ области, а не промах типа, и владельцу надо
 * сказать именно это. Бросает чекер по флагу области `ExprScope.derefDenied`, поэтому имя — здесь.
 */
