 ...
 * интерфейса. Контракт печатается как лежит в дереве (`class(<id>)`), хотя словарь контрактов у
 * `ParseRegistry` ЕСТЬ (`query/parse-ast.ts`): на выводе стоят пины `print.test.ts`, а у встроенных
 * контрактов id и есть key (§1.3) — печать ключом дала бы тот же текст ценой смены пинов. Потребители
 * печати — дифф Ш1 и строки «было → станет» карточек реестра (`ruleCardText`, предусловие действия).
 *
