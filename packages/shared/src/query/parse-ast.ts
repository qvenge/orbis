 ...
 *
 * **Следствие, названное вслух: значение с пробелом обязано быть в кавычках, и это ломало
 * УЖЕ СОХРАНЁННЫЕ тексты.** Печать старой грамматики выдавала `title=Мои задачи`,
 * `tags=дом дача` и `search=hello world` БЕЗ кавычек — этот разбор их отвергает. Полная
 * опись адресов и их вердиктов — `PRODUCTION_QUERY_TEXTS` в `ast-fixtures.ts`; переводили
 * их Задачи 10c (Agenda и конструкторы web) и 21b (тела сидов и заготовка проекта).
 *
 * **РАЗБОР ТЕПЕРЬ ОДИН.** Пока тексты не были переведены, серверный разбор пробовал сначала
 * этот парсер и при отказе кодом «текст старой формы» откатывался к старому через мост
 * (`legacy-bridge.ts`). Задача 21b удалила и старую грамматику, и мост: фолбэка нет,
 * непринятый текст получает честный отказ с кодом и позицией. Признак, по которому это
 * проверяется, а не подразумевается, — маркер `legacy-grammar` греп-гейта
 * (`scripts/check-legacy-form.ts`), обязанный давать ноль вне allowlist.
 *
 ...
 *
 * Экспортирован по той же причине, что `isOrderedPropertyKind` ниже: «список или нет» — это
 * решение ЯЗЫКА, а не парсера. Живой потребитель снаружи один — конструктор запросов в web
 * (`features/query-builder/model.ts`, поле `FieldRef.list`): он по этому же признаку решает,
 * показать ли множественный выбор значений, и своя копия правила разошлась бы с разбором на
 * первом же новом списочном типе — форма предложила бы ввод, который перестал бы сохраняться.
 * (Заводился экспорт ради переходного моста старой грамматики; мост снят Задачей 21b, а
 * причина держать одно правило на язык и на форму осталась.)
 */
