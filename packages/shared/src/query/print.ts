 ...
 * они собирают запрос из ввода владельца — тега и строки поиска — и отдают его ТЕКСТОМ, а не
 * деревом (форма их вызова — `entity.query({query})`), то есть печатают мимо `printQueryAst`. Своя копия правила квотирования
