...
export interface FieldInfo {
   ...
   * обратное. Добавлять `'time'` в `FieldType` уже незачем: союз читал его исчерпывающе
   * с `never`-гардом старый компилятор, а он снят Задачей 9b — сегодня `FieldType` жив
   * ровно для старого парсера и переходного моста и умирает вместе с ними в Задаче 21.
   *
...
/**
 * Значения enum в ТЕКСТОВОМ виде — ровно в том, в каком их вернёт `aspects_legacy->'A'->>'f'`:
 * сортировка enum сравнивает CASE именно с этой текстовой проекцией (§6.1,
 * `query/compile-ast.ts`, sortItem),
