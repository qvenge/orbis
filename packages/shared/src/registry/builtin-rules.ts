 ...
 * умолчанием очистить нельзя — вопрос владельцу). Явный `null` во входе движок видит отсутствием
 * (РЧ-3-3) и заменяет умолчанием ДО стадии 2 (T-фаза идёт раньше валидации) — ветка «NULL →
 * умолчание» снятого кода перенесена самим движком, паритет.
 *
 * Два следствия «любой правки» для конверта без валюты, лёгшего мимо правила (данные до правила,
 * строка была выключена): на ПОСТОРОННЕЙ правке свойств он получает валюту владельца, и вместе с ней
 * (1) снимается перенос остатка — `dropStaleCarryover` читает «валюты не было → стала» как смену
 * идентичности; (2) правка может упереться в `duplicate_envelope`: до подстановки набор с `NULL`
 * валютой (`IS NOT DISTINCT FROM NULL`) не совпадал с живым конвертом той же категории и периода в
 * валюте владельца, после — совпадает. Корпус близнецов: причина `DEFAULT_ON_EVERY_PROPS_EDIT`.
 */
 ...
 * включённом запрете, и уход тикета из ожидания без снятия вопроса отказал бы `INVARIANT` (ответ на
 * чекпойнт агент-лупа перестал бы проходить). Обратное безопасно: без запрета уборка просто убирает.
 * Порядок выключения поэтому один — сперва запрет, затем уборка; включения — обратный. Сторож —
 * запись правил владельца (`registry/ops.ts`); знание о паре живёт ЗДЕСЬ, рядом с самими строками.
 */
export const BUILTIN_RULE_REQUIRES: Readonly<Record<string, readonly string[]>> = {
  [RULE_TASK_WAITING_ONLY.id]: [RULE_TASK_WAITING_FOR.id],
};
