 ...
 *
 * ЧТО ЗАДАЧА 9 ПОПРАВИЛА В НОРМАТИВЕ, когда он впервые поехал в движок и на сверку §С8-15 (оракул
 * снесён в Б-2, Р-32; эталон — снимок вывода движка) — норматив один, второй декларации не заводится:
 ...
 *  — `spent.materialize` был переведён в `false` (хранилища ещё не было) и возвращён в `true`
 *    задачей 11 вместе с таблицей `envelope_spent_cache` (§Б5-5);
 *  — `rollup.applies_to` сокращён до `['spent','effective_limit']`: оракул агрегировал по дереву ровно
 ...
 *    окно»), а оракул (`comingRows`) не фильтровал — предстоящее поступление владелец обязан видеть;
 *  — `planned` получил `where` с отсечением шаблонов повторения (`not class(recurrence) in templates`):
 *    оракул резал их `notRecurringTemplateSql`, а ребра `instance-of` у шаблона нет, и
 *    `excludes_relation` его не отсекает;
 *  — порядок `planned` переведён на `[date, id]` (был `[date, title]`): у оракула ключ был —
...
export const BUDGET_DEF: BudgetSubscription = {
  ...
  // месяца: так считал и прежний Overview кодом (`phaseOf`, снесён в Б-2) — `today` против
  // `periodStart`/`periodEnd` САМОГО конверта, и у произвольного конверта внутри месяца (§2.9) две
  // мерки дают разные ответы.
  ...
  aggregates: {
    spent: {
      ...
      where: OUTFLOW,
      // §Б5-5: ЕДИНСТВЕННАЯ материализуемая ведомость среза. Флаг — половина клапана:
      // вторая половина (`aggregations.spent.published` у аспекта-конверта) говорит, что
      // величина вообще опубликована. Сняв любую из половин, владелец гасит кэш
      // ДЕКЛАРАЦИЕЙ, не трогая ни строки кода.
      materialize: true,
    },
    effective_limit: {
      kind: 'formula',
    ...
    daily_pace: {
      kind: 'formula',
      scope: 'envelope',
      // Охрана `remaining >= "0"` — по прежнему Overview кодом (`statusOf`, снесён в Б-2; 03-budget
      // §2.9б): на перерасходе темп не показывается ВОВСЕ. Спека §Б5-4 пишет формулу без охраны, и без
      // неё сверка §С8-15 «ноль расхождений» задачи 9 покраснела бы на первом же перерасходе: конверт
      // 1000/1500 дал бы «−29.41 в день» там, где прежний Overview молчал (рулинг Ф-Б1-34); с Б-2
      // поведение держит снимок вывода движка. ОВ-Б1-2 — эррата спеки за владельцем.
      expr: {
        op: 'if',
        args: [
          {
          ...
          },
          {
            ...
            args: [
              { agg: 'remaining' },
              // Конец периода — СЛОТ конверта, а не параметр запроса: день считается до конца
              // СВОЕГО периода (`days_inclusive(today, period_end)`), и у конверта, не совпадающего
              // с месяцем, параметр дал бы другое число дней.
  ...
  // одна фаза — `upcoming` (§2.9а): у закрытого конверта перерасход показать
  ...
  lists: {
    ...
    // `postDue` сегодня (он уже факт, `planned=false`, дата = сегодня), — а в предстоящие он не
    // входит (§2.8).
    //
