 ...
 * проверяемый и двойной, оба — грепом по дереву:
 *  1. `git grep translateProgressSource` даёт ноль вызывающих — переходная карта
 *     (`registry/legacy-field-map.ts`) умирает целиком в Задаче 23, и заворачивать текст
 *     в обёртку становится нечему;
 *  2. `git grep legacyAspectsPatch` даёт ноль — вход исполнителя перестал принимать
 *     старую карту аспектов (`entityUpdateExecInput.aspects` уже не union), то есть
 *     прислать текстовый запрос стало нечем ни тулу, ни web.
 * Вместе со схемой снимается ветка `'text' in src.query` в `goals/progress.ts` — она
 * ссылается на этот же абзац. Пока хоть один из двух грепов не пуст, ветка обязана
 * стоять: без неё запись цели из непереведённого пути отказывала бы схемой.
 */
