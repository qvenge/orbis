...
/**
 * ISO-момент внутри вложенного объекта. Ровно тот же текст, что у ветки `timestamp`
 * генератора схем (`registry/value-schema.ts`, пин — `value-schema.test.ts`): у четырёх полей
 ...
 * НЕРАЗОБРАННОМУ блоку («показывается с ошибкой, как сегодня»). Кто её пишет СЕГОДНЯ: тул и
 * роутер владельца принимают значение свойства как есть, и цель, заведённая текстом запроса
 * (моделью по промпту либо владельцем из UI), приезжает именно так. Расчёт прогресса такой
 * источник НЕ считает: `goals/progress.ts` отдаёт по нему честный `invalid_query` —
 * конвертера старого текста не заводится (база пересевается).
 *
 * ОБА ПРИЗНАКА СМЕРТИ ВЕТКИ `{text}` ВЫПОЛНЕНЫ «Пересевом мира», и это записано здесь,
 * чтобы следующий читатель не искал условие, которого уже нет. Условие было такое:
 *  1. `git grep translateProgressSource` — ноль вызывающих. ВЫПОЛНЕНО: переходная карта
 ...
 * тех пор, пока цель можно завести неразобранным текстом (её пишет и владелец из UI, и
 * модель по промпту). Убрать ветку значит отказывать таким целям на записи, а не «снять
 * мёртвый код», — это решение о поведении, и принимать его молча, попутно, нельзя. Оно
...
const ENTRIES: readonly PropertyEntry[] = [
  ...
  },
  {
    ...
    // Граница ФОРМЫ, а не правило (РЧ-14-2): пустой и пробельный образец совпал бы с чем угодно —
  ...
  // (движок — `executor/ancestors.ts`); строка правила — `nearest_ancestor` на `orbis/project` в сиде
  // (задача 13), имя — константа `RULE_NEAREST_ANCESTOR`, общая с движком.
  {
