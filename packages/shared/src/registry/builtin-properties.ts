 ...
 * тулов. Убрав её сейчас, реестр отвергал бы создание цели из UI ещё до того, как UI
 * научится говорить деревом. Расчёт прогресса такой источник НЕ считает:
 * `goals/progress.ts` отдаёт по нему честный `invalid_query` — конвертера старого текста
 * не заводится (база пересевается).
 *
 * ДАТА СМЕРТИ ветки `{text}`: схема СУЖАЕТСЯ до голого `QUERY_AST_BODY` (второй элемент
 * `anyOf` снимается) вместе со снятием её единственного производителя. Признак момента
 * проверяемый и двойной, оба — грепом по дереву. Считаются совпадения ВНЕ двух абзацев,
 * которые это условие и формулируют (сам этот докблок и его адресат — докблок
 * `progressQuerySchema` в `goals/progress.ts`): иначе греп не дал бы ноль никогда, и
 * исполнитель, выполнивший инструкцию буквально, ветку не снял бы вовсе.
 *  1. `git grep translateProgressSource` — ноль ВЫЗЫВАЮЩИХ вне этих двух абзацев:
 *     переходная карта (`registry/legacy-field-map.ts`) умирает целиком в Задаче 23, и
 *     заворачивать текст в обёртку становится нечему;
 *  2. `git grep legacyAspectsPatch` — ноль ВЫЗЫВАЮЩИХ вне этих двух абзацев: вход
 *     исполнителя перестал принимать старую карту аспектов
 *     (`entityUpdateExecInput.aspects` уже не union), то есть прислать текстовый запрос
 *     стало нечем ни тулу, ни web.
 * Сегодня (на момент Задачи 10b) оба грепа дают по два совпадения вне абзацев —
 * определение и вызов, — то есть признак живой и различающий.
 * Вместе со схемой снимается ветка `'text' in src.query` в `goals/progress.ts`. Пока хоть
 * один из двух грепов не пуст, ветка обязана стоять: без неё запись цели из
 * непереведённого пути отказывала бы схемой.
 */
