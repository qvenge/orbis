...
/**
 * Локаль владельца — та, в которой резолвятся ЗАКАВЫЧЕННЫЕ подписи имён (§А5-3б), печатается
 * label-форма запроса (`query/print.ts`) и подписываются поля и аспекты в web (§А9-2).
 *
 * Своей колонки у неё нет: `user_settings` хранит таймзону, валюту и начало недели, но не
 * язык. Дом ОДИН, и он здесь — рядом с `effectiveLabel` и самим `LocalizedText`: локаль
 * читателя и правило fallback — две половины одного ответа «каким текстом показать запись
 * реестра», и разъехаться им нельзя. Раньше константа стояла литералом на сервере
 * (`query/parse-text.ts`), потом переехала к `ParseRegistry` (`query/parse-ast.ts`); оттуда
 * её увёл ТОТ ЖЕ довод, что и `effectiveLabel` Задачей 12: `registry/*` лежит в эагерном
 * барреле `@orbis/shared`, а `query/parse-ast` — в отдельном входе `@orbis/shared/query`, и
 * подписи полей в web (первый кадр) тащили бы за одной строкой весь разбор запросов.
 * `query/parse-ast.ts` реэкспортирует её под прежним именем — потребители канона Q-AST
 * импорт не меняли.
 *
 * Две локали означали бы текст, который клиент принимает, а сервер отвергает, — и наоборот:
 * с Задачи 10c один и тот же текст запроса разбирают оба.
 */
