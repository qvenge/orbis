  ...
  .object({
    ...
    sources: z
      .object({
        ...
        // `show.prefer` Повестки. Порядок перечня и есть приоритет, и читают его ВСЕ половины Budget
        // (Ф-Б2-25): SQL-ведомости (суммы, кэш `spent`, окна списков), JS-чтение (строки списков,
        // лимит и валюта карточки) и хук записи (по какой привязке выбирать конверт). Пустой — выбора
        // нет: JS-чтение записи с двумя привязками (карточка конверта, строка списка) отказывает
        // `SLOT_AMBIGUOUS` (молчаливый выбор дал бы лимит не из того аспекта), а SQL-ведомости и хук
        // берут привязки по рангу — как до перечня.
        movement: z
          .object({
