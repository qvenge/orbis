  ...
  .object({
    ...
    sources: z
      .object({
        ...
        // (молчаливый выбор дал бы лимит не из того аспекта), а SQL-ведомости и хук берут привязки по
        // рангу — как до перечня. ИМЕНОВАННЫЙ ОСТАТОК: ЧЛЕНСТВО в наборах (`facts`/`plans`, членство в
        // контракте, окно конвертов, `where`) перечень не читает — запись входит в набор, если так говорит
        // ХОТЬ ОДНА привязка, а класс (`classKeyOf`) берётся по рангу. Когда два аспекта связывают один
        // слот (`date`, `planned`, `direction`, `period_*`) разными свойствами, членство и значение могут
        // разойтись (будущая по дате двойника трата войдёт в `facts` по вчерашней дате `orbis/financial`).
        // Встроенный сид так слоты не связывает; закрытие — вместе с `prefer` для членства.
        movement: z
          .object({
