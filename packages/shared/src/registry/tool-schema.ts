 ...
 *
 * Зачем это здесь, в shared, а не на сервере: имя тула нужно ТРЁМ местам — реестру,
 * который его публикует (`tools/registry.ts`), диспатчу, который сверяет вызов с реестром
 * (`tools/dispatch.ts`), и исполнителю, который по имени находит аспект
 * (`executor/executor.ts`). Пока нормализаций было две («/» и «-» → «_» у реестра, только
 * «/» → «_» у исполнителя), они расходились на КАЖДОМ аспекте с дефисом в ключе: модель
 * видела `attach_user_sleep_log`, а исполнитель ждал `attach_user_sleep-log`, и
 ...
 *
 * ПОЧЕМУ НЕ ПРОБА ПРЕФИКСА `action_`. Префикс захватил бы реестровые тулы `action_set`/`action_remove`
 * (задача 10): вызов `action_set` ушёл бы в ветку исполнения действия диспатча и получил бы «действия
 * тула нет», внутри пачки — `RUN_ACTION_IN_BATCH`, а в шаге декларации назывался бы ВЛОЖЕННОСТЬЮ, хотя
 * это «шаг вне словаря» (§Б6-3, `ACTION_STEP_TOOL`). Им регулярка не соответствует по построению —
 * после `action_` у них нет второго сегмента. Перебором словаря действий (а не формой) проверка тоже
 * была бы неверна: шаг, зовущий тул ещё не посеянного действия или самого себя, — та же вложенность,
 * а словарь её не знает.
 */
...
export function isActionToolName(name: string): boolean {
  return ACTION_TOOL_NAME_RE.test(name);
}
