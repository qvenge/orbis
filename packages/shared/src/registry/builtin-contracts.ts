    ...
    sets: {
      ...
      facts: {
        ...
        args: [
          ...
          // материализуется (`executor/executor.test.ts`, тест «absent при заданном `default`
          ...
          // движение из `facts`, а с ним из `spent`; прежний оракул (снесён срезом Б-2) считал
          // `coalesce(planned,false) = false` и такие движения тратой СЧИТАЛ — эталон движка
          // унаследовал это поведение. Форма `not(= true)` тотальна в обоих бэкендах одинаково
