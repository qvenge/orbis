 ...
 * `source_contract`/`target_contract` с Б-2 ПРОВЕРЯЮТСЯ на создании ребра (`assertRoleConstraints`,
 * `assertEndContracts`): из системных ролей контракт объявляет `ticket` — его цель обязана быть
 * завершаемой (`orbis/completable`), и с Б-2 это отказ записи, а не лежащее поле.
 *
