 ...
 * названное спекой 1а §11.2. Колонка `service` здесь не годится: служебность прячет записи из ВСЕХ
 * выдач компилятора запросов (Ф-1а-1), а страница — обычная запись, которую владелец видит в списках.
 * Новая колонка «не предлагать модели» — миграция, которой срез 1а не делает (спека §12).
 *
 * ЧЕГО СПИСОК НЕ ДЕЛАЕТ. До среза 2 `aiInstructions` страницы до модели не доходит вовсе: инструкция
 * аспекта живёт только в описании его `attach_*`-тула, а тула и строки индекса у страницы нет.
 * Core-тулы (`entity_create`/`entity_update`) аспект технически принимают, и от авторства агентом
 * страницу защищает лишь то, что модель об аспекте не знает. Это отступление названо в спеке 1а §11.2
 * и снимается срезом 2.
 */
