...
export const BUILTIN_ASPECT_META: BuiltinAspectMeta[] = [
  ...
  },
  {
    ...
    aiInstructions:
      'Применяй, когда у намерения есть измеримая цель («накопить 300000», «прочитать 24 книги»). target_value — decimal-строка, строго больше нуля. progress_source описывает, ОТКУДА берётся факт: query — запрос §6.1 по сущностям, aggregate — count (считает сущности, field не нужен) либо sum/latest (field обязателен — имя поля аспекта, например amount). current_value считает сервер, обходя граф; никогда не задавай и не правь его сам.',
    tagMappings: ['goal'],
