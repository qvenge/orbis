...
//    с телом («Запись» шаблона хоста) живёт под `display:none` (keepMounted) вместе со своим
//    `useBodySave`: клик в тело под слоем через две секунды сдвинул бы `updated_at` — и
//    предложение стало бы `stale` само от себя (CAS `expectedUpdatedAt`, propose.ts).
//    Размонтирование же дёрнуло бы `flush` отложенной правки, то есть сделало бы ровно то, от
//    чего мы прячемся. Класс вешает DetailScreen — на узел показа записи (`record-area`); слой
//    лишь сообщает ему, развёрнут ли.
//
...
/**
 * Ключ строки предложения — ЛОКАЛЬНЫЙ: им ключуется буфер правок и список React, наружу он
 * не уезжает. Наружу уезжает сам адрес строки (`index` + `field`), который собирается ниже.
 *
 * Прежняя редакция обещала «ту же тройку, что у сервера» — это неправда с Задачи 12, и
 * неправда дважды: у серверного `rowKey` (`routines/edits.ts`) разделитель — нулевой байт,
 * а средний слот значит не «аспект», а признак СНЯТИЯ свойства (`unset`). Совпадать они и
 * не обязаны: ключ здесь различает строки ОДНОГО экрана, а не адресует их серверу.
 *
 * Пара `index` + `field` строки различает исчерпывающе: у одной операции строк столько,
 * сколько полей она правит, `index` у них общий, а `field` внутри операции уникален.
 * Слота под `aspect` в ключе больше нет: строки СТРОЯТСЯ на каждом чтении сегодняшним
 * `describeOperations` (сервер не хранит их готовыми), а он этого ключа не пишет ни в одной
 * ветке — то есть пустым слот был бы ВСЕГДА, включая прогоны, записанные до Задачи 12.
 */
function rowKey(op: ProposalRow): string {
  return `${op.index}:${op.field ?? ''}`;
}
 ...
 *    (`isScalar`, lib/registry/format.ts).
 */
...
/** Сетка «подпись → значение» — та же, что у свойств записи (см. AspectSection.tsx). */
const FIELD_GRID =
    ...
    <li className="flex flex-col gap-1 border-line/60 border-b pb-2 last:border-b-0 last:pb-0">
      ...
      {units !== undefined ? (
        ...
        <dl className={FIELD_GRID}>
          <AspectField
            registry={registry}
            // `op.aspect`, а не `op.aspect ?? ''`: сервер этого ключа не пишет с Задачи 12,
            // и пустая строка означала бы «носитель есть» — из-за неё подпись поля САМОЙ
            // записи (`title`, `archived`) обрывалась в сырой ключ прямо под строкой, где
            // то же поле уже подписано словом.
            aspectId={op.aspect}
            field={op.field ?? ''}
