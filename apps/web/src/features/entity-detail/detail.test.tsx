 ...
 * SIDES_LOADED в Blockers.test.tsx).
 */
...
test('в заметку доезжает ПИСАНЫЙ текст: содержимое неизвестного АТРИБУТА — нет', async () => {
  ...
  // поимённых спецслучая (`rawBlock.attrs.markdown`, `queryBlock.attrs.text`), и текст в
  ...
  seedDraft(
    {
      ...
      doc: {
        ...
        content: [
          ...
          // Второй поимённый спецслучай выжимки — запрос смарт-листа. С версии 2 схемы он
          // лежит в `attrs.text`; докблок `draftAsText` называет именно это поле, и здесь оно
          // становится ПРОВЕРЯЕМЫМ утверждением, а не обещанием.
  ...
  // Половина, которую выжимка ДЕРЖИТ: текст незнакомой ноды и запрос смарт-листа доезжают
  // вместе с обычным абзацем.
...
describe('ADE: тикет', () => {
  ...
  test('назначение С НУЛЯ навешивает аспект: «Сохранить» шлёт aspects.attach вместе со свойствами', async () => {
    /**
     * Регрессия жеста, а не косметика. В старой карте запись аспекта `orbis/assignment`
     * внутрь ключа `aspects` навешивала носитель ЗАОДНО со значениями, и «Сохранить» на
     * неназначенной задаче делала два дела разом. В новой форме (§А1-1) это
