...
export function AssignmentCard({ entity }: { entity: Entity }) {
  ...
  function save() {
    ...
    mutation.mutate({
      ...
      /**
       * НАВЕШИВАНИЕ аспекта — ЯВНОЕ и обязательное. В старой карте запись ключа
       * `aspects: {'orbis/assignment': …}` навешивала носитель заодно со значениями
       * (`legacy-form.ts`), и «Сохранить» на неназначенной записи делала два дела разом. В
       * новой форме (§А1-1) это РАЗНЫЕ вещи: `props` кладут значения, `aspects.attach` меняет
       * интерпретацию, — и без этой строки свойства ложились бы СВОБОДНЫМИ. Карточка после
       * такого сохранения продолжала бы говорить «Не назначен» (признак — список аспектов),
       * `isTicket` оставался бы ложным, а грант не смог бы забрать тикет вовсе: периметр
       * исполнителя отбирает очередь по `'orbis/assignment' = ANY(aspects)`
       * (`agent-loop/queries.ts`), и инвариант «executor=agent ⇔ живой грант» сервер на
       * записи без аспекта не проверяет (`invariants.ts`).
       *
       * БЕЗУСЛОВНО, а не под `!assigned`: `attach` идемпотентен по построению
       * (`applyPropsPatch`: `if (!aspects.includes(id)) push`), в журнал уже навешенный
       * аспект не попадает (дельта считается сравнением состояний), а условие по `assigned`
       * читало бы ВОЗМОЖНО УСТАРЕВШИЙ кэш — сняли назначение в другом месте, и «Сохранить»
       * снова положила бы свойства мимо носителя. Жест этой кнопки — «назначить», и он
       * выражается целиком, а не наполовину.
       */
      aspects: { attach: [ASSIGNMENT] },
    });
  }
