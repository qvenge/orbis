...
function EntityBody({
   ...
   * а плашки живут над вкладками и нажимаются оттуда). Второй источник — `onAccept` редактора,
   * и без него
   ...
   *
   * ЧЕСТНО ПРО ПОКРЫТИЕ: мутант, снимающий обновление снимка на входе (то есть замораживающий
   * его на теле момента МОНТИРОВАНИЯ), в сьюте выживает — сценария, где два этих значения
   * расходятся так, чтобы разошёлся и видимый исход, построить не удалось. Всякий раз, когда
   * кэш успевает сдвинуться до открытия разметки, он сдвигается на то же самое, что показано, и
   * обе редакции дают один экран. Строка оставлена: «снимок на момент открытия» — это то, о чём
   * ветка спрашивает, а совпадение с моментом монтирования держится на стечении, а не на
   * построении. Времязависимый тест ради этого писать не стал — он ловил бы часы, а не дефект.
   */
