...
function EntityBody({
  ...
  /**
   * Уход из тумблера: редактор встаёт ЗАНОВО (экран рисует одно из двух) и берёт `doc`. А тот —
   * из кэша, и он отстаёт от набранного ровно на паузу сохранения. Без этой строки открыть и
   * закрыть тумблер, ничего не тронув, значило бы вернуть на экран текст СТАРШЕ набранного —
   * при том что набранное цело и ждёт отправки.
   *
   * Одна сверка на ЖЕСТ, а не на нажатие клавиши: цена та же, что у любой другой кнопки.
   */
