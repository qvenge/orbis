 ...
 * подают сюда новый экран с теми же фикстурами. Окружение (стек навигации, отключённый простой)
 * ставит вызывающий тест — см. `structure.test.tsx`.
 */
export async function captureDetail(
  ...
  // `registry.effective` зависел бы от того, какая фикстура шла первой, а на холодной версии —
  // ещё и от гонки ответов (версия из `entity.get` переключает ключ реестра посреди его
  // первого запроса, и он уходит вторым).
