...
export function useEntityUpdate(entityId: string) {
   ...
   *
   * Откату этого достаточно: снимок брошенной мутации сделан ДО патча преемника, и вернуть
   * его — значит выбросить чужую свежую правку. Кэш при этом лечит инвалидация в onSettled;
   * опора именно на неё, и в офлайне, где перечитывание не доедет, оптимистичный патч
   * отказавшей мутации на экране задержится.
   */
