 ...
 *    ставит форматтер.
 *
 * Известная граница: строка-имя модуля берётся в одинарных, двойных кавычках или бэктиках,
 * но апостроф внутри пути (в JS он потребовал бы экранирования) разбор не поддерживает.
 */
function parseRuntimeImports(src: string): string[] {
  const code = blankComments(src);
  return [...code.matchAll(/^import\b(?!\s+type\b)[^'"`]*?(['"`])([^'"`]*)\1/gm)].flatMap((m) =>
    m[2] === undefined ? [] : [m[2]],
  );
}
