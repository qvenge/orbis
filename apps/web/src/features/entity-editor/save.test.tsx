 ...
 *
 * Хвост оператора НЕ якорится на `';` в конце строки, и это не мелочь оформления. С таким
 * якорем строка `import { BodyDoc } from '@orbis/shared/doc'; // комментарий` не совпадала
 * вовсе — а ленивое `[\s\S]*?` дотягивалось до `from '…';` СЛЕДУЮЩЕГО импорта, то есть
 * тяжёлый спецификатор не просто терялся, а поглощался соседом (замерено прогоном на самом
 * `EditorShell.tsx`, чья первая строка написана ровно так). Страж при этом оставался
 * зелёным — то есть тревожная кнопка всей двухфазности молчала бы от одного снятого слова
 * `type`. Граница оператора теперь — `;` (`[^;]*?` через неё не перепрыгнет), а хвост после
 * закрывающей кавычки не разбирается вовсе.
 */
function parseRuntimeImports(src: string): string[] {
  return [...src.matchAll(/^import\b(?!\s+type\b)\s*(?:[^;]*?\bfrom\s*)?'([^']+)'/gm)].flatMap(
    (m) => (m[1] === undefined ? [] : [m[1]]),
  );
}
