 ...
 *    ставит форматтер.
 *
 * ДИНАМИЧЕСКИЙ `import('…')` рантайм-импортом здесь НЕ считается — это ленивая загрузка, то
 * есть ровно то, что страж защищает, а не то, на что он краснеет. Отсекается лукахедом
 * `(?!\s*\()`, и без него страж закричал бы на корректной лени: `DetailScreen.tsx` грузит
 * `MarkdownToggle` через `lazy(() => import('…'))`, и от красноты его спасал бы только отступ
 * в две колонки — то есть перенос строки форматтером (замерено прогоном обеих редакций).
 *
 * Известные границы разбора (все ЗАМЕРЕНЫ, все оставлены сознательно):
 ...
 *  - строка `import.meta…` С КОЛОНКИ 1 проходит `^import\b` (после `import` стоит точка, то
 *    есть граница слова), а класс «не кавычка» идёт через переводы строк — и за спецификатор
 *    берётся ПЕРВЫЙ строковый литерал файла, какой бы он ни был. Настоящий импорт при этом не
 *    теряется (замерено: `import.meta.env.DEV;` + `const s = 'hello';` +
 *    `import … from '@orbis/shared/doc'` даёт `['hello', '@orbis/shared/doc']`), так что дыра
 *    ЛОЖНОПОЛОЖИТЕЛЬНАЯ, а не молчаливая: страж покраснеет, а не промолчит. Самая дорогая
 *    форма — `const p = './BodyEditor';` следом за такой строкой: тяжёлым сочтётся строка,
 *    к импортам не относящаяся вовсе. Сегодня не стреляет — ни в одном из семи охраняемых
 *    файлов нет `import.meta` с колонки 1 (проверено на всех семи), а любой отступ уводит
 *    строку из-под `^`. Граница ПРЕДСУЩЕСТВУЮЩАЯ: обе прежние редакции ведут себя так же.
 *    Лечится одним символом в лукахеде выше — `(?!\s*[.(])` вместо `(?!\s*\()`; замерено, что
 *    это расходится РОВНО на этом входе и совпадает на пятнадцати других формах записи и на
 *    всех семи охраняемых файлах. Не применено решением раунда правок 2, а не по незнанию:
 *    правка меняет поведение стража, а раунд был про документы;
...
function parseRuntimeImports(src: string): string[] {
  ...
  return [...code.matchAll(/^import\b(?!\s+type\b)(?!\s*\()[^'"`]*?(['"`])([^'"`]*)\1/gm)].flatMap(
    (m) => (m[2] === undefined ? [] : [m[2]]),
  );
}
...
test('характеризация: `import.meta` с колонки 1 добавляет ЛОЖНЫЙ спецификатор', () => {
  // Тест ЗАКРЕПЛЯЕТ НЕВЕРНОЕ поведение — предсуществующую границу разбора, записанную в
  // докблоке parseRuntimeImports. Он стоит здесь не потому, что так правильно, а чтобы
  // граница была видна прогоном, а не только чтением: сегодня она не стреляет (ни в одном из
  // семи охраняемых файлов нет `import.meta` с колонки 1), но обещать по списку полноту,
  // которой нет, хуже, чем не иметь списка.
  //
  // Дыра ложноположительная: настоящий импорт НЕ теряется — рядом с ним появляется лишний
  // спецификатор, взятый из первого строкового литерала файла.
  const src = "import.meta.env.DEV;\nconst s = 'hello';\nimport { A } from '@orbis/shared/doc';";
  expect(parseRuntimeImports(src)).toEqual(['hello', '@orbis/shared/doc']);

  // ЧИНИШЬ? Замена лукахеда `(?!\s*\()` на `(?!\s*[.(])` закрывает ровно этот вход и ничего
  // больше (замерено на пятнадцати других формах и на всех семи охраняемых файлах). Тогда
  // ожидание здесь станет `['@orbis/shared/doc']`, а запись в докблоке уйдёт из «границ».
});
