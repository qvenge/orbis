...
function Widget({ node, updateAttributes }: NodeViewProps) {
  ...
  return (
    ...
    <NodeViewWrapper data-query-widget="" contentEditable={false}>
      ...
      {editing && (
        // initial — текущий атрибут ноды, а не снимок при открытии: у detail снимок был
        // нужен потому, что body под модалкой мог смениться рефетчем и номер блока указал бы
        // на чужой запрос. Здесь адрес — сама нода: приезд чужой версии документа её либо
        // сохранит, либо снесёт вместе с этим NodeView и модалкой.
