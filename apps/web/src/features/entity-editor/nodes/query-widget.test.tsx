...
test('правка блока не съедает обрамляющие пробелы обёртки', async () => {
  ...
  // ноды такими, какими человек их оставил. Цена ошибки прежняя: сидированные smart lists
  // (§3.3) многострочны и сверяются с PRD байт-в-байт — подстриги их редактор на первой же
  // правке одной клаузы, и сид переписался бы одной строкой.
