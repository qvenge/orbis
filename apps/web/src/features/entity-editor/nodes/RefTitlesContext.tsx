 ...
 * потеря заголовков у хвоста тела. Поэтому режем и запрашиваем пачки: у подавляющего
 * большинства тел пачка ровно одна, а тело с двумя сотнями упоминаний платит одним лишним
 * запросом вместо посеревшего документа. Обещать за нарезкой параллельность нельзя:
 * запросы уходят одновременно, но `httpBatchLink` склеивает их обратно в ОДИН HTTP-запрос,
 * и сервер отработает пачки последовательно. Шторм — это запрос на ссылку, а не на две сотни.
 */
