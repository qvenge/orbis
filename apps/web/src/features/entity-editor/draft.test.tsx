import { type BodyDoc, DOC_SCHEMA_VERSION, parseBody } from '@orbis/shared/doc';
...
//
// Версию подделывает САМ ЧЕРНОВИК (предыдущая — прошлое, `v: 999` — откат релиза) при живой
// константе, а не `vi.mock` модуля: мок поднимается на весь файл, и остальные пятьдесят тестов,
// где `parseBody` даёт документы текущей версии, покраснели бы разом.
//
// «Прошлое» выражено как `DOC_SCHEMA_VERSION - 1`, а не литералом: ветка «версия меньше» обязана
// сторожиться и после следующего подъёма схемы. Конверсия черновика (`upgradeBodyDoc`) идёт
// цепочкой 1 → 2 → 3; версию, для которой шага нет (0, из будущего), честно отдаёт человеку
// («конвертировать нечем»). Формы КОНКРЕТНЫХ версий (атрибут `query` у v1, формат тела v3)
// сторожат тесты с литералами ниже.

/** Черновик ПРОШЛОЙ версии схемы; состав нод — штатный, всё известно. */
const OLD: BodyDoc = { v: DOC_SCHEMA_VERSION - 1, doc: ONE.doc };
...
test('черновик прошлой версии с известными нодами перештампован и уходит штатным досылом', async () => {
  ...
  expect(s.api().pendingDraft).toBeNull();
  expect(s.updates()).toHaveLength(1);
  // Уехала ТЕКУЩАЯ версия: серверный гейт версии (executor) отверг бы прошлую терминально, то
  // есть молчаливый досыл стоил бы записи сохранения до перезагрузки.
  expect((s.input(0) as { bodyDoc: BodyDoc }).bodyDoc).toEqual(ONE);
});

test('перештамповка ПРОГОНЯЕТ документ через конверсию: query-блок приезжает с текстом, а не пустым (долг Задачи 20)', async () => {
  // Долг, названный докблоком `toCurrentSchema` дословно: «следующая правка схемы, которой
  // мало переставить число (сменившиеся АТРИБУТЫ существующей ноды — например запрос
  // смарт-листа), встраивает конверсию сюда». Не встрой — состав нод сошёлся бы (имя ноды не
  // менялось), версия перештамповалась бы, а атрибут остался бы старым: схема v2 его не знает,
  // блок уехал бы на сервер ПУСТЫМ и затёр живой запрос владельца. Это ХУЖЕ сегодняшнего
  // терминального отказа.
  //
  // Дерева здесь НЕ появляется, и это рулинг Р-21-7, а не недоделка: `useBodySave` — файл №1
  // списка стража чанка, и разбор по реестру стоил бы 153 кБ gzip в первый кадр. Конверсия
  // ЛИСТОВАЯ: переносит текст и объявляет блок неразобранным, а дерево соберёт сервер.
  //
  // Версия — литерал 1: атрибут `query` — форма именно v1, и цепочка 1 → 2 → 3 обязана
  // перенести его и на третьей версии схемы.
  seed({
    v: 1,
    doc: {
      type: 'doc',
...
test('черновик v2 из localStorage перештамповывается в v3 и сохраняется (формат тела v3)', async () => {
  // Черновик, набранный до выкатки v3, — самый частый сюжет дня выкатки. Узлы v2 все остались в
  // схеме v3, поэтому спрашивать человека не о чем: `toCurrentSchema` через `upgradeBodyDoc`
  // ставит новый штамп и досылает ТОТ ЖЕ документ — с блочными id, без пересборки.
  const doc = {
    type: 'doc',
    content: [
      {
        type: 'paragraph',
        attrs: { id: 'блок-1' },
        content: [{ type: 'text', text: 'черновик v2' }],
      },
    ],
  };
  seed({ v: 2, doc });
  ...
  expect(s.api().pendingDraft).toBeNull();
  expect(s.updates()).toHaveLength(1);
  expect((s.input(0) as { bodyDoc: BodyDoc }).bodyDoc).toEqual({ v: 3, doc });
});

test('черновик v4 (из будущего) вниз не штампуется — предложение выбором, как сегодня', async () => {
  const v4: BodyDoc = { v: 4, doc: ONE.doc };
  seed(v4);
  ...
  expect(s.updates()).toEqual([]);
  expect(s.api().pendingDraft).toMatchObject({ doc: v4, foreignSchema: true });
  expect(stored().doc).toEqual(v4);
});

test('старая вкладка (документ v2) получает отказ «перезагрузите» — черновик цел на диске (Фокус ревью п. 5)', async () => {
  // Вкладка, открытая до выкатки, шлёт документ v2; серверный гейт версии отвечает VALIDATION
  // (`executor.ts`, текст дословный). Отказ терминален, но набранный текст обязан пережить его:
  // черновик остаётся на диске с пометкой и после перезагрузки перештампуется в v3.
  const old: BodyDoc = { v: 2, doc: ONE.doc };
  const s = mount({
    respond: () => {
      throw trpcError(
        'BAD_REQUEST',
  ...
  s.api().onDocChange(old);
  await tick(SAVE_PAUSE);
  expect(s.updates()).toHaveLength(1);
  expect(stored()).toMatchObject({ doc: old, rejected: true });
});
