    ...
    mutate(
      ...
      { id: entityId, bodyDoc: doc as UpdateBodyDoc, expectedUpdatedAt },
      {
        ...
        onError: (err) => {
          ...
          if (terminal) {
            markDraftRejected(entityId);
            // Запоминаем САМ документ, а не факт отказа: следующая запись черновика обязана
            // вернуть пометку именно ему и никому другому (см. `rejectedDocRef`).
