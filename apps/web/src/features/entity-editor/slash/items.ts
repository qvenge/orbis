 ...
 *
 * Пустой запрос грамматика принимает (проверено пробой на КАНОНЕ, а не на умирающем парсере:
 * `parseQueryAst('')` → `{ok: true, ast: {filter: null}}`; пробел вокруг ничего не меняет), но
 * виджет с ним показал бы счётчик ВСЕХ сущностей владельца без единого слова о том, что это
 * заготовка.
 ...
 *
 * Заголовок В КАВЫЧКАХ, а `updated_at` — namespaced key: пробел стал разделителем конструкций
 * (§А5-3), и незакавыченное «Новый список» разбор режет надвое, а голое имя core-свойства он
 * не резолвит.
 */
