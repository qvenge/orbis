...
async function mountEditor(md: string, handler: (p: string, i: unknown) => unknown) {
  ...
  // Клик — ОДИН раз и здесь: без него user-event не вставляет в contenteditable вовсе
  // (замерено: без клика набор не доезжает, абзац остаётся прежним). Он же и укладывает
  // каретку в начало, поэтому позицию задаём СЛЕДОМ, командой редактора.
