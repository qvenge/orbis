...
export function BodyEditor({
  ...
  useEffect(() => {
    ...
    if (!sameDoc(editor.getJSON(), doc.doc)) {
      ...
      //
      // Внутри ветки — потому что извещать не о чем, когда ничего не менялось: вынеси вызов
      // наружу `if`, и он полетит на КАЖДОМ монтировании редактора (сверка там истинна всегда),
      // объявив показанным документ, который человек лишь увидел.
      //
      // ЧЕСТНО ПРО ПОКРЫТИЕ: такой мутант в сьюте ВЫЖИВАЕТ, и это замерено. Прикрывает его не
      // эта строка, а страж на стороне экрана — выход из режима разметки не сажает показанное,
      // если серверное тело сдвинулось с момента открытия (DetailScreen). Пары «мутант + тест,
      // который его красит» здесь нет, и выдавать одно за другое не буду.
      //
      // Уезжает САМ `doc`, а не пересобранная пара: версия у документа своя, и подставь мы сюда
      // текущую `DOC_SCHEMA_VERSION`, при приезде документа СТАРОЙ версии в реф легла бы пара
      // «новая метка + старое тело» — а отправка сверяет версию строго, то есть уехала бы
      // фантомная правка.
