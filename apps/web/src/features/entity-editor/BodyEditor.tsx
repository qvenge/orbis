 ...
 * `_serializeForClipboard` на первый элемент фрагмента (prosemirror-view), и по нему же
 * `parseFromClipboard` потом восстанавливает границы среза.
 */
 ...
 * Признак подделать можно, и это осознанная цена: разметка тогда идёт в разбор ProseMirror, а
 * рубеж там не наш код, а СХЕМА — `<style>`, `<script>` и `<head>` prosemirror-model
 * выбрасывает сам, ноды и марки вне схемы не создаются, протоколы ссылок режет белый список
 * схемы. Проверено тестом «пропуск по data-pm-slice не проносит ни стилей, ни скриптов».
 *
...
function transformPastedHTML(html: string): string {
  return html.includes(OWN_CLIPBOARD_MARK) ? html : htmlToPlainParagraphs(html);
}
...
export function BodyEditor({
   ...
   * подмена под пустой кареткой — прыжок курсора в начало. Стережём первое.
   */
