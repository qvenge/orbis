...
type BlockTarget = { from: number; node: ProseMirrorNode };

/**
 * Что считать «блоком под выделением» ДЛЯ ПЕРЕМЕЩЕНИЯ.
 *
 * Правило больше НЕ общее с «Удалить блок» (решение И1 раунда правок 1): удаление поднимается
 * до ближайшего предка, чей родитель — документ ЛИБО СПИСОК (см. `rangeToDelete` в
 * BubbleToolbar.tsx), потому что снести чеклист из двадцати пунктов за каретку в седьмом —
 * потеря данных, а перемещение оставлено требованием 4 плана как есть: каретка во вложенном
 * списке двигает ВЕСЬ список верхнего уровня. Согласование гранулярности двух жестов вынесено
 * вопросом к дизайну Задачи 17; до тех пор одно правило в другое не переносить.
 *
 ...
 *    внутри пункта списка или цитаты это внутренний параграф, и жест вырывал бы абзац из
 *    пункта, оставляя пустой каркас. Следствие зафиксировано как поведение (требование 4
 *    плана): каретка во вложенном списке двигает ВЕСЬ список верхнего уровня.
 ...
 */
function blockTarget(selection: Selection): BlockTarget | null {
