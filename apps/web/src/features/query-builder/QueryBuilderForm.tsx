 ...
 * нет — id вводится руками; `this` из web сегодня отвечает ошибкой «this вне контекста
 * сущности» (клиент не передаёт thisEntityId), о чём форма честно предупреждает.
 */
      ...
      {rows.map((row, k) => {
        ...
        return (
          <div key={row.index ?? 'new'} className="flex flex-col gap-1">
            ...
            {row.of?.kind === 'this' && (
              <p role="status" className="text-alert text-xs">
                Экран сущности пока не передаёт контекст блока — такой запрос ответит ошибкой «this
                вне контекста сущности».
              </p>
            )}
          </div>
        );
