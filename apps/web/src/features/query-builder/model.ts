 ...
 *
 * ГРАНИЦА ФОРМЫ, названная вслух: блок хранит ДЕРЕВО (с 21a), но у тела есть вторая, равная
 * по силе форма — markdown-проекция, и текст грамматики плоский (§А5-3д): скобок в нём нет.
 * Поэтому форма работает с ВЕРХНИМ уровнем дерева как со списком конструкций, а OR допускает
 * только внутри одного свойства (`{or:[…]}` по значениям). OR между разными свойствами она
 * собрать не даёт вовсе: собранное не пережило бы круг «печать → разбор» (рулинг Р-21-6,
 * кнопка «ИЛИ между полями» в `QueryBuilderForm` погашена именно этим, а не незавершённостью
 * перехода на AST).
 *
