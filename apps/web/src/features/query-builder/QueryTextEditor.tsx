// BLOCK_END — из `@orbis/shared/query` (сторона запроса), а НЕ из `@orbis/shared/doc`, где та
// же пара символов зовётся QUERY_BLOCK_CLOSE. Причина — СЛОЙ, а не вес: этот файл про текст
// запроса, и рубеж он держит тот же самый, что печать key-формы, — чего не напечатает
// `printQueryAst`, того редактор не даёт и набрать. Возьми он константу у схемы документа —
// редактор запроса стал бы зависеть от чужого слоя.
//
...
export function QueryTextEditor({
  ...
  // записи от одного лишь открытия редактора (см. тесты «„Настроить“ открывает редактор
  // блока с ДОСЛОВНЫМ текстом» и «правка блока не съедает обрамляющие пробелы обёртки»).
  // Довод «иначе схлопнутся многострочные сиды» здесь больше НЕ работает: Задача 21b
  // перевела тела сидов в однострочную печать канона — правило пережило свой первый повод.
  //
  ...
  // переносом или пробелом — рукописного; у сидов их больше нет — показанная позиция смещена на длину
  ...
  // Разбор СТРОГИЙ, как на сервере (`query/parse-text.ts`): редактор обязан считать верным
