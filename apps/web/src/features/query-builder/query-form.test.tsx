...
// OR между РАЗНЫМИ свойствами канон выражает, а плоский текст грамматики — нет (§А5-3д), и
// причина у запрета ПОСТОЯННАЯ: тело живёт и markdown-проекцией, где блок печатается текстом
// (рулинг Р-21-6), — а не «блок ещё не перешёл на AST» (перешёл, 21a). Кнопка обязана быть
// видна и погашена: спрятать её значило бы скрыть, что возможность существует, а дать
// нажать — собрать запрос, который не переживёт круг «печать → разбор».
test('кнопка «ИЛИ между полями» видна, погашена и объясняет причину', async () => {
  ...
  expect(document.getElementById(hintId)).toHaveTextContent('не выражается текстом блока');
});
