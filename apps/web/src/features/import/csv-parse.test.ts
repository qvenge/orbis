...
test('detectDelimiter: перевод строки ВНУТРИ закавыченного поля не режет выборку', () => {
  // Назначение платежа с переносом строки — обычное дело в выгрузках. Раньше пятая
  // физическая строка обрывала образец посреди закавыченного поля, оставляя незакрытую
  // кавычку: остаток файла склеивался в одно поле и счёт колонок перекашивался.
  const text = [
    'Дата;Контрагент;Сумма',
    '01.02.2026;"ООО Ромашка',
    'назначение: аренда";1 890,00',
    '02.02.2026;"ИП Иванов',
    'назначение: услуги";-57,00',
    '03.02.2026;Кафе;-100,00',
  ].join('\n');
  expect(detectDelimiter(text)).toBe(';');
});
