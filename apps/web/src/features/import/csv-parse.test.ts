...
// Уборочная фаза (E10). Два дефекта одной причины — выборка резалась по ФИЗИЧЕСКИМ
// строкам, без учёта кавычек.
test('detectDelimiter: перевод строки ВНУТРИ закавыченного поля не режет выборку', () => {
  // Назначение платежа с переносом строки — обычное дело в выгрузках. Пятая ФИЗИЧЕСКАЯ
  // строка обрывала образец посреди закавыченного поля: незакрытая кавычка склеивала
  // остаток в одно поле, минимум по строкам падал до 1, и побеждал дефолтный «,».
  // Закавыченное поле стоит ПЕРВЫМ намеренно — тогда оборванная запись даёт ровно 1 поле.
  const text = [
    'Контрагент;Дата;Сумма',
    'Кафе;01.02.2026;-100,00',
    'Аптека;02.02.2026;-250,00',
    'Метро;03.02.2026;-60,00',
    '"ООО Ромашка',
    'назначение: аренда";04.02.2026;-1 890,00',
    'Кофейня;05.02.2026;-180,00',
  ].join('\n');
  expect(detectDelimiter(text)).toBe(';');
});

test('detectDelimiter: шапка из ОДНОГО поля не обнуляет оценку разделителя', () => {
  // Реальная шапка выписки: строка-заголовок счёта, дальше нормальная таблица.
  // Минимум по строкам давал 1 для любого кандидата → выбирался дефолтный «,».
  const text = [
    'Выписка по счёту 40817810099910004312',
    'Дата;Контрагент;Сумма',
    '01.02.2026;Кафе;-100,00',
    '02.02.2026;Аптека;-250,00',
  ].join('\n');
  expect(detectDelimiter(text)).toBe(';');
});
