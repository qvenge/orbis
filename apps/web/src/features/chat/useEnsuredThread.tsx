 ...
 * ничего не обещает), а потому, что запись доступна только своему ГРАФУ: id треда —
 * формула от графа И записи (`entityThreadId(graphId, entityId)`), а `chat.ensureThread` —
 * ownerOnly. Чужой актор той же записи не откроет вовсе, а свой получит ровно тот id, что
 * лежит в кеше.
 *
 * У ГЛОБАЛЬНОГО треда ключа нет — потому что модуль НАМЕРЕННО не знает аккаунта. Ключ
...
export function useEnsuredThread(entityId?: string): {
  ...
  useEffect(() => {
    ...
    if (cached !== undefined) {
       ...
       * открыть «Тред» (ensure в полёте) → уйти на «Запись» (вкладка размонтирована, промис
