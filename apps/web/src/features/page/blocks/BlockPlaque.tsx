 ...
 */
/**
 * Реестр не приехал (запрос отказал, снимка нет): блок, которому нужен реестр, не разобран, и ждать
 * бесполезно — «Загрузка…» без конца спрятала бы причину (§6.5; финальное ревью, C1-I3). Один текст
 * на блок данных и на `{{cards}}` рендерера.
 */
 ...
 * лишний блок карточек (`SECOND_BLOCK`) — подсказка того же рода, как лишний `{{body}}`: всё
 * нужное уже нарисовано выше. Сломанная разметка контейнера (коды препрохода) — ошибка, её чинят.
 */
export function issueTone(issue: { code: string }): 'misplaced' | 'error' {
  return issue.code === 'BLOCK_MISPLACED' ||
    issue.code === 'SECOND_BODY' ||
    issue.code === 'SECOND_BLOCK'
    ? 'misplaced'
    : 'error';
}
