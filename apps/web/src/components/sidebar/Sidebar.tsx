...
export function MobileSidebar() {
  ...
  const handleBackdropClick = useCallback(() => closeSidebar(), [closeSidebar]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex md:hidden">
      ...
      />
      {/* Drawer */}
      <aside className="relative flex w-[260px] shrink-0 flex-col bg-surface-dim animate-in slide-in-from-left duration-200">
        <SidebarContent />
      </aside>
    </div>
  );
...
const PinnedEntityRow = memo(function PinnedEntityRow({
  ...
  return (
    <div
      className={`group flex items-center gap-1.5 rounded-md px-2 py-1.5 transition-colors duration-150 ${
    ...
    >
      ...
      <IconButton
        ...
        className="hidden group-hover:block touch-action:block"
      />
    </div>
  );
});
