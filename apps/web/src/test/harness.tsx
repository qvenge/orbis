...
export function installCrashTrap(): void {
  ...
  afterAll(async () => {
    // СЛИВ ОТЛОЖЕННЫХ КОЛБЭКОВ ПЕРЕД КОНЦОМ ФАЙЛА — иначе они догоняют уже снятый jsdom.
    //
    // Замерено, а не предположено: `@tiptap/extension-bubble-menu` дебаунсит обновление
    // панели (`handleDebouncedUpdate` → `window.setTimeout(…, updateDelay)`, по умолчанию
    // 250 мс), а его `destroy()` этот таймер НЕ снимает — то есть последняя правка документа
    // в файле оставляет живой таймер, переживающий и `cleanup()` React, и `editor.destroy()`.
    // Проба на `BodyEditor`: в `afterAll` файла висит ровно один такой таймер. Пока файл
    // заканчивался раньше него, колбэк срабатывал уже ПОСЛЕ снятия окружения и падал
    // `ReferenceError: document is not defined` — Vitest показывал это как «Unhandled Errors»
    // при зелёных тестах (флак CI, локально не воспроизводился: вопрос темпа).
    //
    // Ожидание — ровно окно дебаунса плюс запас, и ровно ОДИН раз на файл (не на тест):
    // цена — доли секунды на файлы, монтирующие редактор, и только на них (страж ставится
    // ими же). Слив идёт ДО сверки и при живом слушателе: если добежавший колбэк всё-таки
    // бросит — это увидит `check` ниже, а не тишина после теста.
