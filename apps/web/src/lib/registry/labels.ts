 ...
 * `@orbis/shared`. Карту сняла contract-миграция 0017, таблицу — «Пересев мира», и вместе с
 * ними ушёл параметр `aspectId`: принимать подсказку, которой нечем воспользоваться, хуже,
 * чем не принимать её вовсе. Вызывающие (`AspectSection.tsx`, `AspectField` в `AspectCards.tsx`)
 * зовут функцию одним адресом.
 *
