...
const FIELD_LABELS: Record<string, string> = {
  ...
  // расхождениях его предусловия `archived` приезжает без аспекта либо под псевдо-аспектом
  // `orbis/entity` (см. ASPECT_LABELS ниже). Без подписи владелец читал бы машинный ключ
  // колонки. Слово — то же, что у сервера (routines/constants.ts CORE_FIELD_LABELS): ленту и
  // карточку он читает подряд, и два разных слова про одно поле были бы двумя полями.
  archived: 'архив',
};
...
const ASPECT_LABELS: Record<string, string> = {
  ...
  // ЗАРЕЗЕРВИРОВАННЫЙ ПСЕВДО-АСПЕКТ (D42 Р0-7, server executor ENTITY_PSEUDO_ASPECT): не
  // аспект вовсе, а адрес КОЛОНКИ строки в предусловии — так выражается «archived absent/false»
  // у отложенной архивации. В реестре аспектов его нет, поэтому ни в форму query-блока, ни в
  // карточки аспектов он не попадёт; подпись нужна ровно расхождениям отложенного действия,
  // где иначе владелец читает 'orbis/entity' сырым (фолбэк aspectLabel ниже).
  'orbis/entity': 'Запись',
};
