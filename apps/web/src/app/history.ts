 ...
 *
 * Историю эта функция НЕ пишет — ни записи, и подписка синхронизации в её работе не
 * участвует: вызывается она ОДИН раз при старте, ДО `installHistorySync`. У пришедшего по
 * ссылке ровно один жест «назад», и вести он обязан на корень целевой вкладки, а не из
 * приложения, — но запись этого корня под записью экрана ставит `seedHistory`, которую App
 * зовёт следом. Здесь её появления не искать и лишним `setState` не добиваться.
 *
 * Проверки «мы уже в целевой позиции — делать нечего» тоже сознательно нет: позиция стора
 * совпадает с целью ссылки в самом обычном случае (ту же ссылку открыли второй раз, persist
 * держит эту сущность наверху), сэкономил бы ранний выход ровно ничего, а разбираться,
 * почему при этом разъехалась стартовая расстановка, пришлось бы долго.
 *
