...
//
// Второй реализации Overview нет с Б-2 (Р-32): эталон движка — снимок `test/golden/budget-engine.json`,
// и пересдаётся он ЯВНО, разбором расхождения, а не записью того, что вышло. Ни одной ветки «по имени
// ведомости» здесь нет: `spent`, `period_balance` и `unbudgeted` различаются РОВНО строением своих
// деклараций (`bound_via` / `unbound_via` / ни того ни другого), и это проверяемо — мутационный тест
// сьюта двигает декларацию, а не код.
//
...
// носитель его параметров с Б-2 — строка каталога `budget_rollover` на `orbis/budget` (Р-К-9), а не
// эта декларация.
//
// ПОЛЯ ДЕКЛАРАЦИИ, КОТОРЫЕ ДВИЖОК НЕ ЧИТАЕТ, — названный остаток (правило 5 §С1-4 «почему кодом»,
// реестры остатков срезов Б-1 и Б-2). Мутация любого из них сегодня ничего не меняет, и молчать об
// этом нельзя: читатель декларации вправе думать, что она вся исполняется.
...
//     их в одно место вместе с переводом хука на контракт срез Б-2 не взялся — остаток до ревизии
//     бюджет-хука.
...
//     половина, и её обобщение по контракту срезом Б-2 не сделано (срез перевёл в декларацию
//     только уникальность конверта — строку `duplicate_envelope`). Движок зовёт тот же
//     `selectEnvelope`, чтобы fast-path и хук не разошлись, но правило выбора читает не он.
 ...
 * предстоящих списаниях. Дом — здесь, а не в `aggregates.ts`: тот импортирует ЭТОТ модуль
 * (обёртки), обратный импорт замкнул бы цикл.
 */
export const HORIZON_DAYS = 14;

/** Разделитель частей ключа сортировки — U+0000: на любом печатном символе порядок разъехался бы
 *  на титулах с общим префиксом («Еда» и «Еда и напитки»). */
...
/** Копии `monthRange`/`shiftMonth` из `aggregates.ts`, а не импорт: `aggregates.ts` зовёт ЭТОТ
 *  модуль обёртками, и обратный импорт замкнул бы цикл. Всё, что нужно обоим по СУЩЕСТВУ (карточка
 *  категории), вынесено третьим файлом (`budget/categories.ts`); календарь правилом не является —
 *  `exact_calendar_month` §3.5 живёт в переходе (`rolloverCreate`, `rolloverPreview`), и оба читают
 *  свой параметр из строки каталога (`rolloverRuleOf`). */
...
function sumLedgerSql(
  ...
  if (agg.unbound_via !== undefined) {
    ...
    // (аспект + обязательные слоты), а не голым аспектом, как у прежнего Overview кодом (снесён в
    // Б-2, Р-32): структурно битый конверт перестаёт прятать трату от Unbudgeted. Перемена названа
    // вслух; на фикстуре и в проде не наблюдаема — валидность конверта держит исполнитель.
 ...
 * в нём незаконен) и заодно требует обязательные слоты — то же, что прежний Overview кодом делал
 * разбором `rawEnvelopeOf` («структурно битый конверт не роняет Overview»). ТЕМ ЖЕ ВЫЗОВОМ идут и `where`
 ...
 * враньём на 50 до следующей инвалидации, а это деньги на экране владельца. На прогретом кэше
 * замок не берётся вовсе — то есть массовое чтение писателям не мешает.
 *
 * КТО СТОИТ С ЧИТАТЕЛЕМ В ОДНОЙ ОЧЕРЕДИ, А КТО НЕТ. Тот же замок первым statement'ом своей
 * транзакции берут все, чью операцию признаёт `touchesBudgetContour` (`executor.ts`):
 * создание и правка сущности по свойствам и аспектам контура, архивация, `attach_*` аспекта
 * контура, `property_merge`/`property_merge_undo` и их проигрыш откатом. Это все боевые
 * писатели ведомости на сегодня.
 *
 * НЕ берёт его прямой `relation_create`/`relation_delete` роли привязки не хуком: предикат
 * смотрит на `props`/`aspects`/`archived` входа, а у relation-операции их нет, и замка не
 * будет. Такой писатель сносит строку конверта (`prepareRelationDelete.apply`) вне очереди,
 * и щель «читатель посчитал под замком → писатель снёс без замка → читатель вставил старое»
 * теоретически открыта. В Б-1 боевого писателя этого класса НЕТ: роль привязки объявлена
 * `created_by: 'system'`, и после Р7 её и ставит, и снимает только сервер — ручной вызов
 * получает `ROLE_SYSTEM_ONLY` (В-П-1, Ф-Б1-45), а хуковый идёт внутри транзакции, которая
 * замок уже держит. Остаются механизмы `seed` (фикстуры) и проигрыш inverse одного ребра —
 * оба вне боевого пути. Закрывается это одной строкой (`touchesBudgetContour` → true для
 * `relation_*` с `role === contour.bindingRole`), и запись здесь стоит именно затем, чтобы
 * день, когда такой писатель появится, не начался с поиска причины.
 */
 ...
 * `remaining`/`daily_pace` считаются от НЕЁ, а не от своего конверта (ровно то, что делал прежний
 * Overview кодом, пересобирая `statusOf` на агрегированных величинах).
 *
 ...
 * прежнего Overview кодом «оба конца — `orbis/category`» НЕ повторяется: это второе мнение о роли, а роли §А4-3 и
...
/** Все потомки (рекурсивно); visited-set страхует от цикла в данных. */
 ...
 * появляется — все читатели зовут один `runLedgers`, и «ноль расхождений» (снимок
 * `test/golden/budget-engine.json`) остаётся сверкой одного кода, а не нескольких.
 *
...
async function runLedgers(
  ...
  // же проектор `toWireEntity`, что у одиночного чтения сущности, значит расхождение по типам
  // колонок исключено построением).
 ...
 * Ведомости считаются на ОДНОМ конверте (`only`), поэтому rollup дерева не применяется: экран
 * быстрой траты показывает СВОЙ конверт, а не сумму поддерева (§4.1 vs §2.10).
 */
