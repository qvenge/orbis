 ...
 * ВКЛЮЧИТЕЛЬНО» (sign-off владельца 2026-07-23). Прежний Overview кодом (оракул, снесён срезом Б-2)
 * писал его целыми: 20·spent ≥ 17·limit. Движок подписки считает тот же порог из декларации §Б5-4 —
 * доля `alerts.warn_at` строкой `"0.85"` (ревизия 3: decimal-литералы E — строки), `isAlert` в
 * `subscriptions/budget.ts`, — то есть через это умножение. Результат, округлённый до двух знаков,
 * сдвинул бы вердикт на конверте с копеечным лимитом: 0.85 · 100.04 = 85.034, округлённое — 85.03,
 * и spent = 85.03 из «не тревога» (целыми: 20·85.03 = 1700.60 < 17·100.04 = 1700.68) стал бы
 * «тревога». Сдвиг даёт округление ВНИЗ; вверх (85.0085 → 85.01)
 * вердикта не меняет — между точным и округлённым порогом там нет ни одного двузначного spent.
 */
