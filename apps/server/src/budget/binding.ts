 ...
 * ведомости `spent`/`unbudgeted` движка подписки (`bound_via`/`unbound_via` декларации
 * `orbis/budget-overview`) и карточка импорта. До 0017 множество было шире
...
export async function rebindForEnvelope(
  ...
  // `notRecurringTemplateSql` в `aggregates.ts` пережила снос оракула, державшего её по РП-4, у
  // одного читателя — запроса категорий без конверта в `rolloverPreview` (решение 4 задачи 11 Б-2).
  const e = sql.raw('e');
  const branches: SQL[] = [];
 ...
 * исполнитель до стадий (`lockBudgetContour`) и движок подписки. Ключ правила исполнитель берёт тоже
 * до стадий (`lockUniqueAmongRules` — по форме входа и по аспектам правимых записей), всегда ПОСЛЕ
 * контурного: глобальный порядок «реестр → контур → правила → строки». Страховочный захват ключа
 * правила движком на стадии 4 и его единственное окно (аспект навешен конкурентом между чтением
 * аспектов и строковой блокировкой) названы в докблоке `lockUniqueAmongRules`. Имя ключа
