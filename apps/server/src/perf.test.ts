 ...
 * прогонов, из них три в составе полного `bun run test`); `goal.progress` — 2026-08-09.
 *
 ...
 * - Повторная локальная калибровка 2026-08-09: за свой максимум вышли ВСЕ шесть, а под
 *   конкурентной нагрузкой (полный `bun run test` — та же форма, в которой гейт идёт на CI)
 *   тем более: 17.9 / 15.8 / 98.7 / 51.1 / 32.5 / 44.6 в порядке таблицы.
 * - Худший фактический запас — ×2.35 (`agenda` 51.1 мс), то есть ниже цели ×3 почти вдвое.
 *
...
const BUDGETS_MS: Record<string, number> = {
  ...
  'goal.progress': 120, // 21.0…28.9 изолированно → 39.6 под нагрузкой (×3.0)
};
...
// источника — треть таблицы (966 расходов при 3037 сущностях фикстуры), а на такой
