...
// entity.count:badge — единицы-десятки мс при пороге 60) seq scan по 2000 строк фикстуры
// стоит слишком мало, чтобы его пробить. Такое видно по печатаемым медианам `perf: …`,
// а не по красному прогону, — за тем они и печатаются всегда (D21).
 ...
 * (первые шесть — 2026-07-31, Apple Silicon, локальный Supabase, одиннадцать прогонов, из
 * них три — в составе полного `bun run test`, где серверный сьют идёт параллельно web и
 * конкурирует за CPU; седьмая, `goal.progress`, — 2026-08-09, шесть прогонов: 24.0 / 24.6 /
 * 25.0 / 25.4 / 26.0 мс изолированно и 39.6 мс под конкурентной нагрузкой). Кратность в
 * скобках — запас относительно ЭТОГО числа.
 *
 * Числа в скобках — ИСТОРИЯ КАЛИБРОВКИ, а не потолок, который где-то держится. Это
 * проверено, а не предположено, и с двух сторон:
 * - CI, семь прогонов `ubuntu-latest` (2026-08-03…2026-08-09): пять медиан из шести
 *   превысили свой локальный максимум — `list50` 20.2 против «≤ 15», `badge` 17.4 против
 *   «≤ 13», `agenda` 40.2 против «≤ 27», `backlinks` 27.9 против «≤ 26», `fastpath` 44.2
 *   против «≤ 34»; под своим числом остался только `budget.overview` (74.8 против «≤ 76»);
 * - повторная локальная калибровка 2026-08-09: изолированными прогонами свой максимум
 *   превысили ВСЕ шесть (16.8 / 15.2 / 106.4 / 32.0 / 27.1 / 37.2 в том же порядке), а под
 *   конкурентной нагрузкой — той самой, в которой гейт идёт на CI, — тем более
 *   (17.9 / 15.8 / 98.7 / 51.1 / 32.5 / 44.6).
 *
 * Отсюда две поправки к тому, что стояло здесь раньше. «Типичное значение ниже, то есть
 * реальный запас больше указанного» — неверно: запас, как правило, МЕНЬШЕ указанного.
 * И объявленный минимум ×3 на практике не держится: худшее наблюдённое — ×2.35 (`agenda`
 * 51.1 мс под нагрузкой), ×2.8 (`budget.overview` 106.4 локально), ×2.97 (`list50` 20.2
 * на CI). Разброс между прогонами доходит до ×4 (`badge` на CI: 4.3…17.4 мс).
 *
 * Сами пороги при этом НЕ подняты: ни один из них ни разу не сработал ложно, а поднимать
 * порог по разбросу раннера значит ослаблять гейт ради тишины.
 *
...
const BUDGETS_MS: Record<string, number> = {
  ...
  'goal.progress': 120, // локально ≤ 26 мс (×4.6), под нагрузкой 39.6 (×3.0)
};
...
// исполняла ни одна операция гейта. Порог здесь ловит то же, что остальные шесть: грубую
// регрессию — потерянный индекс на агрегате, чтение всего графа вместо выборки.
