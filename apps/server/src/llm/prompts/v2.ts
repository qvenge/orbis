// apps/server/src/llm/prompts/v2.ts
// Версионированный системный промпт v2 — статика слоя 1 (§7.1). Carried-решение:
// ИЗМЕНЕНИЕ текста промпта = НОВЫЙ файл vN + новая фикстура v(N).fixture.txt;
// v1 (и его фикстура/тест) остаются нетронутыми — это зафиксированный артефакт.
//
// v2 = v1 + ОДИН новый блок «Продолжения разговора» (слайс 3, решение D19):
// варианты следующей реплики ПОЛЬЗОВАТЕЛЯ модель отдаёт маркером в конце ответа,
// второго вызова LLM ради чипов нет. Разбор маркера — ai/suggestions.ts (единственная
// точка формата): менять форму маркера здесь = менять её и там (гард — v2.test.ts).
// Блок добавлен В КОНЕЦ текста: diff фикстур — чистое добавление.
//
// Остальной текст — дословно v1 (правила поведения, «одна сущность на намерение»,
// шпаргалка грамматики §6, блок Budget, соглашение meta-ключей §3.9, протокол
// tool-результатов MVP). История правок v1 — в шапке v1.ts.

export const SYSTEM_PROMPT_VERSION = 'v2';

/**
 * Маркер протокола tool-результатов не менялся между v1 и v2 — реэкспорт, а не копия:
 * два источника истины у формата, который сериализует toolResultMessage (context.ts),
 * разъехались бы молча.
 */
export { TOOL_RESULT_MARKER } from './v1';

export const SYSTEM_PROMPT_V2 = `Ты — AI-ассистент Orbis: персональной системы, где все знания и дела пользователя — сущности с аспектами в едином графе.
