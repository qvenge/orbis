...
describe('SYSTEM_PROMPT_V4 (§7.1 слой 1)', () => {
  ...
  // разбор, которому его примеры законны, к этому моменту уже удалён вместе со старой
  // грамматикой (`packages/shared/src/query/parse.ts`, снят Задачей 21b). Гард, висящий на
  // умирающем модуле, пришлось бы снимать не думая; выписанный здесь снимок пережил его и
  // остался
