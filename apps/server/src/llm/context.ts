...
//
// §Б7-6: промпт v5 приезжает в канал ДВУМЯ кусками (PROMPT_BODY + CONTINUATIONS_BLOCK) —
...
import { SYSTEM_PROMPT_V5, TOOL_RESULT_MARKER } from './prompts/v5';
...
/**
 * Заголовок блока продолжений — точная подстрока SYSTEM_PROMPT_V5.
 *
...
const CONTINUATIONS_START = SYSTEM_PROMPT_V5.indexOf(CONTINUATIONS_HEADING);
...
if (CONTINUATIONS_START < 0) {
  throw new Error(`SYSTEM_PROMPT_V5 не содержит заголовка «${CONTINUATIONS_HEADING}»`);
}

/**
 * Тело промпта — всё до блока продолжений, ВКЛЮЧАЯ разделитель абзаца перед ним.
 *
 * Обе части ВЫЧИСЛЯЮТСЯ из SYSTEM_PROMPT_V5, а не выписаны текстом: правка промпта — это
 * новая линейка (v6, правило v5.ts:2-5), и копия здесь тихо разошлась бы с оригиналом.
 * Пин конкатенации — llm/context.test.ts.
 */
export const PROMPT_BODY = SYSTEM_PROMPT_V5.slice(0, CONTINUATIONS_START);
/** Блок продолжений — хвост промпта; в канале идёт ПОСЛЕДНЕЙ секцией (§Б7-6-2). */
export const CONTINUATIONS_BLOCK = SYSTEM_PROMPT_V5.slice(CONTINUATIONS_START);
 ...
 * `aspects_legacy`, а три остальных потребителя правил — уже из `props`/`aspects[]`. Где
 * эти колонки расходятся — см. `memory/select.ts`: не на правках через исполнителя (он
 * пишет обе), а на строках ДО миграции 0015 и на прямой записи в БД.
 *
...
export async function loadMemory(tx: Tx): Promise<MemoryItem[]> {
  ...
  const items: MemoryItem[] = rows.map((r) => {
    ...
    /**
     * ПРАВИЛО ПОДПИСИ — ОБЩЕЕ У ДВУХ ЧИТАЮЩИХ ПОВЕРХНОСТЕЙ: здесь и `web/EntityRow`.
     * Вызывателей у `EntityRow` ТРИ (`browser/EntityList`, `settings/MemoryScreen`,
     * `agenda/AgendaScreen` — перечень снят грепом `<EntityRow`, а не памятью); правило
     * работает во всех трёх одинаково, хотя в Повестку запись памяти и не попадает — та
     * отбирает по аспектам расписания и задачи. Подпись пересобирается из свойств тогда и
     * только тогда, когда у правила ЕСТЬ ЦЕЛЬ.
     *
     * Почему граница именно там. Сохранённая подпись может ЛГАТЬ ровно в одном случае — в
     * ней записано имя категории, а категорию переименовали; цель и есть признак того, что
     * имя внутри подписи вообще было. У правила БЕЗ цели (глобальное, не денежное)
     ...
     * заголовок образцом, слой памяти выбросил бы саму формулировку.
     *
     * Цель, которая не разыменовалась (запись снесена), к заголовку НЕ возвращает —
     * подпись остаётся одним образцом: заголовок такого правила по-прежнему несёт имя,
     * которого в графе уже нет.
     *
     * ИСКЛЮЧЕНИЕ НАЗВАНО ВСЛУХ: `web/NativeRow` (экран записи) под это правило НЕ подпадает
     * и показывает сохранённый заголовок как есть — потому что там заголовок РЕДАКТИРУЕТСЯ
     * (`<input>`), и поле обязано показывать то, что лежит в колонке, иначе правка писала бы
     * не то, что видит владелец. Устаревшее имя там видно — рядом с ним стоят живые образец
     * и бейдж категории. Инварианта «устаревшее имя не показывается нигде» НЕТ; он был бы
     * ложным. Тот же адрес и у `web/PinnedList` (закреплённые строки печатают `title`) —
     * поверхность вне скоупа Задачи 18, названа, чтобы не искать её заново.
     */
...
export async function buildContext(tx: Tx, input: BuildContextInput): Promise<BuiltContext> {
  ...
  // побайтно равен SYSTEM_PROMPT_V5 — переставлена СБОРКА, а не текст промпта (РП-18).
