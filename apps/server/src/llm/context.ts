...
export async function loadMemory(tx: Tx): Promise<MemoryItem[]> {
  ...
  const items: MemoryItem[] = rows.map((r) => {
    ...
    /**
     * ПРАВИЛО ПОДПИСИ — ОБЩЕЕ У ДВУХ ЧИТАЮЩИХ ПОВЕРХНОСТЕЙ (здесь и `web/EntityRow`, то есть
     * списки Browser и «Память AI»): подпись пересобирается из свойств тогда и только
     * тогда, когда у правила ЕСТЬ ЦЕЛЬ.
     *
