...
export async function loadMemory(tx: Tx): Promise<MemoryItem[]> {
  ...
  const items: MemoryItem[] = rows.map((r) => {
    ...
    /**
     * ПРАВИЛО ОДНО НА ВСЕ ТРИ ПОВЕРХНОСТИ (здесь, `web/EntityRow`, `web/NativeRow`):
     * подпись пересобирается из свойств тогда и только тогда, когда у правила ЕСТЬ ЦЕЛЬ.
     *
     ...
     * подпись остаётся одним образцом. Это то же решение, что у web, и держит инвариант
     * «устаревшее имя не показывается никогда»: заголовок такого правила по-прежнему несёт
     * имя, которого в графе уже нет.
     */
