 ...
 *  - Набор кладётся ВЕСЬ, поэтому выдача — это `act` ИЛИ САМО НАЗВАНИЕ белого списка, при
 *    любом режиме. Второе — не перестраховка, а закрытая дыра (фикс-раунд 3): молчаливым
 *    шагом `{routine_mode:'propose', allowed_tools:[…]}` модель заранее расширяла
 *    доверенность, а потом просила только режим — и карточка вооружения про инструменты
 *    молчала по правилу «у update молчание значит прежний». Владелец подтверждал `act`, не
 *    увидев, ЧЕМ рутина вооружена. Шума на обычной рутине это не добавляет:
 *    `orbis/routine_mode` в аспекте обязателен, а `orbis/allowed_tools` — нет.
 *
 * ЧЕГО ЗДЕСЬ НЕ ВИДНО — ещё два пути разоружения, различимые только по СОСТОЯНИЮ цели. Оба
 * держит диспатч (`autonomyDisarmedByCarrier`, `tools/dispatch.ts`), потому что классификатор
...
export function grantsRoutineAutonomy(tool: string, input: unknown): boolean {
  ...
  if (tool === 'attach_orbis_routine') {
    return isRecord(input.data) && grantsByCarrier(input.data);
  }
  ...
  if (tool === 'entity_create') return grantsByCarrier(props);
...
/**
 * Выдаёт ли доверенность НАБОР, кладущийся целиком (`data` у `attach_*`, `props` у
 * `entity_create`). Одна функция на оба: это ОДНО понятие «с чем рутина рождается», и
 * разъехавшись, они дали бы разный ответ на один смысл — тот самый класс, на котором ветку
 * уже ловили. Именно поэтому `entity_create` propose-рутины СРАЗУ с белым списком требует
 * карточки так же, как attach: шаг «вооружить молча, спросить только про режим» одинаково
 * проходим обоими.
 */
function grantsByCarrier(carrier: Record<string, unknown>): boolean {
  return carrier[ROUTINE_MODE_PROPERTY] === 'act' || ROUTINE_TOOLS_PROPERTY in carrier;
}
