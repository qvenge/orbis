 ...
 * `aspect_delta_remove` (пять тулов среза А, они уже в `REGISTRY_TOOL_NAMES`), плюс
 * `aspect_create`, `aspect_implements_set`, `aspect_implements_remove`, `subscription_set`,
 * `subscription_remove`, `contract_sets_delta_set`, `contract_sets_delta_remove` (семь тулов
 * вехи II — их кладут задачи 15/16, а ответ политики стоит здесь и РАНЬШЕ них). Переходов у
 ...
 * действия вообще отменяемы моделью, — и разбор ушёл владельцу (Р11 рамки — остаток Б-2).
 * С семью новыми тулами цена этого пути выросла. Фону путь закрыт: `undo_last` в
