 ...
 *
 * Читателей ПЯТЬ, и вопросы у них разные: гейт §7.10 ниже решает, поднимать ли уровень до
 * `explicit-confirmation`; сводка карточки (`tools/dispatch.ts`, `autonomySummary`) — что
 * назвать владельцу; проба разоружения через носитель (`autonomyDisarmedByCarrier`, там
 * же) — что отнимает вызов; и ДВА читателя СОСТОЯНИЯ, которые эту доверенность исполняют, —
 * `routines/runner.ts` (режим прогона и белый список реестра тулов) и `routines/context.ts`
 * (секция режима в системном слое). Пока адреса стояли врозь, карточка могла рассказывать не
 * о том, что подняло уровень: это класс обоих Critical ветки («множество должно быть ОДНИМ у
 * всех потребителей»).
 *
 * ОДНОГО МНОЖЕСТВА МАЛО — читать его все пятеро обязаны ОДИНАКОВО ПОЛНО. Фикс-раунд 2 нашёл
 ...
 * читателя, проверь остальных — унификация имён сама по себе ничего не гарантирует.
 *
 * Двое из пяти живут в `routines/*` и читают доверенность из типизированного `RoutineProps`
 ...
 *
 * СПИСОК ПУТЕЙ ЗДЕСЬ НЕ ИСЧЕРПЫВАЮЩИЙ, и вот известный непокрытый — ПАУЗА
 ...
 * рутина ПО СОСТОЯНИЮ» (`autonomyDisarmedByCarrier` в `tools/dispatch.ts`, рулинг Р-12-3) —
