 ...
 *
 * `undo_last` — ЗАКРЫТ В-8 (Р-31, задача 8 среза Б-2), и путь назван, чтобы его не открыли
 * снова. Владелец подтверждает карточкой слияние свойств, оно ложится в журнал одним action'ом —
 * и следующий `undo_last` ОТ МОДЕЛИ применял бы его inverse (`property_merge_undo`), возвращая
 * словарь назад без всякой карточки. Теперь обратные операции известны ДО исполнения и
 ...
 * (`approvePending`, ключ `undo_of`). Фону путь закрыт по-прежнему: `undo_last` в
 * `ROUTINE_CLOSED_TOOLS` (`tools/registry.ts`).
 */
 ...
 *
 * ПУТЬ `undo_last` ЗАКРЫТ В-8 (задача 8 среза Б-2), и он не про формы вовсе. Воспроизведено
 * живьём (фикс-раунд 6): владелец подтверждает карточкой разоружение рутины, исполнение
 * ложится в журнал одним action'ом, — и следующий `undo_last` от МОДЕЛИ применял его inverse,
 * возвращая `mode: act` и прежний белый список без всякой карточки. Теперь `runUndoLast`
