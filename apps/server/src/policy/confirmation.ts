 ...
 *
 * ВЫКЛЮЧАТЕЛИ ЖИЗНЕННОГО ЦИКЛА. Рутину делают живой ТРИ условия (`activeRoutines`,
 * `agent-loop/queries.ts`): аспект на строке, `orbis/routine_stage: active` и `NOT archived`.
 * Ни одно из трёх не выражается свойствами доверенности, поэтому гейт формы их не видит —
 * состояние спрашивает диспатч (`autonomyChangedByCarrier`):
 *  - АСПЕКТ — держится (рулинг Р-12-5, путь 3 выше);
 *  - АРХИВ — держится (рулинг Р-12-6): `entity_update {id, archived: false}` над вооружённой
 *    act-рутиной возвращает её в отбор, а ряд §7.10 смотрит только `archived === true`.
 *    Обратное направление (архивация) держит сам этот ряд и трогать его незачем;
 *  - `stage` — НЕ держится: `entity_update {props:{'orbis/routine_stage':'paused'}}` одной
 *    строкой останавливает вооружённую act-рутину, и замок молчит. Это не дыра реформы, а
 *    давнее осознанное исключение жизненного цикла (первый абзац: «правка расписания и
 *    жизненного цикла доверенности не касается»), и направление риска у него ДРУГОЕ — пауза
 *    права модели не расширяет, а сужает: не скрытая эскалация, а скрытая остановка.
 *    **Рулинг Р-12-4: в этой задаче НЕ покрывается**, вопрос ушёл владельцу.
 *
 ...
 * Правильная форма правила — предикат по РАЗНИЦЕ: сравнить `autonomyArmed` (и признаки
 * живости) ДО и ПОСЛЕ вызова и поднимать уровень на любом расхождении, каким бы вызовом оно
 * ни пришло; тогда перечислять станет нечего. В срезе А это не переписывается — объём далеко
 * за Задачей 12, — а место такому предикату в части Б, где политика §7.10 становится данными
 * (`assign_level`) и правило пишется строкой, а не веткой.
 *
