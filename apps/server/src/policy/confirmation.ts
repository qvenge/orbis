 ...
 *   запросы переписываются), дельта аспекта (скрытия, состав, наборы вариантов), наборы
 *   контракта и подписки поверхностей (Б-1). §С2-1 ряд 2: «обсудить» — для ЛЮБОГО актора,
 *   у рутин — отложенная единица пачки;
 * - `system-object` — объект НЕ ПРИНАДЛЕЖИТ владельцу: встроенное свойство, встроенный
 *   аспект и его `implements` (Б-1), роли `created_by: system`, определения чужих модулей
 *   (их в Б-1 ещё нет — все пять свои, §Б8-2). §С2-1 ряд 3: фону — запрет ПО ОБЪЕКТУ,
 *   который не откладывается никогда.
 */
 ...
 * `graph_id` строки он не может. Namespace даёт тот же ответ БЕЗ чтения, и даёт его строже:
 ...
 *
 * ПРО ВСТРОЕННЫЕ АСПЕКТЫ ЭТО ЗНАЧИТ БУКВАЛЬНО СЛЕДУЮЩЕЕ: дельта поверх любого из четырнадцати
 ...
 * рутина вправе предложить правку отложенной единицей. Тул, заводящий свой аспект,
 * ПРИХОДИТ ЗАДАЧЕЙ 15 (`aspect_create`), и правило, написанное по АДРЕСУ, встречает его
 * готовым: рождение своего аспекта — `own-property` (чужого этим тулом не завести — `key` в
 * `user/`), привязка СВОЕГО аспекта к контракту — `own-property`, привязка ВСТРОЕННОГО —
 * `system-object` (§С2-1 ряд 1 против ряда 3).
 *
 * АДРЕС ПЕРЕСТАЛ БЫТЬ ЕДИНСТВЕННЫМ ВХОДОМ, и исключение названо поимённо: у подписок
 * поверхностей и наборов контрактов ряд задаёт ТУЛ (Р9), а не адрес объекта, — довод
 * записан в `reconfiguresByTool`. Эта проба на них не зовётся вовсе.
 */
...
/**
 * ОТВЕТ ПО ИМЕНИ ТУЛА; `undefined` — «у классификатора ответа на это имя нет».
 *
 * Вынесено из `reconfiguresOf` ради ОДНОЙ вещи, и она несущая: имя, на которое ответ ЕСТЬ,
 * должно получать его до и независимо от `REGISTRY_TOOL_NAMES`. Семь тулов Б-1 (аспект,
 * привязки, подписки, наборы) заводятся задачами 15/16, а замок §С2-1 обязан встретить их
 * готовым — иначе между «тул есть» и «замок написан» существует окно молчаливой мутации
 * реестра, ровно то, что §С8-23 и запрещает.
 */
function reconfiguresByTool(
  tool: string,
  input: Record<string, unknown>,
): Reconfigures | undefined {
  switch (tool) {
    case 'property_create':
      ...
      //
      // Ветка по статусу — рулинг Р-24-8: «завести сразу активной» приводит ровно в то же
      // состояние, что `proposed` + переход 3, и разница в форме вызова не должна менять
      // замок. Кап неразобранных предложений активную строку не считает, поэтому без этой
      // ветки §А2-7 обходился одним полем конверта.
      return input.status === 'active' ? 'behavior-delta' : 'own-property';
    ...
    case 'property_merge':
      ...
      return ownRegistryAddress(input.source) && ownRegistryAddress(input.into)
        ? 'behavior-delta'
        : 'system-object';
    // ВНУТРЕННИЕ ОБРАТНЫЕ ОПЕРАЦИИ РЕЕСТРА (`REGISTRY_OPS`, `executor/executor.ts`). Дефа у них нет,
    // `dispatchTool` их не резолвит — единственный путь к классификатору это свёртка `action.inverse`
    // в `undo_last` (В-8). До неё их ответ падал в хвост `REGISTRY_TOOL_NAMES.has(tool) ? … : 'none'` →
    // `'none'`, то есть откат подтверждённой правки реестра считался «ничего не перенастраивает» —
    // ровно та дыра, которую докблок `reconfiguresOf` ниже называл известной (Р11 рамки Б-1).
    // Ответ по ТУЛУ, а не по адресу: обратная операция адресована той же строке, что прямая.
    ...
    case 'aspect_delta_set':
      ...
      // их отнесение к классам: это перенастройка ПОВЕДЕНИЯ — членства в классах, а значит
      // чекбокса строки, `class=`-фильтров, Agenda и `spent`. Объект при этом не
    ...
    case 'aspect_delta_remove':
      // У снятия дельты полей `classMap`/`selectOptions` в конверте нет — ответ только по
      // адресу (два `case` раздельно, а не сквозным падением).
      return ownRegistryAddress(input.aspect) ? 'behavior-delta' : 'system-object';
    case 'aspect_create':
      // Своя строка рождается: чужого аспекта этим тулом не завести (конверт держит key в
      // `user/`, §А2-1) — тот же довод, что у `property_create` с `proposed`.
      return 'own-property';
    case 'aspect_implements_set':
    case 'aspect_implements_remove':
      // §С2-1 ряд 1 называет «привязку СВОЕГО аспекта» поимённо, ряд 3 — «`implements`
      // встроенных аспектов». Адрес тут несущий, и это единственные два новых тула, у которых
      // он несущий: у остальных ряд задаёт тул (Р9).
      return ownRegistryAddress(input.aspect) ? 'own-property' : 'system-object';
    case 'subscription_set':
    case 'subscription_remove':
    case 'contract_sets_delta_set':
    case 'contract_sets_delta_remove':
      // РЯД ЗАДАЁТ ТУЛ, А НЕ АДРЕС (Р9 рамки Б-1). Адресное правило дало бы `system-object`
      // на любой дельте поверх встроенного контракта или сидовой подписки — и садовник §Б5-2,
      // добавляющий владельцу набор поверх `orbis/completable`, получал бы запрет по объекту
      // на ЗАКОННОМ пути вместо отложенной единицы. §С2-1 ряд 2 («подписка … дельта поведения
      // … наборы») адресует это «любому актору» — то есть уровню, а не запрету.
      //
      // ГРАНИЦА, КОТОРУЮ ЭТА ВЕТКА НЕ ДЕРЖИТ, НАЗВАНА: ряд 3 говорит про
      // `subscription_definitions` ЧУЖИХ МОДУЛЕЙ, а чужих модулей в Б-1 не существует —
      // все пять свои (§Б8-2). Появятся приложения `<app>/…` (§А3-3) — здесь встанет проба
      // владельца строки, и ответ у неё будет `system-object`.
      return 'behavior-delta';
    case 'action_set':
    case 'action_remove':
      // ТОТ ЖЕ ДОВОД, ЧТО У ПОДПИСОК (Р9): ряд задаёт ТУЛ, а не адрес. Действие — это то, что система
      // ДЕЛАЕТ по одному слову владельца; завести его или снять — перенастройка поведения для любого
      // актора (§С2-1 ряд 2). Адресное правило дало бы `own-property` на своей строке, то есть тихий
      // `preview` фоном на шагах, пишущих деньги.
      return 'behavior-delta';
    case 'rule_set':
    case 'rule_remove':
      // §С2-1 ряд 2 называет ПРАВИЛО прямо («подписка, действие, правило, дельта поведения — любой
      // актор»), и ряд задаёт ТУЛ, а не адрес (Р9): адресное правило дало бы `system-object` на правиле
      // поверх встроенного аспекта — то есть закрыло бы ровно тот путь, ради которого В-6 и завёл
      // дельты правил. Правило меняет то, ЧТО система разрешает и проставляет сама, для каждой записи
      // носителя — перенастройка поведения при любом адресе, и от рутины это отложенная единица D42.
      return 'behavior-delta';
  }
  return undefined;
}
...
/**
 * ЧТО ОДНА ОПЕРАЦИЯ ПЕРЕНАСТРАИВАЕТ В СИСТЕМЕ (§С2-1) — по имени тула, АДРЕСУ ОБЪЕКТА и РОДУ
 * ПРАВКИ.
 *
 ...
 * их подписи и области показа, эффективные определения аспектов, их привязки к контрактам,
 ...
 * задачей 16 перестал быть авансом — пин имён держит `confirmation.test.ts`. Срез Б-2 (задача
 * 10) добавил ещё два — `action_set`, `action_remove`, — нового перехода они не заводят, а ложатся
 * в десятый; задача 16 Б-2 — ещё два, `rule_set`, `rule_remove`, и одиннадцатый переход, итого
 * ШЕСТНАДЦАТЬ. Переходов у этих имён ОДИННАДЦАТЬ, и разрез между ними такой: 1–7 — переходы среза
 * А, где ответ решает АДРЕС объекта; 8–10 — переходы Б-1, и у последнего из них ответ решает ТУЛ
 * (Р9); 11 — правила каталога Б-2, ответ тоже по ТУЛУ:
 *
 *  1. родилась новая своя строка (`property_create`) → `own-property`, если она рождается
 *     ПРЕДЛОЖЕНИЕМ (`status: 'proposed'`), и `behavior-delta`, если сразу АКТИВНОЙ
 *     (рулинг Р-24-8): итог у второй ветки тот же, что у перехода 3 («строка в работе»),
 *     а замок обязан стоять по ИТОГУ, а не по форме вызова (Р-12-4). Довод не
 *     умозрительный: кап §А2-7 считает ТОЛЬКО `proposed` (`registry/ops.ts`), то есть
 *     активная строка проходит мимо разбора пачки вовсе, а живой замер Задачи 17 дал
 *     2 из 2 свойств со `status: 'active'` вопреки описанию тула;
 ...
 *     `proposed → active` вводит свойство в работу (в промпт и на поверхности — часть Б),
 *     `active → deprecated` закрывает запись новых значений, а отклонение неиспользованного
 *     предложения СТИРАЕТ строку совсем (`updateProperty`, ветка DELETE). Тот же ответ даёт
 *     и переход 1 со `status: 'active'` — потому что состояние на выходе у них одно;
 ...
 *  8. родился СВОЙ АСПЕКТ (`aspect_create`) → `own-property`: чужого этим тулом не завести —
 *     конверт держит `key` в namespace `user/` (§А2-1), тот же довод, что у перехода 1;
 *  9. у аспекта появилась, сменилась или снялась ПРИВЯЗКА к контракту
 *     (`aspect_implements_set`, `aspect_implements_remove`) — по АДРЕСУ: своего аспекта →
 *     `own-property` (§С2-1 ряд 1 называет «привязку своего аспекта» поимённо), встроенного →
 *     `system-object` (ряд 3 — «`implements` встроенных аспектов»);
 * 10. сменилась ПОДПИСКА поверхности или НАБОР контракта (`subscription_set`,
 *     `subscription_remove`, `contract_sets_delta_set`, `contract_sets_delta_remove`) либо
 *     заведено или снято СВОЁ ДЕЙСТВИЕ (`action_set`, `action_remove` — задача 10 среза Б-2) →
 *     `behavior-delta` ПО ТУЛУ, а не по адресу (Р9). Адресное правило дало бы `system-object`
 *     на дельте поверх встроенного контракта или сидовой подписки — и садовник §Б5-2 получал
 *     бы запрет по объекту на ЗАКОННОМ пути; довод целиком — в `reconfiguresByTool` выше. У
 *     действия адресное правило ошибалось бы в ОБРАТНУЮ сторону: своя строка дала бы
 *     `own-property`, то есть тихий `preview` от AI на декларации, чьи шаги пишут деньги;
 * 11. заведено, заменено, снято или отключено ПРАВИЛО каталога (`rule_set`, `rule_remove` — задача 16
 *     среза Б-2) → `behavior-delta` ПО ТУЛУ: §С2-1 ряд 2 называет правило прямо, а адресное правило
 *     закрыло бы запретом по объекту правило поверх встроенного аспекта — ровно тот путь, ради
 *     которого В-6 завёл дельты правил.
 * Двенадцатого перехода у этих ШЕСТНАДЦАТИ имён нет: `registry_version` двигает каждый из
 * них, и отдельным переходом она не считается — это следствие, а не жест.
 *
 * Переходы 8–10 заведены раньше своих тулов; с задачей 16 (`subscription_set/remove`,
 * `contract_sets_delta_set/remove`) все три ДОСТИЖИМЫ живьём — путь от вызова до отложенной
 * единицы пинится в `dispatch.test.ts` (§С8-23, ряды 2 и 3).
 *
 * `undo_last` ПРОХОДИТ ЭТОТ ПЕРЕЧЕНЬ с В-8 (Р-31, задача 8 среза Б-2), и путь назван, чтобы его не
 * открыли снова. Владелец подтверждает карточкой слияние свойств, оно ложится в журнал одним
 ...
 * реестра; разоружение и оживление через носитель, которых в форме операции не видно, `runUndoLast`
 * ловит тем же сканом, что `runMutation` (`autonomyChangedByCarrier`). Фону путь закрыт
 * по-прежнему: `undo_last` в `ROUTINE_CLOSED_TOOLS` (`tools/registry.ts`).
 */
export function reconfiguresOf(tool: string, input: unknown): Reconfigures {
  const answered = reconfiguresByTool(tool, isRecord(input) ? input : {});
  if (answered === undefined) {
    // FAIL-CLOSED ХВОСТ: тул реестра, заведённый без ветки выше, — `system-object`, а не
    // молчаливое `'none'`. До этой задачи хвост был другим: имя вне `REGISTRY_TOOL_NAMES`
    // отвечало `'none'` ПЕРВОЙ строкой, то есть мутирующий тул, положенный мимо
    // `REGISTRY_TOOLS` (например в `CORE_TOOLS`), проезжал в `execute` молча. Инвариант
    // против этого — `mcp.test.ts`, «каждый писатель реестра виден классификатору».
    return REGISTRY_TOOL_NAMES.has(tool) ? 'system-object' : 'none';
  }
  ...
  // но ТОЛЬКО у имён, на которые ответ вообще есть: `entity_update` с мусором реестра не
  // трогает, и поднимать его до подтверждения было бы отказом на ровном месте.
  return isRecord(input) ? answered : 'system-object';
}
 ...
 *
 * ПУТЬ `undo_last` ЗАКРЫТ В-8 (задача 8 среза Б-2 и её фикс-раунд 1), и он не про формы вовсе.
 * Воспроизведено живьём (фикс-раунд 6): владелец подтверждает карточкой разоружение рутины,
 * исполнение ложится в журнал одним action'ом, — и следующий `undo_last` от МОДЕЛИ применял его
 * inverse, возвращая `mode: act` и прежний белый список без всякой карточки. Теперь `runUndoLast`
 * (`tools/dispatch.ts`) сворачивает inverse в факты ДО применения, и эта функция отвечает на
 * обратную операцию так же, как на прямую: возврат доверенности — `grantsAutonomy`, карточка.
 * ОДНОЙ ЭТОЙ ФУНКЦИИ ДЛЯ ОТКАТА МАЛО, и это проверено пробой гейта: откат подтверждённой архивации
 * act-рутины или снятия её носителя — `{archived:false}` / `{aspects:{attach:[…]}}` без единого
 * свойства доверенности, форма чиста, а рутина оживает вооружённой. Поэтому `runUndoLast` прогоняет
 * обратные операции и через скан носителя (`autonomyChangedByCarrier`) — с той же оговоркой о гонке,
 * что записана в его докблоке.
 *
 ...
 * разнице нужны оба конца. Значит правило этой формы потребует ещё одного расширения языка —
 * контекста «итог вызова» (`$after`, Е-6 спеки ревизии 4: именованный остаток §С11). Номер Е-5
 * ревизия 4 отдала `$touched` — списку МЕНЯЕМЫХ свойств, заведённому срезом Б-2: он называет, что
 * вызов трогает, но не состояние после него. Пока `$after` нет, `assign_level` умеет
