  ...
  for (const conflict of driftConflictDecidable(args.conflicts)) {
    ...
    const { id } = await createSystemPending(tx, {
      ...
      summary:
        ...
        `(${conflict.targetId}). Принять — снять ваш вариант из настройки; записи, у которых ` +
        `уже стоит «${option.mine}», останутся с ним, и их придётся перевести вручную. ` +
        `Отклонить — оставить оба варианта.`,
