...
//
// ЧТО ЗДЕСЬ НЕ ПРОВЕРЯЕТСЯ. Kind `grant` — ссылка в `agent_grants`, и её держит ссылочный пречек
// назначения (`executor/invariants.ts`, `assertGrantAlive`, Р-К-17): у него сверх существования есть
// своё условие («грант не отозван») и своя точка вызова («назначение затронуто патчем»), и переносить
// его сюда значило бы разводить одно правило по двум домам.
//
 ...
 *
 * ПОЧЕМУ ВСЕ, А НЕ ТОЛЬКО ЗАТРОНУТЫЕ. Все зеркала носят ОДНУ роль `ref`, а `rel_uniq` стоит
 * на тройке (источник, цель, роль) — поэтому пара (источник, цель) вмещает ОДНО ребро роли
 * `ref`, и два ссылочных свойства, смотрящих на одну цель, ДЕЛЯТ его. 0017 этого не
 * изменила: она переставила уникальность с проекции роли на саму роль, а роль у зеркал
 * общая, и подпись свойства живёт в `meta`, вне ключа уникальности. Синхронизируй мы только
 ...
 * займёт единственный слот `rel_uniq` на паре (источник, цель), из-за чего вставка
 ...
 *
 * Чинить это здесь нечем: правильное место — сама операция реестра, которая метит свойство
 * вычисляемым (она же обязана снять его зеркала). Ключ для грепа: `flags.computed`.
 *
 * ДВЕ ФАЗЫ, И ПОРЯДОК ИХ СУЩЕСТВЕНЕН: сначала удаляются ВСЕ устаревшие рёбра, потом
 * вставляются недостающие. Причина — общее ребро двух свойств (`rel_uniq` вмещает одно ребро
 * роли `ref` на пару концов): при обходе «удалил-вставил по одному свойству за
...
export async function syncRefMirror(
  ...
  // (цель, свойство): ребро роли `ref` на паре концов одно, и второе свойство, смотрящее на
  // ту же цель, своего ребра не получит — но и не потеряет чужое (см. докблок).
