    ...
    req(user, [
      {
        ...
        input: {
          ...
          tags: [],
          aspects: ['orbis/memory'],
          // Образец заполнен НАМЕРЕННО: без него раньше ответила бы строка каталога
          // `memory_rule_pattern` (стадия 4, `INVARIANT`), а проверка ссылок идёт позже — и проба
          // перестала бы проверять валидатор ссылок.
          props: {
            'orbis/memory_kind': 'rule',
...
test('ref: два ссылочных свойства на одну цель — одно ребро, а не отказ', async () => {
  ...
  // target_id, role)` вмещает её один раз. 0017 этого не изменила: у роли `ref` проекция в
  // снятую колонку была тождественной, поэтому старый и новый ключ запрещают одно и то же
  // (см. `registry/ref.ts` — фаза 2 считает `remaining` по ЦЕЛИ ровно поэтому).
...
test('ref × merge: слияние переписывает подпись зеркала, висячего ребра не остаётся (§А6-2)', async () => {
  // Подпись зеркала — держатель свойства ВНЕ перечня `collectPropertyHolders` (пятым родом там
  // с Б-2 назван `bind`): она хранит id и переживает слияние. Не переписать её значит вывести ребро из
