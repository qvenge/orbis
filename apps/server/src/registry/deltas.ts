...
export const aspectDeltaSchema = z
  .object({
    ...
    /** Отключённые правила носителя (§С3 «удалить = отключить», §Б4-4, Р-2а). Жест владельца `rule_remove`
     *  кладёт сюда id СИСТЕМНОГО правила, а своё правило дельты снимает из `rules` (строка его и есть).
     *  id СВОЕГО правила сюда кладёт пересев, выключая проигравшее в конфликте с новым системным (`mergeRules`,
     *  Р-И-35: декларация остаётся, исполнение — нет, единица пачки возвращает его), — и может положить сам
     *  владелец дверью `aspect_delta_set` на встроенном аспекте (своей строке дельта правил запрещена,
     *  Ф-Б2-30). `rule_set` своего правила снимает его отключение: «завести заново» — значит «пусть работает». */
 ...
 * (глобальное ограничение: прямые записи сида — только system-строки). Такой конфликт (и совпавший id) сид
 * сообщает ЗАМЕТКОЙ без единицы (`reportOwnRowRuleConflicts`, `db/seed-registries.ts`, — там же, кто побеждает
 * в движке: первый по `(носитель, id)`) и живёт до жеста владельца (`rule_remove` своего или отключение
 * системного — оба проходят: мерка писателей — прирост); слияние свойств его не принимает за своё
 * (`assertMergeLeftRegistryReadable` меряет прирост). Предпросмотр `ops.ts check` этой границы не показывает.
 */
