 ...
 *
 * `base_version` здесь не фильтр: он говорит, на какую версию дельта опиралась, а вопрос
 * предпросмотра — «расходится ли КОД с тем, что в базе», и на него отвечает сравнение
 * `prevSystem`/`nextSystem`. Дельта, уже слитая с текущими строками БД, при совпадении кода
 * и базы конфликтов не даст.
 */
export function previewMergeConflicts(
  ...
  return rows.flatMap((row) => threeWayMerge(prevSystem, nextSystem, row).conflicts);
}
