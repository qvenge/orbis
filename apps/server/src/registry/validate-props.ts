...
// Путь записи перешёл сюда Задачей 4b. «Старый валидатор» golden-корпуса приёмки §С8-1
// (zod-схемы аспектов) снят «Пересевом мира»; его вердикты в корпусе ЗАМОРОЖЕНЫ литералом
// (`test/golden/validator-verdicts.json`, `legacyVerdict`) — история сверки цела, а второго
// описания тех же полей в дереве больше нет.
//
...
export function validateEntityProps(
  ...
  for (const [propertyId, value] of Object.entries(entity.props)) {
     ...
     *
     * ПУТЬ СИДА ЭТО ПРАВИЛО ПРОХОДИТ, а не обходит: с «Пересева мира» мир владельца сеет
