...
//
// Почему это отдельный файл, а не правка `executor/aspects-validate.ts`: тот валидирует
// СТАРУЮ форму по `aspect_definitions.schema` из БД и обязан работать нетронутым до
// пересева (Р-24) — он и есть «старый валидатор» golden-корпуса приёмки §С8-1. Подключение
// нового пути к исполнителю — Задача 4b.
//
