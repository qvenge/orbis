...
const EXPECTED_DIFFS: Record<string, { records: number }> = {
  ...
  // «конверт затронут» — как снятый код поступал с тем же конвертом на правке конверта). Снятая
  // `unset`-ом валюта возвращается умолчанием у обоих — расхождения нет (запись «валюта владельца
  // снята правкой»).
