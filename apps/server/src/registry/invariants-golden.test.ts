...
const EXPECTED_DIFFS: Record<string, { records: number }> = {
  ...
  // Перенос C-4 (решение (в) диспатча), задача 14: умолчание валюты конверта — строка каталога
  // `envelope_currency_default` фазы `default` T-правил, и она исполняется на КАЖДОЙ правке свойств
  // записи, где валюты нет после патча (§Б4-3, §1.5 реестра, рулинг 3-2). Снятый код подставлял её лишь
  // когда патч трогал конверт (`touched.includes('orbis/budget')` на пути правки). Разница видна только
  // на конверте, лёгшем без валюты мимо правила, и на посторонней правке: валюта появляется, а перенос
  // остатка снимается как при смене идентичности (`dropStaleCarryover` после T-правил, без гейта
  // «конверт затронут» — как снятый код поступал с тем же конвертом на правке конверта). Второе
  // следствие того же класса (Fable M-3 задачи 14), записью корпуса не снятое: подставленная валюта
  // может сделать такой конверт ДУБЛЕМ живого конверта той же категории и периода в валюте владельца —
  // до правки набор с `NULL` валютой (`IS NOT DISTINCT FROM NULL`) с ним не совпадал, — и посторонняя
  // правка получит `INVARIANT duplicate_envelope`; снятый код на такой правке валюту не подставлял.
  // Снятая `unset`-ом валюта возвращается умолчанием у обоих — расхождения нет (запись «валюта
  // владельца снята правкой»).
  DEFAULT_ON_EVERY_PROPS_EDIT: { records: 2 },
};
 ...
 *
 * Тридцать — задача 14 (остальные инварианты §А7-2): назначение (шесть — обе половины условия,
 ...
 * валюты конверта (пять — включая перенос остатка при подстановке и при снятой валюте) и «чего ждём»
 * (девять — включая снятие и повторное навешивание `orbis/task`).
 ...
 * инвариантов — тем же прогоном, в котором прежние двадцать восемь записей совпали побайтно. Ещё две
 * (конверт с переносом: умолчание на посторонней правке и снятая валюта владельца) сняты тем же живым
 * старым кодом на коммите формы правила памяти задачи 14 — последнем, где подстановка валюты ещё жила
 * кодом.
 */
const CORPUS_SIZE = 58;
