...
export interface RegistrySnapshot extends RegistryDictionaries {
   ...
   * дельт нет, и снимок сам есть эта система (`applyDeltas` без дельт возвращает вход). Читатель ОДИН —
   ...
   * `effectiveRules`) его не отменяет. Движок, сторожа, валидатор и замки читают ЭФФЕКТИВНЫЕ словари
