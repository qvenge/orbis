...
// apps/server/src --include='*.ts' | grep -v test`), их ТРИ, и каждый — со своей причиной,
// а не по недосмотру:
//  1. `registry/roles.ts` → `hierarchicalRolesSql` (круг исполнителя, `agent-loop/queries.ts`,
//     три места): эффективные строки нужны ВНУТРИ компилируемого SQL. Дельт не видит, и
//     это безопасно — их форма флаги ролей не трогает (см. докблок `roles.ts`);
//  2. `routers/aspect.ts` → `aspect.list`: ему нужна колонка `schema` СТАРОЙ формы (Р-24),
//     которой в декларации реестра нет вовсе. Расхождение с дельтами названо в его докблоке;
//  3. `tools/registry.ts` → `loadAspectToolRows`: секции промпта нужны `id` и
//     `ai_instructions`, а `ai_instructions` дельта не меняет (`aspectDeltaSchema` его не
//     содержит) — расхождения нет по построению.
