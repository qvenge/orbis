...
// apps/server/src --include='*.ts' | grep -v test`), он ОДИН, и у него своя причина:
...
// Последним ушёл `tools/registry.ts` → `loadAspectToolRows` (срез 1а, §10): его единственный
// потребитель, секция «Инструкции активных аспектов» промпта, заменена индексом аспектов
// (`llm/aspect-index.ts`), а индекс показывает подпись и описание — ровно то, что меняет
// дельта, — и потому читает ЭФФЕКТИВНЫЙ снимок.
//
