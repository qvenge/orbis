 ...
 *
 * Имена держателей запросов резолвятся в id через `aliasIndex`: текст запроса называет
 * свойство ключом, дерево — идентификатором, а ответ владельцу обязан быть один.
 */
export function dependencyGraph(reg: RegistrySnapshot, usages: DependencyUsages): DependencyGraph {
  ...
  for (const [holder, names] of usages.queryRefs) {
    ...
    for (const name of names) {
      ...
      // Имя, которого в реестре нет, — это опечатка владельца в тексте запроса, а не узел
      // графа: рисовать её зависимостью значило бы обещать, что она на что-то влияет.
 ...
 * свойству, от свойства к аспекту и от свойства к его преемнику, а компактация (§А10-2)
 * не даёт цепочке `merged_into` вырасти длиннее одного шага. Проверка заведена сейчас
 * потому, что первое ребро «свойство → правило → свойство» приезжает с частью Б (§Б4), и
 * заводить сторожа ВМЕСТЕ с тем, что он сторожит, — это заводить его после первой аварии.
 *
