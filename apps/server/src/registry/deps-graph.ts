...
import { propertyNamesInExpr } from '@orbis/shared/expr';
...
export interface DependencyEdge {
  /** Кто зависит: id аспекта, свойства (в том числе свойства, которое пишет правило) либо держателя. */
  from: string;
 ...
 * считать одним держателем. Собирает её `collectPropertyHolders` (`registry/ops.ts`, шесть родов
 * держателей — с задачи 16 и правила своих строк-носителей), она же кормит переписывание ссылок при
 * слиянии: один обход на оба вопроса.
 */
 ...
 *
 * ШЕСТЬ РОДОВ РЁБЕР — пять среза А и шестой среза Б-2, и каждый ловит свой способ «встать на
 * свойство»:
 ...
 *  - `rule` — правило каталога (§Б4, Р-И-22): свойство, которое правило ПИШЕТ (`default`,
 *    `on_enter_class` — вход и снятие при уходе), зависит от свойств, которые оно ЧИТАЕТ (`when`,
 *    значение, свойство-событие). Второй после `merged_into` род, способный замкнуть граф: правило,
 *    пишущее A по значению B, и правило, пишущее B по значению A, по отдельности законны, и
 *    круг между ними виден только графу — ловит его `assertAcyclicGraph` во врезке записи
 *    (`registry/rules.ts`).
 *
...
export function dependencyGraph(reg: RegistrySnapshot, usages: DependencyUsages): DependencyGraph {
  ...
  // ШЕСТОЙ РОД (§Б4, Р-И-22): ПИСАТЕЛЬ зависит от читаемого. Рёбра дают только `on_enter_class` и
  // `default` — те, кто ПРОСТАВЛЯЕТ свойство; предикаты не пишут ничего, и стратифицировать их не по чему.
  // Это единственное направление, способное замкнуться: правило, пишущее A по значению B, и правило,
  // пишущее B по значению A, до §Б4 нашлись бы только на исполнении.
  for (const { rule } of rulesOf(reg)) {
    if (!rule.enabled) continue;
    const reads = new Set<string>(rule.when === undefined ? [] : propertyNamesInExpr(rule.when));
    const writes: string[] = [];
    if (rule.template === 'default') {
      writes.push(rule.params.property);
      for (const n of propertyNamesInExpr(rule.params.value)) reads.add(n);
    } else if (rule.template === 'on_enter_class') {
      if ('property' in rule.params.enter) reads.add(rule.params.enter.property);
      if (rule.params.set !== undefined) {
        writes.push(rule.params.set.property);
        for (const n of propertyNamesInExpr(rule.params.set.value)) reads.add(n);
      }
      // Снятие свойства при уходе — тоже ЗАПИСЬ: ключ `RULE_CONFLICT` её уже так и считает, и граф,
      // считающий иначе, разошёлся бы с валидатором на первом же `waiting_for` (задача 14).
      for (const p of rule.params.on_leave?.unset ?? []) writes.push(p);
    } else continue;
    for (const from of writes) {
      for (const to of reads) if (from !== to) edges.push({ from, to, kind: 'rule' });
    }
  }
 ...
 *
 * РЁБРА СРЕЗА А ЗАМКНУТЬСЯ НЕ МОГУТ, и это утверждение, а не надежда. Они идут от аспекта к
 ...
 * шага. Проверка была заведена срезом А, ДО первого ребра, способного замкнуться: заводить
 * сторожа вместе с тем, что он сторожит, — значит заводить его после первой аварии. Ребро
 * «свойство → правило → свойство» (`rule`) приехало срезом Б-2; сторожа зовут писатели правил
 * владельца (`registry/ops.ts`: `assertRuleInvariants`), слияние свойств (`assertMergeLeftRegistryReadable`)
 * и тесты (`registry/rules.ts`: `assertRulesOfRow` и `assertBuiltinRules` над снимком из кода).
 *
