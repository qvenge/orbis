...
//   → два объявления в `load.ts` и ШЕСТЬ вызовов в `registry/ops.ts`, больше ничего.
// Шесть — это три пары: снимок для правил создания (`currentRegistry`), проба будущего
// снимка перед КАЖДОЙ записью определения (`assertRegistryStaysReadable`) и проба
// применимости дельты (`setAspectDelta`). Пары, а не одиночные вызовы, потому что снимок
// без дельт — не снимок: сумма считается одна и тем же `applyDeltas`.
//
