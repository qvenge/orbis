...
export function resolveMergePair(
  ...
  // Д4: ИСТОЧНИК — НОСИТЕЛЬ ПРАВИЛ (финал Б-2 E-3, рулинг Ф-Б2-32). Шестой род держателя переписывает адреса
  // ВНУТРИ правил, но не их НОСИТЕЛЯ, а носитель у правила — смысл, не адрес: умолчательная область
  // (`effectiveRuleScope` → `{property: source}`) и касательство `{role}`-области (`carrierTouches`). После
  // слияния значений у источника нет, дельта `add[]` переписана на цель, — `ruleTouchesRecord` ложь на каждой
  // записи, правило молча мертво, а отчёт слияния — «успех». Перенос на цель — не молчаливый выбор операции:
  // встроенная цель правит правила только дельтой (новая строка `registry_deltas`, которой `MergeInverse` не
  // откатит), у цели свои писатели, а правило могло быть написано именно про источник. Громкий исход — выбор
  // `MERGE_RULES_CONFLICT`: снять правила источника (`rule_remove`) и завести их на цели. Считаются ВСЕ
  // правила строки, и выключенные: включённое потом, оно было бы тем же мёртвым.
  if (source.rules.length > 0) {
    const rules = source.rules.map((r) => r.id);
    throw new ExecError(
      'REGISTRY_CONFLICT',
      `на ${source.key} стоят правила (${rules.join(', ')}) — после слияния они не касались бы ни одной ` +
        `записи: снимите их до слияния и заведите на ${into.key}`,
      { reason: 'MERGE_SOURCE_RULES', source: source.id, into: into.id, rules },
    );
  }
  return { source, into };
}
