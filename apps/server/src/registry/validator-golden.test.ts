 ...
 * свойство дважды, а плоский `props` такого просто не выражает. Три записи корпуса поэтому
 * стали позитивными («два аспекта делят одно значение» — ровно то, что В1 и объявляет
 * законным), а ОТКАЗ на противоречивом входе остался там, где он и происходит, — на границе
 * исполнителя: `executor/props.test.ts` («financial+budget с разной category_ref в одном
 * create → VALIDATION (В1)») и `executor/legacy-form.test.ts`.
 *
...
const COVERAGE: Record<string, readonly [number, number]> = {
  ...
  'orbis/project': [2, 3],
  'orbis/repo': [2, 6],
  'orbis/assignment': [4, 5],
  'orbis/agent-run': [5, 22],
  'orbis/routine': [2, 10],
};

/**
 * Размер корпуса и его разбивка — все три числа точные. Позитивных стало 38 (было 35), а
 * негативных 111 (было 114): три записи `MERGED_VALUE_CONFLICT` переведены в позитивные —
 * см. докблок `EXPECTED_DIFFS`.
 */
const CORPUS_SIZE = 149;
const POSITIVE_RECORDS = 38;
 ...
 *
 * Стало 110 в Задаче 23a, и обе половины прироста — следствие перевода корпуса, а не новой
 ...
 * пропуска нет), и ещё 20 приносят три записи В1, ставшие позитивными.
 */
const MUTATIONS_CHECKED = 110;
