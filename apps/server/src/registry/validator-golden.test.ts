...
// источникам: позитивные формы — из сьюта zod-схем аспектов (снят вместе со схемами),
// `query/fixtures.ts`, `src/test/perf.ts` и сида категорий; вердикты выведены чтением тех
// же zod-схем и §А8. Корпус, написанный ПОСЛЕ валидатора, подтверждал бы его сам собой.
// Старые вердикты ЗАМОРОЖЕНЫ литералом (`legacyVerdict`) — источник их снят, история цела.
//
// Каждое НЕ перечисленное расхождение — дефект перевода. Список `EXPECTED_DIFFS` закрыт:
// расширять его можно только доказанным фактом (адрес в §А8 против замороженного вердикта),
// а не наблюдением «тест покраснел».
 ...
 * VALIDATION (В1)») и сьютом границы входа исполнителя.
 *
 ...
 * на каждый из четырнадцати аспектов. Пин точный, а не «хотя бы по одной», ровно потому, что
 ...
 * Все три движения объяснены в докблоке `EXPECTED_DIFFS`. Срез 1а: +2 записи аспекта `orbis/page`
 * (позитив и негатив `minItems`) — размер 149, позитивных 37, негативных 112 (`POST_FREEZE_ASPECTS`).
 */
const CORPUS_SIZE = 149;
const POSITIVE_RECORDS = 37;
const NEGATIVE_RECORDS = 112;
...
/**
 * Аспекты, рождённые ПОСЛЕ заморозки `legacyVerdict` (РП-12). До заморозки такого аспекта не было —
 * старой формы у его записей не существовало, и свидетель совпадает с новым вердиктом по
 * определению: `legacyVerdict = newVerdict`, без `expectedDiff`. Список явный, чтобы запись
 * пост-заморозочного аспекта нельзя было выдать за «так решала старая форма» — и чтобы новая
 * запись СТАРОГО аспекта не могла под этим предлогом переписать свидетеля.
 */
const POST_FREEZE_ASPECTS: readonly string[] = ['orbis/page'];
...
describe('golden «сущность → вердикт» (приёмка §С8-1)', () => {
  ...
  test('у записей аспектов, рождённых после заморозки, свидетель совпадает с новым вердиктом (РП-12)', () => {
    const late = records.filter((r) => r.aspects.some((a) => POST_FREEZE_ASPECTS.includes(a)));
    // Не вакуумно: у каждого пост-заморозочного аспекта есть записи корпуса.
    for (const aspectId of POST_FREEZE_ASPECTS) {
      expect(late.some((r) => r.aspects.includes(aspectId))).toBe(true);
    }
    for (const r of late) {
      expect(`${r.name}: ${r.legacyVerdict}/${r.expectedDiff ?? '-'}`).toBe(
        `${r.name}: ${r.newVerdict}/-`,
      );
    }
  });

  test('состав корпуса запиннен ТОЧНО: 14 аспектов, разбивка позитивных и негативных, причины расхождений', () => {
