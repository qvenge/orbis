...
export interface ExecuteRequest {
  ...
  editedFrom?: string;
  /**
   * Автор-приложение действия (§Б6-4, концепция страниц §7): по этой паре в журнале видно,
   * ЧТО исполнялось, а не только «batch из трёх операций». Ключи в `ActionRecord` кладутся
   * ПО НАЛИЧИЮ — как `run_id` (контейнмент-проба jsonb, докблок `ActionRecord`). Есть только у
   * batch-пути: действие исполняется всегда с `batchId` (`actions/run.ts`).
   */
...
export interface ActionRecord {
  ...
  type:
    ...
    // Снятие названо `action_removed`, а не `action_deprecated`: владелец в списке «отмени последнее»
    // читает «снял действие», а не служебное слово колонки.
    | 'action_set'
    | 'action_removed'
    // rule_set / rule_removed — операции КАТАЛОГА ПРАВИЛ (§Б4-1): `entity_id: null` по тому же доводу,
    // что у реестровых — меняется устройство системы, а не запись графа. Снятие названо `rule_removed`
    // (как у действия): у системного правила оно означает «отключено», у своего — «снято», и владельцу
    // в списке «отмени последнее» точнее одно слово на обе ветки, чем служебное имя механизма.
    | 'rule_set'
    | 'rule_removed'
    // rule_delta_restored — внутренняя обратная операция тулов правил на встроенной строке (задача 16, N-1,
    // N3-1): записи одного правила в дельте носителя — к прежним, поверх текущей строки (прочие правила и
    // настройка носителя не трогаются). Отдельный тип — по доводу `aspect_row_restored`: «возврат» под
    // заголовком «Правило …» врал бы владельцу о том, что делает откат.
    | 'rule_delta_restored'
    ...
    | 'module_set'
    | 'batch'
    // action — действие §Б6-4: резолвленная пачка с автором-приложением (`action_id`,
    // `module`). `entity_id: null` по тому же доводу, что у batch: у группы шагов одной
    // сущности-адреса нет. Отличается от голой пачки ТИПОМ, а не догадкой по полям: читатели
    // журнала, отбирающие записи пробой по типу (скан перекатегоризации `ai/escalation.ts`),
    // обязаны назвать третий тип явно — сам в дизъюнкцию проб он не попадает.
    | 'action';
  ...
  run_id?: string;
  /**
   * Действие реестра и его модуль (§Б6-4) — у строк `type: 'action'`. По отсутствию ключа,
   * а не по null, — тем же доводом, что `run_id` выше; у действия ядра (`module: null`) ключа
   * `module` нет вовсе.
   */
  action_id?: string;
  module?: string;
