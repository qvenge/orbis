...
// но идентичность конверта оно читает у СИСТЕМНОЙ строки `duplicate_envelope` (до дельт владельца), а не
// держит своей копией.
 ...
 * больше негде.
 *
 * Читается СИСТЕМНАЯ строка — носитель `orbis/budget` в словарях до дельт владельца
 * (`RegistrySnapshot.system`; нет поля — дельт нет, и снимок сам есть система). Две причины (финал Б-2
 * E-1, рулинг Ф-Б2-31):
 * - отключение владельцем (`rule_remove` → дельта `rulesDisabled`, §Б4-4) вынимает правило из
 *   ЭФФЕКТИВНОГО списка (`effectiveRules`), и чтение оттуда роняло каждую правку свойств конверта с
 *   переносом голым `Error` (500);
 * - до финала Б-2 после отключения id освобождался для своего правила на ДРУГОМ носителе, и поиск «по id
 *   по всему снимку» подменял бы идентичность чужим набором (перенос переживал бы смену валюты); теперь id
 *   отключённого системного правила занят (`refuseSystemRuleId`), а носитель задан явно — порядок строк снимка и второе правило уникальности конверта (§Б4-1) не решают ничего.
 *
 ...
 * вправе отключить правило) не переключает второй. Выключенная или отключённая владельцем строка
 * по-прежнему называет четвёрку, и перенос прошлого периода снимается при её смене — иначе он молча
 * завышал бы лимит (03-budget §2.6).
 *
 * Строки нет в СИСТЕМНОМ снимке (или на её id лежит не `unique_among`) — ошибка СБОРКИ, а не умолчание:
 * системная строка сеется всегда, владелец её отключает, но не удаляет; отсутствие значит несделанный
 * пересев, и молчаливое «идентичности нет» здесь стоило бы денег.
 */
export function envelopeIdentityOf(reg: RegistrySnapshot): readonly string[] {
  const row = (reg.system ?? reg).aspects
    .get('orbis/budget')
    ?.rules.find((r) => r.id === RULE_ENVELOPE_UNIQUE.id);
  if (row?.template === 'unique_among') return row.params.properties;
  throw new Error(
    `строка каталога «${RULE_ENVELOPE_UNIQUE.id}» (unique_among на orbis/budget) не найдена в системном снимке реестра — пересейте реестры: без неё идентичность конверта не определена`,
  );
}
 ...
 *
 * Что такое «идентичность», решает реестр — `envelopeIdentityOf(reg)`: набор системной строки
 * уникальности конверта, включена она, выключена или отключена владельцем (рулинги Ф-Б2-21, Ф-Б2-31).
 *
