 ...
 * «конверт-родитель» состоит из трёх частей, и остальные две написаны ТРЕМЯ разными
 * способами на пяти местах:
 *   • признак «источник — конверт»: `'orbis/budget' = ANY(<алиас>.aspects)` (1, 2, 4);
 *     `p.aspects_legacy ? 'orbis/budget'` (5) — ПОСЛЕДНЕЕ место старой формы, его переводит
 *     Задача 10b; НЕ ПРОВЕРЯЕТСЯ ВОВСЕ у (3) — там источник ограничен списком id, который
 *     вызывающий уже отобрал по аспекту бюджета, и вторая проверка была бы тавтологией.
 *     На интервале дуальной записи (§А1-1) обе колонки заполняет один писатель, поэтому оба
 *     способа дают одно множество — на этом и держался перевод (4) в Задаче 10a;
 ...
 * Все расхождения — ДО-реформенные и ЗАПАРКОВАНЫ осознанно (см. отчёт Задачи 7b, §6):
