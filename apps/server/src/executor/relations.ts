 ...
 * владельца с `has_relation` по роли, которую ставит сам владелец (`created_by` не `system`), до этой
 * границы не доезжает: его отказывает ЗАПИСЬ правила (`RULE_RELATION_UNCHECKED`, ступень 10
 * `assertRule`, задача 16) — иначе ребро, меняющее вердикт конца, ложилось бы, а отказ приходил бы на
 * следующей правке самой записи.
 *
 ...
 * схемой с среза А, а читатель на записи ребра появился только в этом срезе.
 *
