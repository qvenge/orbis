 ...
 *   • признак «источник — конверт»: `'orbis/budget' = ANY(e.aspects)` (1, 2);
 *     `p.aspects_legacy ? 'orbis/budget'` (4, 5); НЕ ПРОВЕРЯЕТСЯ ВОВСЕ у (3) — там источник
 *     ограничен списком id, который вызывающий уже отобрал по аспекту бюджета, и вторая
 *     проверка была бы тавтологией. На интервале дуальной записи (§А1-1) обе колонки
 *     заполняет один писатель, поэтому первые два способа дают одно множество;
