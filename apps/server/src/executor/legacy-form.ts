 ...
 * Отсюда единственное умолчание, и оно норматив, а не упрощение: свойство, не объявленное
 * ни одним из аспектов сущности, в карту не попадает — в старой форме у него не было места
 * по построению.
 *
 * ИМЯ ПОЛЯ у ВСЕХ объявленных свойств есть (см. `legacyFieldOfProperty`): встроенным его даёт
 * переходная таблица §А8, своему свойству владельца — локальная часть его key, то есть ровно
 * то имя, под которым его завёл конструктор аспекта. Прежняя редакция этой шапки утверждала
 * обратное («свойства без пары §А8 в карту НЕ попадает») и противоречила собственному коду
 * ниже: то умолчание жило в Задаче 4a, когда `props` никто не писал, а с появлением писателя
 * было снято — проекция, теряющая поле владельца, молча выключила бы его фильтры в запросах
 * и его строку в карточке тула.
 *
