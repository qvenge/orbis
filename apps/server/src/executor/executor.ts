...
async function prepareEntityUpdate(
  ...
  if (input.bodyDoc !== undefined) {
    ...
    // Третья проверка, которой здесь не было (итоговое ревью, находка 3): что проекция —
    // НЕПОДВИЖНАЯ ТОЧКА канона. Версия и структура сверялись, а то, ради чего вся конструкция
    // затеяна, — нет, и ровно через этот шов проходила порча вложенных оград и подписей со
    // скобкой. Логика страховки живёт в @orbis/shared/doc рядом с самим сериализатором (там же
    // её и проверяют без базы): отказа не будет — автосохранение терминальный VALIDATION
    // остановил бы, — текст уезжает в БД байт в байт, а непрошедший документ подменяется
    // rawBlock'ом, который печатается дословно.
    //
