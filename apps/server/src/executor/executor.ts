  ...
  try {
    ...
    return await withIdentity(db, req.identity, async (tx) => {
      ...
      if (ctx.internalUndo) {
        ...
        // ребра бывает иерархической — считать предков до хука значило бы считать их по
        // графу, которого к коммиту уже нет. (С 0017 хук снимает РОВНО `envelope-binding`,
        // роль неиерархическая, и порядок стал безвредным — но он остаётся правильным, и
 ...
 * Контур адресуется двумя способами сразу: значениями `props`/`unset` по id ИЛИ key свойства
 * и списком аспектов (`[…]` у create, `{attach, detach}` у update). Прежний предикат («ключ
 * 'orbis/financial' в СТАРОЙ карте аспектов») видел только третью, ныне снесённую форму — и
 * на переведённом пути замок молча перестал бы браться. Тесты этого не показали бы: дедлок
 * ловится не отказом, а таймаутом под конкуренцией.
 *
...
function namesAspectOrProperty(
  ...
  // Две формы `aspects`, и обе обязаны быть видны: список навешиваемых (create) и
  // `{attach, detach}` (update). Третьей — старой карты аспектов — больше нет: контракт её
  // отвергает раньше (снос 23b), поэтому здесь она даёт пустой список, а не ветку.
 ...
 *
 * ИМЕНОВАННЫЙ ОСТАТОК КОДОМ (Р-К-17, правило 5 §С1-4): монотонность — механика записи (токен
 * конкурентности §5.2), а не утверждение о значениях записи. Правило каталога видит состояние записи и
 * не видит часов соседнего тика; шаблона «строго больше прежнего» каталог не держит и ради одной
 * core-колонки не заводит (правило 6 §10).
 */
...
async function prepareEntityCreate(
  ...
  // Стадия 1: exec-надмножество внутренней формы §А1-1 (`props`/`unset` + список аспектов);
  // старая карта аспектов из него снята вместе с формой данных (23b).
  ...
  // Каталог правил (§Б4-3): условные ограничения записи — строками реестра (§А7-2, задачи 4 и 14).
  // Инвариант «не-шаблон `orbis/financial` несёт `occurred_on`» и его пара про `recurring`, условие
  // гранта назначения, субъект прогона, форма правила памяти и «чего ждём» живут ДАННЫМИ
...
async function prepareEntityUpdate(
  ...
  if (hasPropsInput(input)) {
    ...
    // (`{aspects: {'orbis/task': {…}}}`). С «Пересева мира» такая нагрузка не разбирается
    // ВОВСЕ: граница входа принимает только `props`/`unset`/`aspects.{attach,detach}`
    // (`propsPatchFromInput`), и старая карта получает `VALIDATION` — то есть откат такой
    // записи не «неточен», а честно отказан. Точный откат гарантирован для записей,
    // сделанных начиная с перевода журнала на свойства.
    //
...
async function prepareRelationCreate(
  ...
  // Ограничения роли по реестру (§А4-2): контракты концов, acyclic, target_max_incoming, гейт
  // created_by. Аспекты концов спрашиваются ТОЛЬКО через контракт, объявленный ролью (§А4-2), —
  // догадки «смысл ребра по аспектам концов» здесь по-прежнему нет.
  // Идёт ПЕРВОЙ: она же проверяет, что роль вообще существует (`VALIDATION`, пин —
  ...
  await assertRoleConstraints(ctx.tx, ctx.registry, key, batch?.graph(), {
    ...
    op: 'create',
    // Контракты концов судятся по строкам, уже взятым под FOR UPDATE стадией 3 (с учётом пачки).
    ends: { source: stateOf(source), target: stateOf(target) },
  });
...
/**
 * Имена, при которых транзакция берёт замок реестра. Шестнадцать публичных тулов плюс четыре
 * ВНУТРЕННИЕ обратные операции — те же четыре, что перечислены ниже: их зовёт только undo.ts
 * через `execute` во внутреннем режиме, в `CORE_TOOLS` их нет, и `dispatchTool` их не
 * резолвит (реестр тулов не знает таких имён), — и переключатель модуля `module_set` (его зовёт
 * ручка владельца; довод — комментарий у имени в списке).
 *
...
export const REGISTRY_OPS: ReadonlySet<string> = new Set([
  ...
  'property_merge_undo',
  'aspect_row_restore',
  // Обратная операция тулов правил на встроенной строке (задача 16, N-1) — та же природа, что у трёх выше.
  'rule_delta_restore',
...
const propertyMergeUndoInput = z
  .object({
    ...
    // И снова так же — прежние `implements` своих аспектов, чьи привязки слияние перевело на цель
    // (пятый род держателей, срез Б-2). Элементы — как лежали в jsonb: откат присваивает их обратно
    // целиком и не толкует, а форма привязок старше любой сегодняшней схемы.
    binds: z
...
/** Прежняя СЫРАЯ дельта носителя-встроенной строки (N-1); у своей строки дельты нет (Ф-Б2-30). */
async function ruleDeltaBefore(
  tx: Tx,
  graphId: GraphId,
  target: { carrier: { kind: 'aspect' | 'property' | 'role'; id: string }; own: boolean },
): Promise<Record<string, unknown> | null> {
  const { kind, id } = target.carrier;
  if (target.own || kind === 'role') return null;
  return readRuleDelta(tx, graphId, { kind, id });
}

/**
 * ОБРАТНАЯ ОПЕРАЦИЯ ТУЛОВ ПРАВИЛ — одна мерка на `rule_set` и `rule_remove`.
 *
 * СВОЯ СТРОКА: правило правится ЗАМЕНОЙ (§С3), и откат возвращает ту декларацию, что стояла (у своей строки
...
function ruleInverse(
  address: unknown,
  target: { carrier: { kind: 'aspect' | 'property' | 'role'; id: string }; own: boolean },
  prevDelta: Record<string, unknown> | null,
  ruleId: string,
  before: unknown,
  ...
  // `rule_set` правила, которого не было: обратное — снятие (у `rule_remove` своей строки без правила
  // обратного нет вовсе — вызывающий его не пишет).
  return { op: 'rule_remove', payload: { target: address, rule: ruleId } };
}
...
async function prepareRuleDeltaRestore(_ctx: ExecCtx, rawInput: unknown): Promise<PreparedOp> {
  const input = parseEnvelope(ruleDeltaRestoreInput, rawInput, 'rule_delta_restore');
  const journal = registryPlan(
    'rule_delta_restored',
    'rule_delta_restore',
...
async function prepareRuleSet(_ctx: ExecCtx, rawInput: unknown): Promise<PreparedOp> {
  ...
  return {
    ...
    async apply(applyCtx: ExecCtx): Promise<OpOutcome> {
      ...
      journal.inverse.push(ruleInverse(input.target, target, prevDelta, input.rule.id, before));
...
async function prepareRuleRemove(_ctx: ExecCtx, rawInput: unknown): Promise<PreparedOp> {
  ...
  return {
    ...
    async apply(applyCtx: ExecCtx): Promise<OpOutcome> {
      ...
      // Своя строка: правила не было — снятие было не-операцией, и обратного нет (Ф-Б1-56).
      if (!target.own || before !== null) {
        journal.inverse.push(ruleInverse(input.target, target, prevDelta, input.rule, before));
      }
