...
export type PropsPatch = {
   ...
   * носителя не подменяет вовсе — переключатель семантик у границы входа снят вместе с
 ...
 *    противоречив по построению, и здесь важно не «что правильнее», а что исход ОДИН и тот
 *    же при любом порядке ключей во входе. (Второго противоречия — «два значения одного
 *    свойства» — во входе больше не бывает вовсе: плоский `props` их не выражает, и класс
 *    отказа `merged_property_conflict` исчез вместе со старой картой.)
 *
 ...
 * состояние «после» — то, что записано, и между ними стоит то, что пишет мимо патча: ПРАВИЛА
 * каталога (`on_enter_class` дописывает `orbis/completed_at`) и `dropStaleCarryover` (снимает
 * перенос), подстановка умолчания валюты кладёт `orbis/currency`. Inverse, собранный
 ...
 * ключом — ровно как это делает `effectiveRegistry` при коллизии id.
 *
