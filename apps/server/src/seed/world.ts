...
//
// ЗОВЁТСЯ НА КАЖДОМ ЗАХОДЕ ВЛАДЕЛЬЦА (рулинг Р-24-6), а не только на первом: guard по строке
// `user_settings` снят, потому что `db/reset-world.ts` эту строку сохраняет — и после
// пересева владелец с маркером оставался бы без мира. Идемпотентность держит проба по PK
// выше, повтор даёт `{created: 0, skipped: 18}`.
//
// ЦЕНА ТОГО ЖЕ РЕШЕНИЯ: сущность мира, снесённую владельцем ФИЗИЧЕСКИ, следующий заход
// заведёт заново — сид по-прежнему не отличает «никогда не было» от «удалил осознанно».
// В продукте физического удаления нет (архивация архивную сущность из `missingIds` не
// убирает, id остаётся занят), поэтому вход достижим только прямым SQL или пересевом.
// Адресные досевы (горизонты E4, «Рутины» V1.9) остаются в `onboarding.ts` со своими
// наборами — см. их докблоки.
//
