 ...
 * сев мира остался бы незамеченным навсегда. Вопрос «свежий ли владелец» задаётся ДО
 * транзакции онбординга, потому что она сама на него и отвечает созданием строки.
 *
 * Сев мира ТОЛЬКО свежему — не оптимизация: сид не отличает «никогда не было» от «владелец
 * удалил», и общий досев вернул бы выброшенное (тот же довод, что у адресных бэкфиллов).
 */
