...
// встроенная строка реестра — `system-object`, и фону она запрещена ПО ОБЪЕКТУ. Адрес здесь
// не единственный вход, а ПОСЛЕДНЕЕ слово: у `property_create` ряд решает `status`
// (`proposed` — «своя строка», сразу активная — перенастройка поведения), у
...
const contractSetsDeltaSetJsonSchema = {
  ...
  properties: {
    ...
    setsDelta: {
      type: 'object',
      description:
        'СВОИ именованные наборы классов: {"my_open":["active"]}. Имя набора — латиницей, ' +
        'строчными, с «_» (та же форма, что у слота). Встроенные наборы ' +
...
export const REGISTRY_TOOLS: OrbisToolDef[] = [
  ...
  },
  {
    ...
    description:
      ...
      'чувствительности — отказ. Встроенное действие этим тулом не правится: нужны другие шаги — ' +
      'заведи свою копию с ключом user/….',
