 ...
 * (`dispatch.test.ts`, «места сборки карточек — перечень закрыт»): появится пятое — тест
 * разойдётся, и решать про него придётся осознанно. Прежние редакции этого докблока
 * насчитали сперва ДВЕ карточки (неправда на explicit), потом ТРИ (неправда на пачке).
 *
