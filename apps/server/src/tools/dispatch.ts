 ...
 *
 * СЧЁТ «ЧЕТЫРЕ» ЗДЕСЬ АБСОЛЮТНЫЙ, И ОХРАНА ПОД НИМ ТОЖЕ АБСОЛЮТНА — по всему серверу, а не
 * по списку файлов: раунд 3 нашёл, что пин по двум названным файлам пропускал пятую сборку в
 * третьем, тогда как эта строка утверждала «мест четыре» без оговорок. Граница охраны —
 * сервер: web карточки РИСУЕТ, но не собирает, производитель у них один (`Card`,
 * `tools/registry.ts`). Прежние редакции этого докблока насчитали сперва ДВЕ карточки
 * (неправда на explicit), потом ТРИ (неправда на пачке) — счёт правился после того, как
 * ошибка проезжала в ветку, и охрана переписана так, чтобы третьего раза не было.
 *
