  ...
  if (level === 'explicit-confirmation') {
    ...
    const pending = await withIdentity(ctx.db, ctx.actorUserId, async (tx) => {
      ...
      if (registryParts.length > 0) {
        // МАСШТАБ ПАЧКИ НЕ ТЕРЯЕТСЯ. Собранная сводка вытесняет фолбэк целиком, а у батча
        // фолбэк говорил «N операций» — владелец, подтверждающий слияние внутри пачки из
        // одиннадцати, обязан видеть и то, и другое: уровень поднял ряд 4a, но подписывает
        // он всю группу. У одиночного вызова приписки нет — она была бы шумом.
        const n = batchPayload?.operations.length;
        summaryParts.push(
          n === undefined
            ? registryParts.join('; ')
            : `${registryParts.join('; ')} — в пачке из ${n} ${operationsNoun(n)}`,
        );
      }
...
/**
 * КАК НАЗВАТЬ ВЛАДЕЛЬЦУ МУТАЦИЮ РЕЕСТРА — одной фразой и ОДНОЙ функцией на ВСЕ ТРИ карточки,
 * которыми эта мутация до него доходит:
 *  1. `confirmation_card` mode `preview` — §С2-1 ряд 4b, своя строка, исполнено и показано
 *     (`runMutation`, ветка `level === 'preview'`);
 *  2. `confirmation_card` mode `explicit` — ряд 4a, слияние/статус/дельта, ждёт нажатия
 *     (`runMutation`, ветка `level === 'explicit-confirmation'`, через `summaryParts`);
 *  3. `deferred_action_card` — та же операция от рутины, единица пачки D42
 *     (`snapshotRegistryUnit` ниже).
 ...
 * с предложением рутины. ПЕРЕЧЕНЬ ПРОВЕРЯЕМ ГРЕПОМ по вызовам этой функции:
 * `grep -c 'registryOperationSummary(' apps/server/src/tools/dispatch.ts` → 5: три вызова,
 * объявление и эта строка. Четвёртой карточки, несущей мутацию реестра, в сервере нет. Прежняя редакция
 * этого докблока насчитала ДВЕ и была неправдой ровно на explicit-карточке (гейт-ревью
 * Задачи 16): та собиралась без единой фразы и падала в фолбэк по имени тула.
 *
