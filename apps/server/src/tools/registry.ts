...
export type Card =
  ...
  // решается он в пачке, рядом с другими единицами того же прогона. Поля обязаны дословно
  // совпадать с web-типом (chat/cards/types.ts) — union'ы сервера и web намеренно не общие.
  | {
...
const entityGetJsonSchema = {
  ...
  properties: {
    ...
    include: {
      ...
      description:
        'по умолчанию body+relations; backlinks — кто ссылается: явные связи роли mention (via "relation") и упоминания через body_refs (via "mention"), без архивных, до 100; thread — сообщения треда сущности',
    },
  },
  required: ['id'],
  additionalProperties: false,
