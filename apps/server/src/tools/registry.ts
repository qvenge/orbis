...
export type Card =
  ...
  | {
      ...
      /**
       * «Было → станет» карточки ДЕЙСТВИЯ (эррата Ф-Б2-18) — та же форма строк, что у
       * `deferred_action_card` ниже: у пакета одно свойство правится на многих записях, и `diff`,
       * ключуемый полем, схлопнул бы их в одну строку. Есть только у карточки действия; web-тип
       * поля не объявляет и лишний ключ просто не читает (union'ы сервера и web не общие).
       */
  ...
  properties: {
    ...
    ast: {
      ...
      description:
        ...
        'просрочено) И не заблокировано». Имена в дереве — key или id свойств и аспектов ' +
        '(те же, что в тексте запроса), но не подписи.',
    },
  },
  $defs: astDefs,
...
const rolloverJsonSchema = {
  ...
  properties: {
    ...
    rows: {
      ...
      minItems: 1,
      description:
        'по строке на категорию: limit — лимит нового конверта (обычно лимит прошлого, свойство orbis/limit), carryover — остаток прошлого конверта (remaining из budget_status; может быть отрицательным). Суммы бери как есть, не округляй',
      items: {
        type: 'object',
...
const AGENT_VERB_TOOLS: OrbisToolDef[] = [
  {
    ...
    description:
      ...
      'последнего прогона. Брать в работу можно только тикеты с claimable=true (тикет в классе new ' +
      'или queued контракта делегирования; какие статусы туда отнесены, решает реестр владельца) — ' +
      'остальные ждут владельца. По дороге сервер помечает брошенные прогоны ' +
...
const CORE_TOOLS: OrbisToolDef[] = [
  ...
  },
  {
    ...
    // Предпросмотра переноса у модели нет (рулинг 10-1: задача 11 переводит только tRPC-ручку
    // экрана), поэтому описание опирается на то, что у неё есть, — `budget_status` прошлого месяца.
    description:
      'Перенести остатки бюджета в новый месяц: завести конверты целевого месяца с лимитами и ' +
      'перенесённым остатком. Лимиты и остатки бери из budget_status за прошлый месяц (orbis/limit ' +
      'конверта и remaining); остаток родительской категории уже включает остатки подкатегорий — не ' +
      'переноси его дважды. Покажи владельцу суммы и только потом зови — действие пишет деньги и ' +
      'создаёт сразу все конверты одной группой. Повторный вызов с тем же batchId безопасен.',
