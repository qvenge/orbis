...
/** Образцы запросов из текста описания: фрагмент в ёлочках, внутри которого есть `=`. */
...
describe('buildToolRegistry: состав (§9.2 + §7.6)', () => {
  ...
  // с грамматикой молча.
  //
  // Запрос от прозы отличает `=`, а не место в строке: ёлочки в этом файле — штатная
  ...
  // же законной правке текста. Признак `=` внутренний для грамматики — запроса без него не
  // бывает, у прозы он не встречается — и потому не зависит от того, где в описании проза
  // стоит; привязка к маркеру «Примеры:» такой устойчивости не даёт.
  test('entity_query: каждый пример из description разбирается парсером §6', async () => {
    ...
    // и без этой строки правило «пример — это то, где есть `=`» осталось бы без теста.
    expect(grammarExamples('Смотри «что по бюджету?»: «tags=work» — вот это запрос.')).toEqual([
      'tags=work',
    ]);
