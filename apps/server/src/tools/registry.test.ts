...
describe('§С8-23: инвариант против fail-open — писатели реестра, замок и ось worker', () => {
  ...
  test('писатели реестра разобраны, и КАЖДЫЙ берёт замок реестра', () => {
    ...
    // У тулов правил (задача 16 Б-2) на СВОЕЙ строке — так же: обратное к заведению — снятие, к снятию и
    // замене — `rule_set` с прежней декларацией; на ВСТРОЕННОЙ строке обратное — внутренняя
    // `rule_delta_restore` (записи одного правила в дельте — к прежним поверх текущей строки, N-1/N2-1/N3-1).
    ...
    expect(internal.length).toBeGreaterThan(0); // не вырожденно: сегодня их пять
