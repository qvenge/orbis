 ...
 *
 * Отдельный тип, а не выведенный из zod-схемы аспекта: запрос внутри — ДЕРЕВО (§А5-2/Р12),
 * а старая схема объявляла его строкой. Схемы снял «Пересев мира», и второго описания этой
 * формы в дереве больше нет — единственное здесь.
 */
 ...
 * не имеют. Паттерн — локальная копия формы «положительный decimal»; РЕЕСТР выражает то же
 * границей типа у `orbis/target_value` (`exclusiveMin: '0'`), и расхождение двух форм
 * поймал бы валидатор записи раньше этой схемы.
 */
