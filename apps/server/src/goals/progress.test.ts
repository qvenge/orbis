...
describe('entity.get: прогресс приезжает с целью и только с ней', () => {
  ...
  test('обычная сущность не платит за расчёт цели ни одним запросом (Р14)', async () => {
    ...
    try {
      ...
      // ТРИ СЛЕДА РАСЧЁТА, которых у обычной сущности нет: таймзона владельца (её берёт
      // контекст компилятора), savepoint (fail-soft §12 п.6) и сам агрегат. Их отсутствие
      // и есть доказательство, что ветка по аспекту не пускает обычную сущность в расчёт.
      //
      // Прежним следом было чтение `aspect_definitions`. С процессным кешем реестра
      // (§А10-1, Задача 14) он пропал у ОБЕИХ сторон: снимок владельца попадает в кеш ещё
      // при создании этих двух сущностей, и «реестр не читался» перестало значить «расчёт
      // не запускался». Следы ниже кешем не снимаются — каждый из них делает сам расчёт.
      const marks = (queries: string[]) => ({
        timezone: queries.some((q) => q.includes('"timezone"')),
        savepoint: queries.some((q) => q.startsWith('savepoint')),
        aggregate: queries.some((q) => q.includes('count(*) FROM entities')),
      });
      expect(marks(plainQueries)).toEqual({ timezone: false, savepoint: false, aggregate: false });
      expect(marks(goalQueries)).toEqual({ timezone: true, savepoint: true, aggregate: true });
      // Измерено: 7 запросов на обычную сущность (begin + 2 identity + сущность + связи +
      // ВЕРСИЯ РЕЕСТРА + commit) и 11 на цель (+ вторая проверка версии — её делает кеш
      // реестра, + таймзона, + savepoint, + агрегат).
      //
      ...
      // У цели было 14, стало 11: процессный кеш эффективных определений (§А10-1, Задача
      // 14) снял четыре запроса снимка реестра и добавил один — проверку версии, по которой
      // он и решает, годится ли снимок. Это и есть обещанное «упадёт заметно»: цена реестра
      // на горячем кеше — ОДИН точечный SELECT вместо четырёх проходов по таблицам.
      // У обычной сущности число не изменилось: снимка она и раньше не грузила.
      expect(plainQueries.length).toBe(7);
      expect(goalQueries.length).toBe(11);
    } finally {
      await counted.end();
    }
  });
