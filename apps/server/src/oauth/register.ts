...
function isAllowedRedirect(uri: string): boolean {
  ...
  // как `${uri}?code=…`, а `…/cb#frag?code=…` уводит код во фрагмент — до локального
  // сервера клиента он не доедет вовсе, и вход молча не состоится. Проверяем по СТРОКЕ,
  // а не по `parsed.hash`: у `…/cb#` hash пуст, а '#' остаётся и ломает ровно так же.
