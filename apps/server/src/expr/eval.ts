 ...
 *     порядок и есть хронологический (так сравнивал и снесённый срезом Б-2 Overview кодом; фазы
 *     периода движок Budget теперь считает этой функцией — `phaseOf` в `subscriptions/budget.ts`).
 ...
 *     деньгами тихо (ровно то, что делал `carryover ?? '0'` оракула, снесённого срезом Б-2, — и там
 *     это было допустимо только потому, что после среза А валидатор не пускает нестроку в
 *     decimal-свойство, Р-К-13);
