 ...
 *
 * Аргумент — `{prop}` списочного свойства; прочее отказывает, и каждый отказ назван:
 *  — `{ctx:'$sensitivity'}`/`{ctx:'$touched'}` доходят сюда и отвергаются `valueSql` ИМЕНЕМ контекста
 *    (`EXPR_BACKEND_UNSUPPORTED`): у классификатора SQL-бэкенда нет вовсе (§Б3-2а Е-4/Е-5); туда же
 *    `agg_via`/`deref` — формы, которых у бэкенда наборов нет;
 *  — ветка `{slot}` из декларации НЕДОСТИЖИМА: чекер типизирует слот скаляром (у слота контракта нет
 *    кардинальности — `contractSlotTypeSchema`) и отвергает `empty` над ним. Она — защита входа мимо
 *    чекера, и тот же вход без области контракта получает `EXPR_SHAPE` от `slotScopeOf`;
 *  — core-проекция: колонки ядра все скалярны (`CORE_COLUMN`), так что ветка `storage === 'core'`
 *    сегодня отсекает то же, что «не список». Она стоит ради первой списочной колонки ядра (теги):
 *    читать `props->…` у core-свойства значило бы спросить ключ, которого в `props` нет, и ответить
 *    «пусто» на каждой строке — молчаливая неправда вместо отказа. Пин — синтетическое свойство в тесте.
 */
 ...
 * архивен», дословно как у прежнего оракула агрегатов (снесён срезом Б-2).
 *
