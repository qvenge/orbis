 ...
 * константы здесь нет.
 *
 * САМОССЫЛКА ЧЕРЕЗ ЭТУ ДВЕРЬ ДО `checkExpr` НЕ ДОХОДИТ: у цикла глубина не ограничена, и
 * гейт выше объявляет его слишком глубоким (`VALIDATION`, `reason: 'EXPR_TOO_DEEP'`) раньше,
 * чем до самоссылки доберётся `checkExpr` (там своя проверка стоит ПЕРЕД гейтом глубины и
 * даёт `EXPR_RECURSION`). Расхождение диагнозов на живом пути недостижимо: сюда значение
 * приезжает из JSON — из аргумента тула или из jsonb реестра, — а в JSON цикл невыразим;
 * `EXPR_RECURSION` остаётся за структурами, собранными в процессе, и его называет `checkExpr`,
 * когда его зовут напрямую.
 */
import {
  checkExpr,
  EXPR_TREE_DEPTH_CAP,
