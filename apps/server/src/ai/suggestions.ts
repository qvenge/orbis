...
// (блок «Продолжения разговора», действующая версия — prompts/v3.ts) — менять формат =
// менять И промпт (гард — v3.test.ts: пример маркера из текста промпта прогоняется через
// этот парсер).
//
