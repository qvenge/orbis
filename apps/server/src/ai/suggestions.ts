...
// (блок «Продолжения разговора», действующая версия — prompts/v5.ts) — менять формат =
// менять И промпт (гард — v5.test.ts: пример маркера из текста промпта прогоняется через
// этот парсер).
//
