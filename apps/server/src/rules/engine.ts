 ...
 * 12 туда же ушла уникальность конверта — строка `duplicate_envelope` шаблона `unique_among`, с задачи
 * 14 — остальные инварианты §А7-2: условие гранта назначения и субъект прогона (пары
 * `requires_when`/`forbidden_when`), форма правила памяти (две `requires_when`), «чего ждём»
 * (`on_enter_class` + `forbidden_when` по классу контракта `orbis/delegable`) и умолчание валюты конверта
 * (`default` с параметром движка). Кодом остались именованные остатки — живость гранта
 * (`assertGrantAlive`) и снятие устаревшего переноса (`dropStaleCarryover`).
 *
 ...
 *
 * ИМЕНОВАННЫЙ ОСТАТОК — зона владельца НЕ ленива (перенос m-4 задачи 4, решение задачи 14). Рёбра и
 * параметры читаются только по ссылкам правил, а день и зона — при ЛЮБОМ применимом правиле. Почему
 * кодом, а не отбором: зону спрашивает не только `$today` и календарные формы, но и сравнение, когда
 * один операнд — дата, а другой — момент (`compare` в `expr/eval.ts` приводит момент к дню владельца);
 * решить статически, нужна ли она, можно только по ТИПАМ операндов, и ошибка такого отбора молча
 * сменила бы вердикт (без зоны `compare` сравнивает день с моментом сырым текстом, а не днём владельца).
 * Цена названа: один мемоизированный SELECT `user_settings` на транзакцию у каждой записи с
 * применимыми правилами — с задачи 14 это и `orbis/agent-run` (каждый `orbis_run_step`),
 * `orbis/assignment`, `orbis/memory`.
 */
...
async function assertUniqueAmong(
  ...
  // Замок — реентерабельный повтор: пред-стадийный проход исполнителя (`lockUniqueAmongRules`) уже взял
  // его по форме входа либо по аспектам правимой записи. Здесь он страхует то, что проход увидеть не
  // может, — аспект, навешенный конкурентом между чтением аспектов и строковой блокировкой (остаток в
  // докблоке прохода), и вызов движка мимо `execute`: без замка две конкурентные записи одного набора
  // не видели бы незакоммиченных строк друг друга (write-skew).
