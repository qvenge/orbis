...
// отвергает по построению (см. докблок `parse-ast.ts`). Строки датасета объявлены
// свойствами (`props`/`aspects[]`) и уезжают в БД через `entityColumnsFrom`
// (`test/helpers.ts`) — ради громкого отказа на адрес свойства, которого нет в реестре.
//
...
/**
 * Свойства фикстуры → колонки значений строки через общий помощник `entityColumnsFrom`
 * (`test/helpers.ts`), а не литералом на месте.
 *
 ...
 * опечатка в id свойства иначе уехала бы в базу молча — строка есть, значение под
 * неизвестным ключом лежит, а запрос по этому полю не находит ничего, не сказав почему.
 */
