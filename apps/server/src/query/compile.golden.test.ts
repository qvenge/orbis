...
test('самый глубокий эталон набора — 7 уровней, то есть много ниже капа глубины', () => {
  // Число живёт в докблоке `QUERY_TREE_DEPTH_CAP` («самый глубокий эталон — 7») и в тексте
  // отказа гейта; посчитанное здесь, оно перестанет быть правдой громко, а не молча.
  //
  // До Б-1 максимум был 8 и держался на двух эталонах сахара: узел `sourceNotIn` нёс массив
  // значений, то есть лишний уровень JSON. Набор контракта — пара скаляров, уровень ушёл, и
  // семёрка теперь у ШЕСТИ эталонов (#0, #2, #13, #19, #20 и class-форма тела сида), а не
  // у двух — то есть число перестало зависеть от одной конструкции.
