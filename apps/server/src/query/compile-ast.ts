...
// §А4-3). ЕДИНСТВЕННЫЙ компилятор запросов сервера: старый `compile.ts` вместе со всеми
// потребителями снят Задачей 9b, старая ГРАММАТИКА (текст → плоский AST) удалена Задачей 21b
// вместе с переходным мостом — сюда приходит канон и только он.
//
 ...
 *
 * `graphId` — id ТЕКУЩЕГО ГРАФА (D44, спека §2), а не актора: из него узел `$owner` языка E
 * подставляется в SQL значением (`expr/compile.ts`). Сам компилятор его для изоляции НЕ
 * подставляет (её даёт RLS) — он в контексте потому, что этим же контекстом ходят соседи по
 * конвейеру: снимок реестра `reg` снят в ОПРЕДЕЛЁННОМ графе, и компилят, исполненный в чужом
 * графе, молча собрался бы из чужих свойств. Поле называет тот граф, в котором компилят
 * обязан исполняться.
 *
 ...
 *
 * ЧТО ЭТА ПРОВЕРКА НЕ ДЕЛАЕТ: она сверяет ВИД свойства, а не форму значения. Форму и
 * существование дня проверяет соседний гейт — `assertScalarType` выше по файлу
 * (`literalFormViolation` по схеме записи плюс `hasValidCalendar`), поэтому
 * `orbis/due_date='банан'` со входа `ast:` отвергается структурно, а не доезжает до
 * Postgres. Две проверки, а не одна, потому что вопросы разные: у токена литерала нет
 * вовсе, а у литерала не бывает «вида свойства».
 */
