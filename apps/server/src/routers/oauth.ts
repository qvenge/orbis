 ...
 *
 * 64 код-поинта выбраны от законной подписи: «Claude Code» — 11 символов, самые длинные
 * реальные имена клиентов — десятки. Запас есть, а строка остаётся строкой.
 */
const CLIENT_NAME_MAX_POINTS = 64;
 ...
 *
 * Режем по КОД-ПОИНТАМ, а не по код-юнитам UTF-16: `slice` на имени из эмодзи разрубает
 * суррогатную пару. Проба на живой базе (ревью Task 7): оставшаяся половина пары вместе
 * со следующим за ней «…» уезжает в Postgres ОДНИМ U+FFFD — хвост `1f642 d83d 2026`
 * возвращается как `1f642 fffd`. То есть метка гранта перестаёт совпадать с подписью,
 * которую владелец видел на кнопке, и сам маркер обрезки исчезает.
 *
 * Кластеры графем (ZWJ-последовательности, флаги, комбинирующие знаки) код-поинты не
 * склеивают: разрез внутри такого кластера даёт другой видимый символ, но строка остаётся
 * корректной и в БД не портится. Дальше не идём — за Intl.Segmenter здесь платить нечем.
 */
function displayName(raw: string): string {
  const collapsed = raw.replace(/\s+/gu, ' ').trim();
  const points = [...collapsed];
  return points.length <= CLIENT_NAME_MAX_POINTS
    ? collapsed
    : `${points.slice(0, CLIENT_NAME_MAX_POINTS - 1).join('')}…`;
}
...
export const oauthRouter = router({
  ...
  // Наружу — wire-форма (таймстампы ISO-строками), а не доменный GrantSummary: по HTTP
  // Date всё равно уезжает строкой, и соглашение wire.ts требует говорить это формой, а не
  // полагаться на вывод типов транспортом. Типом клиент не обманулся бы и без этого —
