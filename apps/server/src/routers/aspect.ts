...
//
// ЧИТАЕТ ТАБЛИЦУ НАПРЯМУЮ, МИМО СНИМКА, И ПОТОМУ НЕ ВИДИТ ДЕЛЬТ (§А3-2) — названное
// расхождение, не забывчивость. Через снимок эта ручка пройти не может по построению:
// колонки `schema` (старая форма значений, Р-24) в декларации `AspectDefinition` нет вовсе,
// а экрану настроек она нужна до миграции 0017. Наблюдаемое следствие: аспект,
// переименованный дельтой владельца, покажется здесь ПОД СИСТЕМНОЙ подписью, тогда как в
// форме записи и в `attach_*` он уже свой.
//
// Расхождение существует ровно до появления писателя дельт (тулы реестра — Задача 15);
// сегодня дельты кладёт только слияние на пересеве, и подписи оно не выдумывает. Снимается
// оно НЕ здесь: 0017 сносит колонку `schema` вместе со старым путём валидации (Задача 23),
// и в тот же день экран настроек переезжает на `registry.effective`, а ручка исчезает
// целиком. Признак, по которому следующий читатель поймёт, что момент настал: в схеме
// больше нет `aspect_definitions.schema`.
