...
describe('agentRun.answerCheckpoint (С3, приёмка 8)', () => {
  ...
  test('тикет planned, waiting_for снят, на прогоне reply; один action (undo одним движением); следующий claim видит reply в history (приёмка 8)', async () => {
    ...
    expect(out.ticket.props['orbis/task_status']).toBe('planned');
    expect(out.ticket.props['orbis/waiting_for']).toBeUndefined();
    expect(out.run.id).toBe(runId);
    const reply = out.run.props['orbis/run_reply'] as { text: string; at: string };
    ...
    expect(out.run.props['orbis/run_checkpoint']).toMatchObject({ question });
    // Вопрос закрыт: исход `answered` — прогон уходит из блока «Ждут ответа» (V1, D38)
    expect(out.run.props['orbis/run_outcome']).toBe('answered');
  ...
  test('ответ на прогон, законченный без вопроса (finished/abandoned), исход не переписывает — только reply', async () => {
    ...
    expect(out.run.props['orbis/run_outcome']).toBe('finished');
    expect((out.run.props['orbis/run_reply'] as AnyRecord).text).toBe('Принял, спасибо');
    expect(out.ticket.props['orbis/task_status']).toBe('planned');
    expect(out.ticket.props['orbis/waiting_for']).toBeUndefined();
  });
