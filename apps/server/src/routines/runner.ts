...
/**
 * ОБРЕЗАН ЛИ ОТЧЁТ ПРОГОНА резаком выше — ПРАВИЛО, живущее рядом с самим резаком.
 *
 * Читатель один — проба П4 (`scripts/probe-p4.ts`), и нужда у неё конкретная: пометки обрыва
 * (`STEP_LIMIT_NOTE`, `MAX_TOKENS_NOTE`) `cap` дописывает В КОНЕЦ, а режет ровно хвост —
 * значит на отчёте длиннее потолка признак обрыва теряется молча, и читатель, доверившийся
 * тексту, назовёт оборванный прогон успешным. Экспортировать одно ЧИСЛО было недостаточно:
 * условие `length >= REPORT_CAP` выведено из того, КАК устроен `cap`, и при смене резака
 * (иная длина многоточия, обрезка по словам, суффикс «…ещё N символов») разошлось бы с ним
 * молча. Предикат меняется вместе с резаком, потому что стоит с ним в одном месте.
 *
 * ПОЧЕМУ НЕ ПО МНОГОТОЧИЮ. Отчёт вправе законно кончаться на «…» — это обычный знак прозы,
 * и проверка по последнему символу давала бы ложную тревогу. Правило поэтому о ДЛИНЕ: любой
 * прошедший через `cap` текст имеет длину ровно `limit`, а длиннее не бывает вовсе. `>=`, а
 * не `===`, — сторона ошибки выбрана осознанно: ровно на потолке отличить «обрезанный» от
 * «уместившегося впритык» нечем, и назвать замер несостоявшимся дешевле, чем соврать о нём.
 */
export function isReportTruncated(report: string): boolean {
  return report.length >= REPORT_CAP;
}
