 ...
 * в предложении, и у строк одной операции он общий; различает их `field`, а ключ строки на
 * экране — пара `index` + `field` (`ProposalCard`, `ProposalOverlay`). Пары с `aspect` там
 * больше нет: этого ключа сервер не пишет с Задачи 12 (см. `aspect` ниже).
 *
 * Подписи полей здесь НЕ переводятся: сервер отдаёт АДРЕС (id свойства `orbis/task_status`
 * либо имя поля записи `title`), а человеческое имя ему ставит web ПО РЕЕСТРУ
 * (`lib/registry/labels.ts`, §А9-2) — второй источник подписей на сервере разошёлся бы с
 * экранным на первом же переименовании владельцем. `summary` — запасной текст той же
 * строки: он нужен там, где адреса в реестре нет вовсе (`body`, `tags`, `emoji`).
 */
export interface ProposalOperationView {
  ...
  /**
   * ПУСТУЕТ с Задачи 12 и не заполняется НИ ОДНОЙ веткой: строка предложения адресуется
   * СВОЙСТВОМ (§А1-1), а не парой «аспект + поле». Старых прогонов это тоже касается —
   * строки не хранятся, а строятся `describeOperations` на каждом чтении, то есть всегда
   * сегодняшним кодом.
   *
   * Поле остаётся в форме как след старой формы и умирает вместе с ней (Задача 23).
   * Зависящих от него читателей нет ни на одной стороне: web с Задачи 13a подписывает
   * строку по `field` из реестра, а единственное место, которое `op.aspect` вообще берёт
   * (`ProposalOverlay` — передаёт его подсказкой резолва подписи), одинаково работает и с
   * пустым значением. Снятие поля из контракта — правка обеих сторон разом.
   */
  aspect?: string;
...
function updateRows(
  ...
  // подписывает такую строку по `field` (id свойства) из РЕЕСТРА (§А9-2, Задача 13a).
