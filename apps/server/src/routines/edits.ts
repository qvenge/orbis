...
/**
 * Ключ строки предложения НА СЕРВЕРЕ: `index`, средний слот, `field`.
 *
 * Средний слот — НЕ аспект (строк по аспектам не бывает с Задачи 12, §А1-1), а признак
 * СНЯТИЯ свойства (`UNSET_ROW_KIND` ниже): «поставить X» и «снять X» — две разные строки
 * одного свойства, и общий ключ схлопнул бы их в одну.
 *
 * С ключом ЭКРАНА (`rowKey` в `ProposalOverlay`) он не совпадает и совпадать не должен: тот
 * локальный (буфер правок и список React), собирается парой `index` + `field` и наружу не
 * уезжает. Наружу уезжает АДРЕС строки, а его сверяет `assertRowExists` — он же отбивает
 * правку с непустым `aspect` кодом `edit_key_missing`, не доходя до сравнения ключей.
 * Прежняя редакция этого докблока обещала «ту же тройку, что у экрана», и перестала быть
 * правдой, когда экран перешёл на пару (Задача 13a).
 */
