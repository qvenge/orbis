...
export interface ProposalBodyRow {
   ...
   *
   * Есть ровно тогда, когда сервер довёл предложенное тело ДО ДОКУМЕНТА, то есть при
   * `units` и при `skipped: 'rewritten'`. При `body_changed` его нет (разбор не начинался —
   * принимать всё равно нечего), при до-разборном `too_large` — тоже: документ и есть то,
   * чего потолок не даёт построить.
   */
  proposedDoc?: BodyDoc;
}
