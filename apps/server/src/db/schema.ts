...
export const entities = pgTable('entities', {
   ...
   * (та же дыра, что у `body_refs`: `db/backfill-body-doc.test.ts` называет её вслух).
   *
 ...
 * прежнее условие П5, §0.2-18). Таблица засеяна срезом Б-1; пустой остаётся только
 * `action_definitions` — до среза Б-2. Пустая таблица заводилась здесь, а не миграцией в
 * Б-1, потому что drift, `/health` и `ops.ts check` обязаны знать все пять реестров уже в
 * срезе А (§А12-1 п.4), а знать несуществующую таблицу они не могут.
 *
