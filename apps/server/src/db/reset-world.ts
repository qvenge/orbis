 ...
 * восьмая таблица, которая однажды сошлётся на `entities` и в список не попадёт, уронит
 ...
 *
 * `envelope_spent_cache` (0018) — ровно тот случай, о котором абзац выше предупреждал:
 * восьмая связь на `entities` приехала срезом Б-1, и без строки ниже прод-пересев упал бы
 * на висячей ссылке кэша.
 */
// Константа ПРИВАТНАЯ модулю — как и была до среза (тогда `GRAPH_TABLES`). Переименование
// добавило `export` без потребителя: в `reset-world.test.ts` лежит своя литеральная копия
// `WORLD_TABLES_UNDER_TEST`, и она там намеренно — список сноса обязан быть переписан в тесте
// руками, иначе пин «список отчёта = список сноса» сверял бы константу саму с собой.
const WORLD_TABLES = [
  'entities',
  'relations',
  'chat_threads',
...
export interface ResetWorldReport {
  ...
  /** Отчёт пересева шести системных реестров — он же двигает системную версию. */
 ...
 * 5. Пересев шести системных реестров тем же кодом, что `ops.ts seed-registries`. Системную
