 ...
 * политикой `owner_owns_row` (0014): видны СВОИ строки, «свои» определяется через
 * `auth.uid()`. У стартовой проверки актора нет вовсе — она ходит под `authenticated` с
 * пустыми claims, и `auth.uid()` там NULL, то есть дельт она не видит НИ ОДНОЙ (проверено
