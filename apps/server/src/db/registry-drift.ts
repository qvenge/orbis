...
/**
 * Запрос живых дельт для ПРЕДПРОСМОТРА конфликтов слияния (§А3-3) — текст один на
 * `ops.ts check`, как и запросы дрейфа выше.
 *
 * ПОЧЕМУ ЕГО НЕТ В СТАРТОВОЙ ПРОВЕРКЕ И НЕТ В `/health`. `registry_deltas` живёт под
 * политикой `current_graph_select` (0021): видны строки ТЕКУЩЕГО графа, и то лишь актору с
 * действующим грантом в нём. У стартовой проверки нет ни актора, ни текущего графа — она
 * ходит под `authenticated` с пустыми claims, обе половины предиката ложны, то есть дельт
 * она не видит НИ ОДНОЙ (свойство только усилилось против 0014, где хватало пустого
 * `auth.uid()`; проверено
 * пробоем: `SET LOCAL ROLE authenticated` + `SELECT count(*) FROM registry_deltas` = 0 при
 * непустой таблице). Поле «конфликты» в /health поэтому могло бы быть только пустым всегда
 * — мёртвый адрес, который выглядит как гарантия. Живой ответ даёт `ops.ts check`: он
 * читает админской ролью и вызывается человеком перед пересевом и после него (runbook §1).
 *
 ...
 * требует пару «актор + граф», а у стартовой проверки нет ни того, ни другого; политика
 * чтения встроенных (`read_builtin_or_own`, 0021: `graph_id IS NULL OR (graph_id =
 * current_graph_id() AND actor_reads_current_graph())`) при пустых claims пропускает ровно
 * встроенные строки — ради этого у шести реестров СВОЯ форма политики, а не общий шаблон.
 *
