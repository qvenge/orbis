...
--
-- ПОЧЕМУ ВСЕ ЧЕТЫРЕ ФУНКЦИИ — plpgsql. ЭТО ЗАМЕРЕНО, НЕ ВКУС, И МЕХАНИЗМ НАЗВАН ТОЧНО.
-- Кеша планов у SQL-функций до PostgreSQL 18 нет вовсе (локально и в CI — 17.6): тело разбирается
-- и планируется ЗАНОВО на КАЖДОМ статементе, а политика зовётся на каждом статементе транзакции.
-- Инлайн, который обычно спасает короткие sql-функции, здесь НЕДОСТУПЕН ни одной из четырёх — и не
-- из-за сублинка, а из-за `SET search_path = ''`: функцию с непустым `proconfig` PostgreSQL не
-- инлайнит НИКОГДА (пробито A/B на этом же сервере: то же тело без `SET` разворачивается в план,
-- с `SET` остаётся вызовом `Output: current_graph_id()`). А `SET search_path = ''` здесь обязателен
-- — на нём стоит запрет обхода RLS (0013:7-8), так что выбора между инлайном и безопасностью нет.
-- Цена перепланирования измерена: у функций гранта ≈0,3 мс на статемент, у `current_graph_id()`
-- 0,046 мс против 0,008 мс в plpgsql. На пути записи она копится — `fastpath:create` (десяток
-- статементов в одной транзакции) давал 55–59 мс на sql-форме против 46–53 мс на plpgsql
-- (замеры Г-4, фикс-раунды 1 и 2). У plpgsql план тела кешируется в СЕССИИ, а соединения живут в
-- пуле; холодная цена измерена отдельно и ничтожна — +0,21 мс один раз на соединение, окупается
-- вторым же статементом. Семантика и свойства те же: STABLE, SECURITY INVOKER, пустой search_path,
-- все имена со схемой — обхода RLS по-прежнему нет.
-- Не возвращай эти функции на `LANGUAGE sql` «потому что короче»: вернётся и цена на записи.
