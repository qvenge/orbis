...
-- аккаунта по построению. Выражения 35 политик RLS и 14 partial-предикатов индексов (из 17
-- partial-индексов схемы: три остальных ключ не называют — `entities_graph_updated` WHERE NOT
-- archived и два partial-unique тредов по entity_id) Postgres переписывает сам (хранятся
-- деревом, прецедент 0015:20,23) — политик и грантов здесь нет намеренно, новый предикат
-- приезжает миграцией 0021.
