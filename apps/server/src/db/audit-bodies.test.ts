...
test('печать списка виновных — СТРОКОЙ, а не `[object Object]` (ре-ревью раунда 7, Д3)', () => {
  // Прежняя печать жила в прод-обёртке, перебирала массив объектов как массив строк и выдавала
  // `[object Object]`: шаг регламента «взять напечатанные id» был неисполним. Сьют этого не
  // ловил, потому что проверял СТРУКТУРУ результата, а не вывод, — поэтому формирование строк
  // вынесено сюда и проверяется буквально.
  const lines = formatFlagged([
    { id: '019e4466-aaaa-7e07-b5d4-64be9721da51', gates: ['канон неустойчив'] },
    { id: '019e4466-bbbb-7e07-b5d4-64be9721da52', gates: ['пропало слово', 'пара разошлась'] },
  ]);
  expect(lines).toEqual([
    '  019e4466-aaaa-7e07-b5d4-64be9721da51  — канон неустойчив',
    '  019e4466-bbbb-7e07-b5d4-64be9721da52  — пропало слово, пара разошлась',
  ]);
  for (const line of lines) expect(line).not.toContain('[object Object]');
});
