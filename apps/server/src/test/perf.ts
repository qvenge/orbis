...
export async function seedPerfFixture(db: Db, ownerId: string): Promise<void> {
  ...
  // одна другая операция гейта — это и есть непокрытый кусок goal-пути. Выборка взята
  // самая широкая из осмысленных (все расходы фикстуры, ~960 строк): чем больше строк
  // проходит агрегат, тем заметнее в замере потерянный индекс или чтение всего графа.
  await runBatches(db, ownerId, [
    {
      tool: 'entity_create',
      input: {
