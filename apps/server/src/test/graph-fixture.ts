 ...
 *
 * На дереве обход вниз укладывается в порог П6 с запасом (медиана 71 мс при 100 мс), а
 * пересчёт предков в него всё равно не укладывается — но уже по ДРУГОЙ причине, разобранной
 * в докблоке `KNOWN_MISS` (`perf/graph.test.ts`): планировщик пересортировывает `nearest` и
 * `root` на каждую строку поддерева. Ромбы эту причину усиливали, но не создавали.
 */
