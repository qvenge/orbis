...
// Корпус объёма для приёмки §С8-15 и сверки «ноль расхождений» подписки Budget (задачи 9, 12 Б-1;
// с Б-2 — со снимком `test/golden/budget-engine.json`):
...
// протухал бы каждую полночь. Отсюда правило потребителям:
// `budgetOverviewOf(tx, …, { month, today: VOLUME_TODAY }, def, reg)` — да; `budgetOverview(db, …)` на
// системных часах — НЕТ: конвейер §2.8 нашёл бы просроченные инстансы (`post-due.ts`) и мутировал бы
// корпус между замерами.
//
