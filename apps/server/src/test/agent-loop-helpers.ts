...
export function agentLoopHelpers(db: Db): AgentLoopHelpers {
   ...
   *
   * `mechanism: 'seed'` (§А4-4) — решение осознанное, и вот его размен. Хелпер кладёт
   * ГОТОВОЕ состояние круга исполнителя: тикеты, прогоны на любой стадии, брошенные и
   * закрытые. Свойства прогона `system_writable` (§А2-5), поэтому без механизма сид ловил бы
   * `COMPUTED_WRITE` — и тест падал бы на подготовке, а не на проверяемом поведении. Цена:
   * фикстуре агентного цикла разрешено записать ЛЮБОЕ служебное свойство, то есть на этих
   * сьютах гейт `system_writable` не сторожит.
   *
   * Почему это приемлемо: гейт проверяется там, где он и живёт, — на боевых путях
   * (`props.test.ts`, `executor.test.ts`, `dispatch.test.ts`), причём в обе стороны и
   * мутационно. Сузить хелпер до пер-вызова значило бы размазать `mechanism` по сотне
   * фикстур ради права, которое фикстура и не проверяет. Вычисляемые свойства
