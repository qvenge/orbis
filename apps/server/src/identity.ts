 ...
 *
 * Проверка — по бренду, а не по `instanceof`: `instanceof` ломается о второй экземпляр модуля
 * (два деплоя, hmr, пересборка) и подделывается `Symbol.hasInstance`, а приватное поле — слот
 * самого объекта.
 *
