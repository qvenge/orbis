...
// только на синхронном теле (тест, вышедший в макрозадачу — любой поход в БД, — её игнорирует). Вернись
// пометка сценарию (регрессия или мутация шага 4 задачи 5) — на асинхронном теле Bun её молча
// проигнорирует, и «зелёный под пометкой» перестанет валить сьют; текстовый сторож пометок
// (`gate-c8-18.test.ts`) от тела не зависит. Поэтому походы собраны в `beforeAll`, тела читают итог.
 ...
 *  Маска — КОПИЯ `COMMENT_ONLY_LINE` гейта (с задачи 18 узкая: метод-генератор `*имя()`, код после
 *  закрытого на той же строке блочного комментария и декремент `--имя` — код, а не проза); копия
 *  потому, что импорт `scripts/` из тестов сервера не дотягивается, а совпадение исходников пиннит
 *  `scripts/check-legacy-form.test.ts`. */
const COMMENT_ONLY_LINE = /^\s*(?:\/\/|\*(?:\s|\/|$)|\/\*(?!.*\*\/\s*\S)|--(?:\s|$)).*$/;
