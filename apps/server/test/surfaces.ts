...
export async function seedSurfaceWorld(
  ...
  // их только регистрирует. Хук `beforeAll(ensureGraphs)`, который он вешает, исполнится РОВНО
  // ОДИН РАЗ — в области того файла, который импортировал этот модуль первым: остальные берут
  // модуль из кеша ES-модулей, и своего хука не получают. Строки заводит вот эта строка.
