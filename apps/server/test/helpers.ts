...
export async function truncateAll(): Promise<void> {
  ...
  // +34 % к серверному прогону. Пара удалений идёт ОДНОЙ транзакцией: строковый триггер И-1
  // отложенный, и удали мы членство отдельным statement'ом, на его коммите граф был бы ещё жив
  // и без владельца — 23514.
  //
  // ЧТО ИМЕННО ОБЕЩАЕТ КОНТРАКТ, СКАЗАНО ПО ЗАМЕРУ. Прежняя редакция говорила «граф личности
  // процесса есть, ровно с одним действующим грантом owner» — это НЕПРАВДА: `DELETE … WHERE
  // graph_id NOT IN (реестр)` оставляет ВСЕ строки членства заминченных графов, включая чужие
  // гранты от `addMember` (оператор Б в графе А). Измерено фикс-волной: у графа с одним вызовом
  // `addMember` действующих грантов 2 и до, и после `truncateAll`. Честный контракт такой:
  // чужого ГРАФА после вызова нет; граф личности процесса жив и его грант owner жив; выданные
  // тестом ЧУЖИЕ гранты в этом графе — тоже живы, и сьют, которому важно их отсутствие, обязан
  // либо минтить свой граф, либо снимать их сам. Наблюдаемых последствий сегодня нет: чтения
  // `graph_members` в сьютах идут либо под RLS (свой аккаунт), либо со `WHERE graph_id = …`, а
  // резолвер тика берёт только `owner`.
  const keep = [...MINTED];
...
export interface CustomAspectSpec {
  ...
  tagMappings?: string[];
  /**
   * Привязки аспекта к контрактам (§Б2-1). Тип — `unknown[]`, а не входная форма
   * `aspectImplementsSchema`, и это решение, а не долг: хелпер пишет значение в jsonb КАК ЕСТЬ, без
   * разбора, а приёмка гейта §С8-18 (`gate-c8-18.test.ts`) сверяет сырое содержимое колонки и держит
   * свои спеки под `as unknown[]`. Сужение типа потянуло бы правку файла гейта ради одной аннотации
   * (проверено фикс-раундом задачи 2 Б-2: TS2345/TS2322 ровно там). Форму привязки проверяют схема
   * и `checkImplements` на пути исполнителя, а не эта фикстура.
   */
  implements?: unknown[];
  /**
