...
describe('e2e слайс 1a: день из 02 §5 (два пользователя)', () => {
  ...
  test('шаг 1: seedOnboarding(A) — 17 сущностей, настройки, глобальный тред', async () => {
    ...
    // 12 категорий + 5 smart lists (три исходных + два горизонта E4) = 17 сущностей
  ...
  test('шаг 7: exportData(A) — 20 сущностей, 1 связь, 1 тред, 7 сообщений (вкл. audit и undo)', async () => {
    const exp = await a.user.exportData();
    expect(exp.format).toBe('orbis-export');
    expect(exp.version).toBe(1);

    // 17 сидов + «Обед» + «купить кроссовки» + «Дождаться зарплаты» = 20
  ...
  test('шаг 8: B независим — seed, query категорий видит только свои, undoLast не дотягивается до A', async () => {
    ...
    // Срез изоляции №3: экспорт B — только его 17 сидов, без данных A
    const bExp = await b.user.exportData();
    expect(bExp.entities.length).toBe(17);
    ...
    expect(aExp.entities.length).toBe(20);
