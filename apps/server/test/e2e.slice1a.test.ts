...
describe('e2e слайс 1a: день из 02 §5 (два пользователя)', () => {
    ...
    const sneakers = await a.entity.create({
      ...
      input: {
        ...
        props: {
          ...
          // единственная проверка ОСМЫСЛЕННОСТИ выдачи сидированных блоков (шаг 4b). Без срока блок
  ...
  test('шаг 4b: все query-блоки шести смарт-листов исполняются сервером', async () => {
    // Сервер разбирает текст каноном (§А5-3) и только им: переходный мост, читавший старую
    // форму тел, снят Задачей 21b вместе с ней, а сами тела ею же переведены в key-форму.
    // Проверять это надо на НАСТОЯЩИХ телах из БД, а не на образцах в тесте: образец
    // переживёт правку сида, а владелец увидит красную плашку.
    //
