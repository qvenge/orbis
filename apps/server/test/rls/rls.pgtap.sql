...
-- ВНИМАНИЕ, читающему красный прогон: две проверки НИЖЕ фиксируют свойство ОКРУЖЕНИЯ
-- (как настроены default privileges базы), а не свойство нашего кода. На проекте, где
-- default ACL окажется шире здешнего `Dxtm`, они покраснеют — и это НЕ дыра в
-- безопасности: закрытость держат следующие три проверки, которые выдают GRANT сами и
-- всё равно видят ноль строк. Красный тест здесь означает ровно одно: у чужих ролей
-- появилось право, которого мы им не давали, — повод посмотреть, откуда оно взялось,
-- а не повод считать таблицу открытой.
