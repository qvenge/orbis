...
import { PENDING_MARK_FILES } from './pending-marks';
...
describe('гейт §С8-18: доказательства вехи I', () => {
  test('пометки ожидаемого провала — только в файлах PENDING_MARK_FILES (Р-К-9, РП-7)', () => {
    ...
    // Исключение среза «Г» (Р-КГ-11) СНЯТО миграцией 0021; срез Б-2 (Р-К-32) вернул сторожу
    // ИМЕНОВАННЫЙ список вместо пустоты: помеченные тесты живут от вехи 0 до вехи III, и
    // выключенный на это время сторож не заметил бы забытой пометки. Равенство строгое —
    // укорачивает список та задача, которая зеленит свой файл; пустой список = прежнее «ноль».
    const files = [
      ...new Set(
        gitGrep(pattern, ['apps', 'packages', 'scripts']).map((l) => l.slice(0, l.indexOf(':'))),
      ),
    ].sort();
    expect(files).toEqual([...PENDING_MARK_FILES].sort());
  });
