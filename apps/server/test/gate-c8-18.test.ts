...
describe('гейт §С8-18: доказательства вехи I', () => {
  test('в репозитории не осталось ни одной пометки failing/todo (Р-К-9)', () => {
    ...
    // Исключение среза «Г» (Р-КГ-11) СНЯТО миграцией 0021: два сюжета «граф ≠ аккаунт» зелены
    // без пометок, и сторож вернулся к форме «ноль совпадений во всём дереве». Ослабленный
    // сторож в проде — дефект сам по себе: следующая забытая пометка прошла бы молча.
