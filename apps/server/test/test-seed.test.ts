...
describe('мир §С8-26', () => {
  ...
  test('рёбра: participant ВХОДИТ в событие (Р-И-7), envelope-binding поставил хук', async () => {
    ...
    // записи, а мир молча обещал обратное. Предикат — тот же, что у ведомости `spent` движка
    // подписки (набор `facts` через `bound_via`): сумма привязанных `expense`, не шаблонов
