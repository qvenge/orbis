...
// `bun run test`/`test:perf` (находки 38/51): сев корпуса стоит 22…25 с (число печатает сам
// прогон строкой `perf: корпус … засеян за N с`; четыре засева этой задачи — 22,7 / 22,9 /
// 23,1 / 24,5 с), а под
 ...
 *
 * ПЕРЕЗАМЕР ПОСЛЕ 0017 (2026-09-02, MacBook, локальный Postgres в Docker, корпус 50 000 /
 * 149 999, два прогона подряд — свежезасеянный корпус и он же из кеша): `descendants_of:subtree`
 * p95 = 67,3 мс на свежем и 78,1 мс на кешированном, `recompute:subtree5k` — 679,9 и 673,5 мс.
 * Числа записаны потому, что снимают известный долг ветки: ДО миграции 0017 тот же замер на
 * свежезасеянном корпусе давал p95 = 134 мс против порога 100 (медиана 74, три выброса из
 * двадцати), и порог брался только на прогретом (82 мс). Причина ухода вдвое названа
 * замером, а не догадкой: `rel_uniq` переехал с производной `relation_type` на саму роль
 * `(source_id, target_id, role)`, и обход получил ВТОРОЙ пригодный индекс с Index Only Scan —
 * `target_id` лежит в самом индексе, ходить в кучу больше не надо (`explain.test.ts`,
 * инвариант `WALK_INDEXES`). Пороги при этом НЕ трогаются: они дословно из спеки, и снижать
 * их «по факту замера» значило бы менять приёмку под результат.
 */
...
test('корпус наполнен: обход идёт по данным, а не по пустоте', async () => {
  ...
  // Обход НЕ выродился в «весь корпус»: поддерево узла первого уровня (SUBTREE_ROOT_LEVEL)
