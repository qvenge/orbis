 ...
 *
 * ПЕРЕЗАМЕР ПОСЛЕ РЕФОРМЫ СВОЙСТВ (2026-09-02, MacBook, локальный Postgres в Docker, два
 * прогона подряд; фикстура пересевается каждым прогоном, поэтому «прогретого» корпуса у этого
 * файла не бывает — греется только кеш страниц): 12,4/13,1 · 11,0/11,2 · 51,0/54,0 ·
 * 25,9/27,5 · 20,8/20,9 · 41,7/39,4 · 25,8/28,0 мс. Все семь — лучше худшего наблюдённого
 * ниже, и ни одна не сдвинулась от прошлого замера больше чем на пятую часть, поэтому пороги
 * НЕ пересчитаны: гейт ловит регрессию в разы, и подтягивать его вниз по шуму раннера значит
 * менять смысл гейта, а не его точность.
 */
 ...
 * платит и БЕЗ кэша (их платил и оракул, снесённый срезом Б-2). Цена запроса ЦЕЛИКОМ печатается
 * второй строкой БЕЗ порога: она честно зависит от машины, и прятать её незачем — дрейф виден и
 * на зелёном.
 *
