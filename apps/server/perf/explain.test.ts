...
test('несущий индекс рекурсивного обхода: обход идёт ПО ИНДЕКСУ с префиксом source_id', async () => {
  ...
  // …и никакого ТРЕТЬЕГО индекса на `relations` в плане обхода быть не может: `used` —
  // подмножество двух допустимых по построению фильтра, поэтому проверяется само множество.
  expect(used.every((i) => (WALK_INDEXES as readonly string[]).includes(i))).toBe(true);
}, 300_000);
