...
// запроса политику `current_graph_select` (0021: security qual «строка ТЕКУЩЕГО графа ∧ актор
// держит грант» — `graph_id = (SELECT current_graph_id()) AND (SELECT actor_reads_current_graph())`),
// и планировщик видит
...
// Политика `current_graph_select` (0021) — security qual, а операторы containment НЕ leakproof
...
test('вердикт по entities_props_gin: горячий запрос — containment по props', async () => {
  ...
  // на все три GIN и не в индексе: политика `current_graph_select` приходит security qual'ом, а
 ...
 * политики `current_graph_select`, и вывод менялся на противоположный. Ровно та ловушка, о
