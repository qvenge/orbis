...
apps/web        — PWA (React): экраны (Browser, Budget, Agenda, чат),
                  ...
                  tRPC-клиент; страницы и шаблоны [D45] — features/page:
                  рендерер показа тела (Renderer), экран записи через
...
apps/server     — Hono + @hono/trpc-server: tRPC-роутеры entity/relation/
                  ...
                  конвейер мутаций (PRD 01 §9.2), движок правил каталога —
                  применение шаблонов requires_when/forbidden_when/
                  on_enter_class/default/unique_among на трёх путях записи
                  (PRD 01 §4.16) [D43], actions — резолв декларации действия
                  в шаги и их исполнение одним batch'ем (run_action,
                  PRD 01 §9.2), политика подтверждений
                  ...
                  (PRD 01 §8), экспорт (PRD 01 §9.4), пачка данных блоков
                  страницы — routers/entity-blocks (entity.blocks: SAVEPOINT
                  на блок, одна транзакция исполнения) и пачка правок
                  entity.updateBatch (PRD 01 §9.1) [D45], индекс аспектов
                  промпта — llm/aspect-index (PRD 01 §7.1) [D45], перепись
                  корпуса тел перед форматом v3 — db/census-v3 (ops.ts)
...
packages/shared — Zod-схемы wire-контрактов (вход/выход tRPC-процедур,
                  ...
                  правила каталога и действия (PRD 01 §4.16) [D43];
                  грамматика тела v3 [D45] — doc/page-grammar (листовой
                  препроход маркеров {{…}}: ЕДИНСТВЕННАЯ копия правил,
                  сторож scripts/grammar-copies.test.ts), doc/placement
                  (матрица «где работают блоки», ошибки мест, bodyIssues),
                  doc/nodes/{layout,record-blocks} (узлы columns/column/
                  tabs/tab/recordBlock/aspectCard); выбор шаблона —
                  pages/choose-template (чистая функция для всех клиентов);
                  контракты пачек — contracts/blocks (PRD 01 §3.18, §9.1)
```
...
9. Экран записи не тянет схему документа [D45]: `DetailScreen` и его эагерные файлы (шаблон хоста, рендерер, первый кадр тела) импортируют из `@orbis/shared` только листовые сабпаты `doc/page-grammar`, `doc/placement`, `doc/types`, но не баррель `@orbis/shared/doc` (схема tiptap нужна лишь ленивому редактору). Держат это `scripts/check-lazy-chunks.ts` (ребро `DetailScreen ↛ doc` по dist и ленивость чанка `DetailMenu`) и `save.test.tsx` (поимённый список эагерных файлов).
10. Маркеры тела `{{…}}` распознаёт одна копия правил — `packages/shared/src/doc/page-grammar.ts` [D45]. Первый кадр, рендерер, бейдж закреплённого и обёртка блока запроса читают её; свой регэксп маркера в другом месте — дефект (сторож `scripts/grammar-copies.test.ts` со списком законных мест и причинами).
...
flowchart TD
    ...
    BodyQuery["блоки данных {{query:...}} в теле\n(пачкой entity.blocks, D45)"]
...
В-третьих, из перечня будущих потребителей убраны **авто-чекины привычек**: аспекта `orbis/habit` в системе нет, и потребителем он не был никогда. **Подписки поверхностей** из перечня «будущих» ушли: срез Б-1 их исполнил — `orbis/agenda` и `orbis/budget-overview` компилируются движками подписок в тот же SQL, у деклараций есть валидатор и тулы владельца (PRD 01 §6.3, §11). Будущей точкой вызова осталась одна: **область показа свойства** (`scope`: значение хранится и читается графом зависимостей, в SQL не компилируется). **Правила каталога** срез Б-2 исполнил (PRD 01 §6.3), но точкой вызова компилятора они не стали: условие правила — дерево языка выражений, его считает интерпретатор на записи.

**Что на диаграмме изменил срез страниц 1а** [D45]. Блоки данных тела больше не ходят за данными по одному: клиентский собиратель (`lib/query-blocks/batch`) складывает блоки окна в одну пачку `entity.blocks` — до 30 блоков по тексту запроса, ключ блока — нормализованный текст, поэтому первый кадр и редактор делят кеш и не спрашивают дважды. Сервер разбирает каждый текст по реестру владельца, компилирует тем же компилятором, материализует повторы по объединению окон (своей транзакцией, как `entity.query`) и исполняет пачку в одной транзакции с SAVEPOINT на блок — ошибка блока остаётся его плашкой. Точкой вызова это не новая: пачка — тот же модуль, что процедура запроса. Бейджи закреплённых по-прежнему считает отдельный `entity.count` — второй путь данных, его снимает срез 1б (размещения).
