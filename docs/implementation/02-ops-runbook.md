...
| `SUPABASE_URL` | база JWT-верификации: issuer-пиннинг + из него выводится JWKS-адрес (`auth.ts`) | `https://<PROD_REF>.supabase.co` |
| `SUPABASE_JWT_SECRET` | верификация JWT (HS256, fallback к JWKS) | из Supabase `Project Settings → API` |
