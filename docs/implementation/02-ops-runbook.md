...
> **Статус (после Фазы A слайса 1c-2):** `render.yaml` в корне пока ещё спайковый
> (`orbis-spike05`, watch-ветка `spike/phase-0`, `rootDir: spikes/spike-05-deploy`). Его
> прод-финализация (re-point на `main`, прод-секреты, раздача web same-origin) — задача **Фазы B**
> (владельческие гейты). До неё этот раздел описывает целевую конфигурацию, а не текущий файл.
