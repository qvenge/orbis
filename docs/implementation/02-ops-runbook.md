...
>
> **2. Проверить, что пересев доехал.**
>
> ```bash
> curl -fsS https://<prod-host>/health     # ждём ровно {"status":"ok"}
> bun scripts/ops.ts check                 # то же самое вторым способом, только чтение
> ```
>
> `{"status":"ok","aspectDrift":["orbis/goal"]}` — пересев не выполнен или выполнен не в ту
> базу. `"aspectDrift":"unknown"` — проверка не отработала (БД была недоступна на старте),
> состояние реестра неизвестно: перезапустить сервис и посмотреть снова, «unknown» за «ok»
> не считать.
>
> **3. Досев горизонтов существующему аккаунту — происходит сам, но знать, чего ждать.**
...
>
> **4. Убедиться глазами (это же — приёмка 02-core-os §9 п. 9).**
>
...
>
> **5. Заполнить строку истории перф-замера** (§8 ниже): деплой фазы E — последний релиз
> слайса 3, то есть тот самый «первый релиз со слайса 3», который обязан внести четыре
> клиентских числа. Условия замера — там же; пропуск помечать прочерком, а не пустотой.
