...
> подтверждением пересева**. Подтверждают только п. 2 и создание цели в п. 4.
>
...
> ```
>
> `✗ orbis/goal: в реестре БД НЕТ` — пересев не выполнен или выполнен не в ту базу:
...
> пересев ушёл не в ту базу. `"aspectDrift":"unknown"` — проверка не отработала (БД была
> недоступна на старте), состояние реестра неизвестно: перезапустить ещё раз и посмотреть
> снова, «unknown» за «ok» не считать.
>
