...
-- все четыре: plpgsql, STABLE, SECURITY INVOKER, SET search_path = '' (правка 21.09 по гейт-ревью и
-- ре-ревью Г-4, Ф-Г-68 с уточнением): тело на `sql` PostgreSQL 17.6 перепланирует на КАЖДОМ статементе
-- (+13 мс на создание записи), а инлайна, ради которого стоило бы терпеть, нет ни у одной — функцию с
-- непустым proconfig планировщик не инлайнит никогда, а SET search_path = '' здесь обязателен
...
agent_grants.issued_by NOT NULL;  pgTAP — plan(158);  пометок test.failing в дереве — 0
```
...
```
  (в) перед `SELECT finish();` — группа 23, **23 проверки**; `SELECT plan(125);` → **`SELECT plan(158);`**:
```sql
...
BEGIN
	RETURN (nullif(current_setting('request.jwt.claims', true), '')::jsonb ->> 'graph')::uuid;
END $$;
--> statement-breakpoint
...
```
  ...
  Ожидание: `db:prepare` — EXIT=0, накат `0000…0021`, `test:rls` — `plan(158)` без «Looks like you planned»; политик всего
...
*Consumes:* Г-1…Г-4 в `main` (`0019…0021`, `plan(158)`, ноль пометок `test.failing`, маркер `owner-key`); базовая линия Г-0;
...
GRANT SELECT ON graph_members TO orbis_app;
```
    и число проверок в тексте рядом — `plan(158)`. Сверка блока с миграциями: `grep -n '^GRANT\|^REVOKE' apps/server/src/db/migrations/*.sql`.
...
```
  ...
  pgTAP на восстановленной базе — `plan(158)` зелёный (файл — одна транзакция с ROLLBACK, данные не трогает).
  **Тем же заходом — ревизия следов двух закрытых дыр** (правка 21.09 по гейт-ревью Г-4; исправлена
  21.09 по финальному ревью, линза RLS): до среза в базе можно было завести тред на сущности ЧУЖОГО
  графа (`chat_threads.entity_id`) и строку кэша на конверте чужого графа
  (`envelope_spent_cache.envelope_id`) — политики этого не проверяли. Прод не пуст, поэтому счёт таких
  строк снимается на восстановленном дампе **ПОСЛЕ `db:migrate`** (не до: три миграции идут ОДНОЙ
  транзакцией, состояния «накатан только `0019`» не существует, а до миграции колонка зовётся
  `owner_id`). Строки-дыры миграцию переживают — измерено финальным ревью, счёт до и после равен:
```sql
SELECT count(*) FROM chat_threads t JOIN entities e ON e.id = t.entity_id WHERE e.graph_id <> t.graph_id;
SELECT count(*) FROM envelope_spent_cache c JOIN entities e ON e.id = c.envelope_id WHERE e.graph_id <> c.graph_id;
```
  Ожидание — нули (у владельца один граф, разойтись было нечему). Ненулевой счёт — СТОП и доклад: это не
  «починить политикой», а разобрать, откуда строки, и решить их судьбу ДО прод-процедуры. Числа — в
  `progress.md` и в приёмку. Любой отказ
...
- [ ] **Шаг 16: статусы.** `04-decision-log.md`, D44 «Статус» — дата прода и итог приёмки (9/9); `03-pending.md` §2.2а — «в
  ...
  цифры (счётчики сьютов, `plan(158)`, семь медиан Г-0 → Г-4, длительность окна, расход токенов), находки ревью
...
- [ ] **Шаг 17: `/Users/birzhan/projects/orbis/.superpowers/sdd/2026-09-20-graph-ownership/handoff-b2.md` — факты для правки плана Б-2 (спека §8.2; правка — отдельным словом владельца).**
  ...
  3. **pgTAP:** `SELECT plan(158);` (`rls.pgtap.sql:6`) — пять мест плана Б-2 с `plan(97)` (`:225, :321, :355, :3421, :3592`);
...
| IV | Г-4 | **гейт: `graph-vs-account.test.ts` зелёный без пометок**; `plan(158)`; политик 77, `owner_owns%` — 0; тест дрейфа зелёный без идентичности; четыре мутации политик; перф по Р-КГ-10, вердикты `explain` = Г-0 |
| V | Г-5 | гейты доков (фраза PRD — 0, `owner_id` в живых доках — только исторические записи); пин версий; финальное ревью APPROVE; репетиция на дампе (журнал 17 → 20, политик 77, `plan(158)`); прод; приёмка 9/9; `handoff-b2.md` |
...
- **Согласованность типов.** Сверены имена и формы между задачами: `graphId`/`graph_id` (Г-1 → все); `mintGraph`/`freshGraph`/
  ...
  же; `plan(97)` → `plan(125)` (Г-2, +28) → `plan(158)` (Г-4, +23); журнал миграций 17 → 20 записей; политик 39 → 44 (Г-2) → 77 (Г-4).
