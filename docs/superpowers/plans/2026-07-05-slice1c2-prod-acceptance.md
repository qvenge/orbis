...
### Осознанно НЕ починенное в ревью 2026-07-09 (вход в план слайса 2)

Найдено и подтверждено адверсариальной верификацией, но не исправлено — цена/риск правки
выше пользы прямо сейчас. Каждый пункт — кандидат в слайс 2.

> **Полный список всех незакрытых находок (включая ~35 Minor с file:line и сценариями
> отказа): `docs/superpowers/reviews/2026-07-09-review-backlog.md`.** Ниже — только крупное.

- **Конкурентный ретрай `ai.sendMessage` во время первого прогона** (нашли независимо два ревьюера).
  `appendMessageIdempotent` даёт `replayed=true`, но ответа ещё нет → запускается второй
  параллельный tool-цикл: двойное исполнение действий, двойной метеринг, два assistant-ответа
  с одним `replyTo`. Окно — вся длительность цикла (десятки секунд). Фикс требует либо
  advisory-lock на весь цикл (транзакция на минуты — не годится при пуле max=3), либо
  persist-маркера «in progress» и нового wire-статуса для UI. Архитектурное решение.
- **Частичный цикл при сбое провайдера**: действия шагов 1..k применены, ответа нет; легитимный
  ретрай §7.9 гоняет новый полный цикл, и модель создаёт сущности повторно (id генерирует она).
  Митигация сейчас вероятностная: audit-строки прошлого прогона попадают в контекст.
- **`attach orbis/budget` обходит «один budget-parent»** (§4.2/§13.7): инвариант энфорсится только
  в `relation_create`. Attach на сущность, уже являющуюся parent'ом транзакций, ретроспективно
  делает её вторым budget-parent'ом.
- **`refusal` модели сводится к `end_turn`** — отказ неотличим от успеха. Расширение `stopReason`
  ломает пиннинг-тест Вехи 0; решение принималось осознанно, но стоит пересмотреть.
- **Optimistic-check body в пределах одной миллисекунды**: `updatedAt = clock()` не монотонен,
  два апдейта в один тик оставляют `updated_at` прежним → stale-правка проходит проверку.
- **`chat_messages` доступна на UPDATE/DELETE под `authenticated`** (§4.6 append-only не энфорсится
  БД). Сейчас недостижимо (Data API off, tRPC-поверхности нет) — defense-in-depth.
- **Экспоненциальный CTE ацикличности**: путь-массив вместо посещённых вершин; ромбовидный
  blocks-граф из ~30 рёбер взрывает перечисление простых путей.
- **UI retry-буфера**: тап по индикатору не открывает список ожидающих и не даёт отменить запись
  (02 §2.6); переход «⏳ ждёт отправки» → «⚡ без AI» после успешного flush не реализован;
  ручного «отправить сейчас» нет — при транспортном сбое на живой сети очередь ждёт события `online`.
- **UNAUTHORIZED → мгновенный signOut** без попытки `refreshSession`: протухший токен посреди
  мутации убивает сессию.
- **Дыры инвалидации §5.1**: `entity.update` инвалидирует только `entity.get` этого id — списки
  Browser, счётчики и пины показывают устаревшее до 30 c.
