...
>
> **СТАТУС: АРТЕФАКТ — ИСПОЛНЕНИЕ НА HOLD.** Владелец 2026-07-05 распорядился: слайс 1c-1 писать и сразу исполнять (СДЕЛАНО — main `273e051`, CI green), а 1c-2 «Прод» написать артефактом и держать исполнение до возврата владельца. Причина HOLD — владельческие инфра-гейты: Render-аккаунт/Blueprint, секреты на Render, реальный `ANTHROPIC_API_KEY` (гейт llm-smoke), решение по прод-проекту Supabase.
>
> **Две фазы.** **Фаза A (Задачи 1–8)** — чистый код/CI/Docker/бэкап, БЕЗ владельческих гейтов: исполнима автономно по отдельному «go» владельца, не дожидаясь инфраструктуры. **Фаза B (Задачи 9–13)** — инфра, деплой, приёмка §8: требует владельческих гейтов. Не начинать Фазу B, пока гейты не открыты.
...
- **DSN-режимы (обязательно):** боевой пул API — session-пулер `:5432` c `PG_PREPARE=true`; любой transaction-режим `:6543` — только `PG_PREPARE=false` (prepared statements несовместимы). Хост — `aws-1-eu-central-1.pooler.supabase.com` (кластер `aws-1`, не `aws-0`). Username кастомной роли через пулер — `orbis_app.<PROD_REF>`. Прямой `db.<ref>.supabase.co` — IPv6-only, НЕ используется (Render egress IPv4).
...
- [ ] **Step 1: Падающий тест** — вызвать процедуру `ai.sendMessage` с `ctx.ai = undefined` → ожидать проброс ошибки (а не тихую сборку). Пример:
```ts
...
```
- [ ] **Step 2: Запустить — падает** (`bunx bun test apps/server/src/ai/send-message.test.ts` — сейчас фолбэк собирается и падает иначе/не падает).
- [ ] **Step 3: Реализация** — заменить тело `defaultAiDeps()` на `throw new Error('ai deps must be injected; ctx.ai is required')`; в `ai.ts:40` оставить `ctx.ai ?? defaultAiDeps()` (теперь бросит при отсутствии). Убедиться, что тесты, которым нужен `ai`, инжектят его.
- [ ] **Step 4: Прогнать** server-тесты — зелёные.
- [ ] **Step 5: Commit** `fix(server): defaultAiDeps fail-fast throw вместо ленивой сборки (§DI hardening)`.
...
- [ ] **Step 1: Падающий golden-тест** — тред с ДВУМЯ сообщениями в одну и ту же `createdAt` (разные id), пагинация по границе именно между ними → оба должны появиться ровно один раз (без пропажи/дубля). Ассерт по стабильному порядку `(createdAt desc, id desc)`.
- [ ] **Step 2: Запустить — падает** (текущий ms-курсор теряет одно из двух).
- [ ] **Step 3: Реализация сервер** — расширить `before` до составного: принимать `before: { createdAt: string; id: string }` (или `"<iso>|<id>"`), фильтр `or(lt(createdAt, c.createdAt), and(eq(createdAt, c.createdAt), lt(id, c.id)))`. Сохранить `limit max(200)`, DESC.
- [ ] **Step 4: Реализация клиент** — `useChatThread` формирует курсор из `(oldest.createdAt, oldest.id)` вместо только `createdAt`; типы wire обновить.
- [ ] **Step 5: Прогнать** server + web тесты — зелёные (проверить, что существующие chat-тесты не сломаны сменой формы курсора).
- [ ] **Step 6: Commit** `fix(chat): составной курсор (createdAt,id) в listMessages — устойчивость к ms-коллизии`.
...
- [ ] **Step 1: Падающий тест (out-of-order replay)** — user-сообщение A (упало без ответа), затем идёт B и получает ответ Rb; ретрай A с тем же client-id → replay НЕ должен вернуть Rb (чужой ответ). Ассерт: replay для A возвращает либо свой ответ Ra (если есть), либо честно проходит в провайдер, но НИКОГДА Rb.
- [ ] **Step 2: Запустить — падает** (текущий `findAnswerAfter` вернёт Rb).
- [ ] **Step 3: Реализация** — при создании assistant-сообщения писать `metadata:{ cards, replyTo: userMessage.id }` (`:281`); в replay-ветке (`:167`) искать ответ по `metadata.replyTo === appended.message.id`, а не по временно́му курсору; `findAnswerAfter` оставить как фолбэк/удалить по факту.
- [ ] **Step 4: thread_post идемпотентность** — тест: ретрай агентского `thread_post` с тем же client-id не создаёт второй пост (ON CONFLICT). Дореализовать при необходимости.
- [ ] **Step 5: pending-dedup по batch_id** — тест: ретрай моделью того же batch на explicit-уровне не плодит вторую pending-карточку (`progress.md:250-251`); дедуп по исходному `batch_id`.
- [ ] **Step 6: Прогнать** — зелёные.
- [ ] **Step 7: Commit** `fix(ai): детерминированный replay по metadata.replyTo + thread_post/pending идемпотентность (§7.9 retry contract)`.
...
- [ ] **Step 1: Падающий тест** — POST `/mcp` с телом > лимита БЕЗ `content-length` (chunked/stream) → ожидать 413 (сейчас проскакивает).
- [ ] **Step 2: Запустить — падает.**
- [ ] **Step 3: Реализация** — подключить `@hono/hono` `bodyLimit({ maxSize: MCP_MAX_BODY_BYTES, onError: → 413 })` на роут `/mcp` (считает фактически прочитанные байты, не доверяя заголовку); удалить ручной NaN-nit или оставить как быстрый пред-чек. Сохранить 405-method-gate.
- [ ] **Step 4: Прогнать** — зелёные (413 и для content-length, и для chunked).
- [ ] **Step 5: Commit** `fix(mcp): платформенный bodyLimit /mcp закрывает chunked-обход (§MCP hardening)`.
...
- [ ] **Step 1: Решение** — ограничить `push` только ветками, которые НЕ покрыты PR: напр. `on: push: {branches: [main]}` + `pull_request: {}` (feature-ветки проверяются через PR, main — через push). ЛИБО `concurrency.group: ci-${{ github.event.pull_request.number || github.ref }}` + отмена. Выбрать первый (проще, детерминированнее).
- [ ] **Step 2: Правка** `ci.yml`: `on: { push: { branches: [main] }, pull_request: {} }`. (Проверить: слайсовые ветки исполняются через PR → CI на PR; прямой push в main — CI на push; двойного прогона нет.)
- [ ] **Step 3: Verify** — на тестовом PR ровно один CI-ран; push в main — ровно один.
- [ ] **Step 4: Commit** `ci: дедуп push↔PR (push только main, feature — через PR)`.
...
- [ ] **Step 1: Dockerfile** (пример; уточнить по факту зависимостей):
```dockerfile
...
```
- [ ] **Step 2: `.dockerignore`** — `node_modules`, `**/dist` (кроме собираемого в образе), `.git`, `supabase/.temp`, `spikes/**/.env`, `apps/server/.env`.
- [ ] **Step 3: Verify локально** — `docker build -t orbis .` успешно; `docker run -e DATABASE_URL=... -e PORT=3001 -p 3001:3001 orbis` → `curl localhost:3001/health` = `{status:'ok'}` (нужен доступ к БД; допускается smoke без БД на /health, если он не бьёт в БД).
- [ ] **Step 4: Commit** `feat(ops): прод-Dockerfile монорепо (Bun 1.2.7, web build + API)`.
...
- [ ] **Step 1: Падающий тест (статика)** — GET `/` возвращает `index.html`; GET `/assets/<x>.js` отдаёт ассет; неизвестный не-API путь → SPA-fallback `index.html`; `/trpc/*`, `/mcp`, `/health` НЕ перехвачены статик-роутом.
- [ ] **Step 2: Запустить — падает** (API сейчас не раздаёт статику, `index.ts` только `/trpc/*`,`/mcp`,`/health`).
- [ ] **Step 3: Реализация сервер** — подключить `serveStatic` (`@hono/node-server/serve-static` или встроенный Bun-статик) с корнем `apps/web/dist`; порядок роутов: API-роуты ПЕРЕД статикой; SPA-fallback (`notFound → index.html`) ТОЛЬКО для не-API GET (не ломать 404 API). Учесть, что PWA `sw.js`/`manifest.webmanifest` отдаются с корректным content-type.
- [ ] **Step 4: Реализация клиент (опц. режим B)** — `trpc.ts`: `const apiBase = import.meta.env.VITE_API_URL ?? ''; url: `${apiBase}/trpc``. Пусто → относительный (режим A). Не менять поведение по умолчанию.
- [ ] **Step 5: Прогнать** server + web тесты — зелёные; `cd apps/web && bunx vite build` даёт dist; локально `docker run` (Task 6) → `curl /` = html, `curl /health` = ok, `curl -XPOST /trpc/...` работает.
- [ ] **Step 6: Commit** `feat(server): same-origin раздача web-dist (SPA-fallback) + опц. VITE_API_URL клиента`.
...
- [ ] **Step 1: Перенести + адаптировать `scripts/backup.sh`** — заменить проверку `spike_items` на реальные таблицы (напр. проверять непустоту `entities`/наличие ожидаемых таблиц в дампе); DSN через `ADMIN_DSN` env (session-пулер `postgresql://postgres.<PROD_REF>:<pwd>@aws-1-eu-central-1.pooler.supabase.com:5432/postgres`); пред-чек версии pg_dump (иначе `docker run postgres:17-alpine pg_dump`).
- [ ] **Step 2: `.github/workflows/backup.yml`** — `on: schedule: [{cron: '0 3 * * *'}] + workflow_dispatch`; шаг `pg_dump` через `ADMIN_DSN` (secret); дамп в артефакт Actions (retention 7–30 дней). Секрет `ADMIN_DSN` — владельческий гейт (Фаза B), поэтому workflow добавляется, но реальный секрет заводится позже.
- [ ] **Step 3: Runbook `docs/implementation/02-ops-runbook.md`** — секции: деплой (Blueprint, секреты, PAT-выпуск `bun scripts/issue-pat.ts`); бэкап (ручной `backup.sh` + cron + восстановление `psql < dump`); keep-warm (polling петли / апгрейд $7); пауза Supabase Free ~7 дней (restore из dashboard); health-мониторинг `/health`.
- [ ] **Step 4: Verify** — `bash scripts/backup.sh` синтаксически валиден (shellcheck); workflow-yaml валиден; runbook без TBD/TODO.
- [ ] **Step 5: Commit** `feat(ops): backup.sh в scripts/ + GH Actions cron + ops-runbook`.
...
- [ ] **Step 1: Владельческое решение** — прод-проект Supabase eu-central-1 (чистый прод ИЛИ переиспользовать spike-проект `bmxofoqkksofqojnwpjx`, очистив `spike_items`). Зафиксировать `PROD_REF`. Data API — ОТКЛЮЧЁН (один путь мутаций).
- [ ] **Step 2: `db:prepare` против прода** — `DATABASE_URL_ADMIN='postgresql://postgres.<PROD_REF>:<pwd>@aws-1-eu-central-1.pooler.supabase.com:5432/postgres' bun run db:prepare` (миграции + роль `orbis_app` (пароль из env) + сид реестра аспектов + сид онбординга + RLS-тест). Проверить `has_schema_privilege` на `auth` (тихий провал GRANT — грабля findings:50).
- [ ] **Step 3: Verify** — RLS-матрица зелёная против прод-пулера на `:5432` (`prepare:true`) И `:6543` (`prepare:false`); `pg_roles`: `orbis_app` `rolbypassrls=false,rolsuper=false`; логин `orbis_app.<PROD_REF>` через пулер проходит.
- [ ] **Step 4: Зафиксировать** прод `DATABASE_URL` (session-пулер, `orbis_app`) для Render-секрета (Task 10).

### Task 10: `render.yaml` re-point на прод + секреты

**Владельческий гейт:** Render-аккаунт, Blueprint, все `sync:false` секреты.

- [ ] **Step 1: Переписать `render.yaml`** — `branch: main`, `rootDir: .` (корень), `dockerfilePath: ./Dockerfile` (Task 6), `healthCheckPath: /health`, `region: frankfurt`, `plan: free`. Убрать `SPIKE_CHECK_TOKEN` и spike-rootDir.
- [ ] **Step 2: envVars** (`sync:false` где секрет): `DATABASE_URL` (прод orbis_app session-пулер), `PG_PREPARE=true`, `PORT=3001`, `ORBIS_PAT_HASH`, `ORBIS_PAT_OWNER_ID`, `ANTHROPIC_API_KEY`, `ORBIS_LLM_MODEL` (опц.), `SUPABASE_JWT_SECRET`/`SUPABASE_JWKS_URL`, а также build-env для web: `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY` (прод), `VITE_API_URL` пусто (режим A same-origin). Сверить `healthCheckPath` payload (`/health` отдаёт `{status:'ok'}`).
- [ ] **Step 3: PAT-выпуск** — `bun scripts/issue-pat.ts` (печатает `ORBIS_PAT_HASH` + сырой токен ОДИН раз); сырой токен — владельцу для агента Claude Code; hash+owner_id — в Render-секреты. НЕ логировать/не коммитить сырой токен.
- [ ] **Step 4: Blueprint apply** (владелец) — подключить репо, создать сервис из `render.yaml`, задать `sync:false` секреты в Render UI.
- [ ] **Step 5: Commit** `feat(ops): render.yaml re-point на main — прод API+web same-origin, прод-секреты`.
...
- [ ] **Step 1: Deploy** через Render (авто по push main после Blueprint) — дождаться живого сервиса.
- [ ] **Step 2: Прод-smoke** — `curl https://<prod>/health` = `{status:'ok'}`; открыть web в браузере → login (Supabase прод) → онбординг (сид) → fast-path «обед 340» → мгновенная «⚡ без AI» → Browser список/бейджи → detail → чекбокс task → настройки (смена timezone) → «Экспорт данных» скачивает `orbis-export`. (= ручной прогон из Verification 1c-1, но на ПРОДЕ.)
- [ ] **Step 3: MCP-smoke** — агентом с прод-PAT: `/mcp` initialize + `entity_query` (по `updated_at`) + один `entity.create` через §7.10 → появляется в web владельца с `actor_kind=agent`.
- [ ] **Step 4: Замер cold-start** — после 15+ мин простоя первый запрос; зафиксировать латентность в runbook; решить keep-warm vs апгрейд.

### Task 13: Приёмка §8 слайса 1

**Источник:** `docs/prd/00-product.md` §8/§9. Всё — в ПРОДЕ, реальным агентом Claude Code через MCP/PAT.

- [ ] **Step 1:** Агент через `/mcp` (PAT) создаёт проект-сущность **«Orbis»** (`title==='Orbis'`) в проде.
- [ ] **Step 2:** Агент переносит документацию — спеки/планы как **note-сущности** (`entity.create`, note), связанные `parent` с проектом.
- [ ] **Step 3:** Разработка слайса 2 ведётся через **задачи в Orbis**: агент двигает статусы (`entity.update`), пишет заметки (`thread_post`) в тредах.
- [ ] **Step 4:** Владелец с телефона (PWA) **наблюдает** (карточки/аудит `actor_kind=agent`) и **отвечает в тредах**.
- [ ] **Step 5:** Проверить видимость/атрибуцию/обратимость действий агента (журнал §7.8, Undo, `actor_kind=agent`, `source=mcp`).
- [ ] **Step 6:** Отметить приёмку в `docs/prd/00-product.md` §8 + приёмочный лог как артефакт; зафиксировать явное поведение touch-семантики треда (Task 3) как принятое.
...
## Владельческие гейты (сводка — блокируют Фазу B)

1. Render-аккаунт + Blueprint (подключить репо, создать сервис из `render.yaml`).
2. Секреты на Render (`sync:false`): `DATABASE_URL` (прод), `ORBIS_PAT_HASH`, `ORBIS_PAT_OWNER_ID`, реальный `ANTHROPIC_API_KEY`, `SUPABASE_JWT_SECRET`/JWKS, `VITE_SUPABASE_*`, `ADMIN_DSN` (для backup cron).
3. Решение прод-vs-spike проекта Supabase (`PROD_REF`); при переиспользовании spike — очистить `spike_items`.
4. Развилка топологии раздачи web: A) same-origin (рекомендовано, реализовано в Task 7) vs B) раздельные origins (+CORS-middleware +`VITE_API_URL`).
5. Стоимость/cold-start: принять free с keep-warm ИЛИ апгрейд Render Starter $7/Fly ~$2.24 (нужна карта).
6. Бэкап-расписание/хранилище дампов (GH Actions cron artifact vs иное).

## После 1c-2

Слайс 1 закрыт (§8 принят). Далее — **Слайс 2** (Budget view, recurring, Agenda-lite, CSV-импорт с дедупом через `entity_origins` — стабы уже в `packages/shared`), который по критерию §8 ведётся уже ЧЕРЕЗ Orbis (агент двигает задачи в проде). Отложенное низкого приоритета: стриминг экспорта (если упрётся в память на объёме доков), health-мониторинг/алертинг, pgTAP косвенных политик.
