...
- **РП-24.** Ветка `pages-slice-1a`, worktree `.claude/worktrees/pages-slice-1a`, мерж в `main` после каждой задачи.
...
| В-4 | Выкатить индекс аспектов (закрывает прод-дефект §С8-35 п. 3) отдельно сразу после задачи 1? | нет: вместе со срезом (прод только с владельцем, автодеплой выключен) | 1, 19 |
...
- **Ветка и дерево.** Ветка `pages-slice-1a` от свежего `origin/main`, работа только в worktree
  `/Users/birzhan/projects/orbis/.claude/worktrees/pages-slice-1a`; основное дерево не трогать (кроме docs-коммитов
  автодеплоя задач 1 и 19). Свой `bun install`; `apps/server/.env` и корневой `.env` — копии из основного дерева.
  У ветки снять upstream (`git branch --unset-upstream`; Ф-Б2-2): иначе голый `git push` уйдёт в `main`.
- **Автодеплой Render выключен задачей 1** docs-коммитом в `main` ДО создания ветки (образец `33763b0`), возвращается
  задачей 19 (образец `9cbff00`). Перед КАЖДЫМ мержем в `main` — `mcp__render__get_service` → `autoDeploy: "no"`
...
- **Мерж.** После закрытия задачи (гейт-ревью APPROVE + зелёный CI ветки): `git fetch` → rebase на `origin/main` →
  push ветки → CI → ff-push в `main`. Один имплементер в дереве в каждый момент; серверные сьюты делят одну
...
самостоятельна и первой же поднимает обвязку среза: автодеплой выключен ДО первого мержа, ветка и база в известном
состоянии, базовая линия снята.

**Файлы:**
- Изменить (ОСНОВНОЕ дерево, `main`, docs-коммитом): `render.yaml` — строка `autoDeploy: false` после `:12`
  `    branch: main` (ключа сегодня нет; образец `33763b0`).
...
- [ ] **Шаг 2: автодеплой off — docs-коммит в `main` и пуш (РП-24, Ф-Б2-2).** Вставить строкой 13 (отступ 4 пробела):
```yaml
...
```
  Ожидание: все EXIT=0; счётчики против базовой линии шага 4 — только новые тесты. Протокол закрытия — «Мерж»
  глобальных ограничений; перед ff-push — `get_service` (`autoDeploy: "no"`).
...
```
  ...
  из колонки), названное спекой 1а §11.2; снимает срез 2». Прогон шага 1 → PASS.
...
- [ ] **Шаг 3: ранбук.** Чек-лист «Деплой среза 1а (страницы)» — порядок задачи 19 одним списком: `ping` → `check`
  ...
  РП-2) → `/health` → автодеплой обратно → смоук («Обновить» сервис-воркера; живая приёмка владельца). Миграций нет.
...
- [ ] **Шаг 6: коммит (в ветке; мерж — по протоколу).**
```
...
- [ ] **Шаг 6: фикс-волна** одним имплементером (`model: opus`) по `dispatch-final-fix.md`; гейт фикс-волны
  ...
  `check-lazy-chunks`, `check-legacy-form --gate`. Мерж в `main` по протоколу. **ЖЁСТКАЯ ОСТАНОВКА:** задача 19 — только
  отдельным словом владельца.

### Задача 19: Прод-процедура среза 1а, живая приёмка владельца, итог

**Зачем:** весь срез в `main` и отревьюирован, но на проде — код до 1а, реестр без аспекта `orbis/page`, автодеплой
выключен задачей 1. Порядок «данные ДО кода» (урок D42): пересев, сразу деплой; окно между ними — простой (Ф-1а-2,
РП-2). Миграций нет, `reset-world` не нужен (сид аддитивен). До пересева владелец видит числа корпуса (§5.9) и
отвечает словом. **Исполняется только вместе с владельцем.**

**Файлы:**
- Изменить (ОСНОВНОЕ дерево, `main`, docs-коммитами): `render.yaml` — снять строку `autoDeploy: false` (образец
  `9cbff00`); `docs/prd/04-decision-log.md` — статус D45 «и в проде <дата>».
...
- [ ] **Шаг 2:** предпроверки веток: `main` = `origin/main` = ветка; CI на `main` зелёный (`gh run list --branch main
  --limit 3`); финальное ревью закрыто (`ls $L/final/` — APPROVE).
...
- [ ] **Шаг 8:** `bun scripts/ops.ts seed-registries` → «свойств 79 … аспектов 14 …», версия system-реестров +1,
  ...
  немедленно.
- [ ] **Шаг 9:** деплой: `mcp__render__get_service` (`autoDeploy: "no"`, `branch: main`) → `mcp__render__trigger_deploy`
  → `mcp__render__get_deploy` до `live`; коммит = HEAD `main`. Время окна (сид → live) — в `progress.md`.
...
- [ ] **Шаг 11:** автодеплой обратно — снять строку `autoDeploy: false`, коммит `ops(render): автодеплой возвращён
  после прод-процедуры среза 1а` (`git commit -m "…" -- render.yaml`), пуш; `get_service` → `autoDeploy: "yes"` (иначе —
  dashboard).
...
| 14 | выкатка: остановка перед продом, автодеплой off, пересев до кода, подъём версии клиента | 1 / ш. 2; 18 / ш. 6; 19; 8 / ш. 5 |
...
- **Промежуточные состояния.** Формат v3 — одной задачей 8; экран записи до задачи 14 — прежний (РП-23), после — через
  ...
  (модель аспект не видит — РП-1); автодеплой выключен с задачи 1 до задачи 19.
