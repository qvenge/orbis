...
**План ИСПОЛНЕН целиком** (обновлено Задачей 17, 2026-08-16). Все 17 задач закрыты в ветке
`notion-editor`: реализация → ревью → раунды правок → точечное ре-ревью. Чекбоксы шагов ниже
так и остались непроставленными — ходом работы распоряжались отчёты задач
(`.superpowers/sdd/2026-08-14-notion-like-editor-v2/task-N-report.md`), а не разметка плана;
проставлять их задним числом значило бы утверждать, что каждый шаг исполнен ДОСЛОВНО, — а часть
шагов сознательно исполнена иначе (список расхождений — в конце файла).

| Задача | Статус |
|---|---|
| 1 (полифилы) | СДЕЛАНА — `83ac6a2` |
| 2 (схема, канонизация, поблочный `raw`) | СДЕЛАНА — `85c70ab`..`a2f4974` |
| 3 (`audit-bodies`) | СДЕЛАНА — ..`ee7157d` |
| 4 (миграция `body_doc`, бэкфилл) | СДЕЛАНА — ..`883de2a` |
| 5 (executor: канон, гейт §5.2, UI-схемы) | СДЕЛАНА — ..`ce98b4f` |
| 6 (`entity.suggest` / `entity.resolveRefs`) | СДЕЛАНА — ..`6df313f` |
| 7 (базовый редактор, гашение фантомов, двухфазность) | СДЕЛАНА — ..`59c4c46` |
| 8–9 (чип ссылки, виджет смарт-листа) | СДЕЛАНЫ — ..`eb00371`, ..`60c59fe` |
| 10 (slash-меню и `@`) | СДЕЛАНА — ..`268361c` |
| 11 (bubble-меню, перемещение блоков) | СДЕЛАНА — ..`f30ca8c` |
| 12 (markdown-тумблер) | СДЕЛАНА — ..`e5ac002` |
| 13–14 (сохранение по паузе, офлайн-черновик) | СДЕЛАНЫ — ..`274ca69`, ..`966e360` |
| 15 (три таба, `key`, `flush`, ленивый тумблер) | СДЕЛАНА — ..`0853fd7` |
| 16 (уборка, страж чанков, PRD) | СДЕЛАНА — `0779cc1`..`24b2db2` |
| 17 (сквозная сверка документов) | СДЕЛАНА — этим коммитом |

**Что описывает итог.** Состояние построенного описывает дизайн-спека
`docs/superpowers/specs/2026-08-13-notion-like-editor-design.md` — она приведена к факту
Задачей 17 и с этого момента главнее обоих планов. Оставшееся за владельцем — прогон
`bun scripts/ops.ts audit-bodies` на проде перед бэкфиллом и проверка мобильного ввода на живом
устройстве.
...
## Что вышло иначе, чем написано в плане (дописано Задачей 17 по факту кода)

Текст задач выше не переписан: это инструкции, которые давались исполнителям, и подгонять их
под результат значило бы стереть след решений. Но читатель, пришедший в план за справкой о
СЕГОДНЯШНЕМ коде, обязан знать, где буква плана и факт разошлись. Полное описание построенного —
в дизайн-спеке; здесь — только сами расхождения.

1. **Страж чанков (З16, п. 3): имя `BodyEditor` лежит НЕ в `SHARED_CHUNKS`.** В
   `scripts/check-lazy-chunks.ts` заведён отдельный список `LAZY_EDITOR_MODULES` —
   `['BodyEditor', 'MarkdownToggle']`: `SHARED_CHUNKS` описывает общие чанки ленивой части, а
   это ленивые модули внутри экрана, и второй из них план не охранял вовсе. Требование «не в
   `LAZY_SCREENS`» соблюдено — тот по-прежнему сверяется с `lazy()`-точками `router.tsx`.
2. **Миграция `0007` названа `0007_flat_triathlon.sql`** (имя выдал drizzle; карта файлов v1
   обещала `0007_body_doc.sql`) и содержит РОВНО одну строку — добавление колонки. Индекса
   заголовка в ней нет: он убран до выезда миграции, потому что под RLS не-leakproof квал не
   встаёт вперёд security-квала и индекс был бы мёртв (замерено EXPLAIN'ом).
3. **`entity.suggest` ищет по ВХОЖДЕНИЮ, поле входа — `term`.** Префиксный поиск (З6, дизайн Р8)
   не реализован: якорь отнимал бы находимость, а платы за его снятие нет — индекса всё равно
   нет. Релевантность держит первый ключ сортировки.
4. **`/`-меню: «Смарт-лист» ВСТАВЛЯЕТ блок-заготовку**, а не открывает `QueryBlockEditor`;
   **пункта «Создать сущность» в нём нет** — создание из набранного живёт последней строкой
   `@`-меню, и появляется она только после ответа поиска (иначе Enter заводил бы дубль).
5. **Bubble-меню: «превратить в…» не сделано** — в панели четыре действия форматирования и
   «Удалить блок».
6. **`rawBlock` рисуется голым `<pre data-raw>`**: пометки «не разобрано» на блоке нет, править
   его на месте нельзя (нода атомарная) — правка идёт через markdown-тумблер.
7. **Мобильная панель «+» отложена решением КОНТРОЛЛЕРА, а не владельца** — в цикле владелец по
   этому пункту не высказывался. Отдельная задача не заведена; ждёт подтверждения.
8. **Строки PRD в З16 п. 5 устарели вместе с правкой:** после перестановки пунктов «Body» — это
   пункт 4 раздела «§3.5 Detail-экран», а не пункт 3, и ссылаться на номера строк в `02-core-os.md`
   больше не нужно — раздел адресуется заголовком.
9. **Файлы, которых нет в карте v1, но которые появились по необходимости:**
   `packages/shared/src/doc/manager.ts` (свой markdown-менеджер),
   `apps/server/src/seed/seed-canon.test.ts` (каноничность сидов — в `apps/server`, потому что
   shared не зависит от server), `apps/web/src/features/entity-editor/strip-ids.ts` (сравнение
   документов по смыслу; ЛИСТОВОЙ файл — его зовёт эагерный чанк detail), `body-box.ts`
   (общая коробка первого кадра и редактора), `SaveIndicator.tsx`.
10. **Числа бандла** (замер `bun run --filter @orbis/web build` на закрытии работы): первый кадр
    **218.86 кБ gzip** (не вырос), `doc` **154.48**, `BodyEditor` **27.88**, `DetailScreen`
    **18.49**, `MarkdownToggle` **0.90**. Оценки «~156 kB» и «~160 kB» в тексте задач выше — того
    времени, когда схема и редактор мыслились одним чанком.
