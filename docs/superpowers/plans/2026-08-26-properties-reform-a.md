...
- **РП-3. Две формы входа исполнителя в переходный период.** Внутренняя форма `entityPropsPatch` (`props` по key|id, `unset`, `aspects: {attach, detach}`) появляется в exec-надмножествах контрактов уже в Задаче 4b (union со старой картой `aspects: {id: {field: value}}`), потому что inverse/undo (6), глаголы/материализация (10a/10b) пишут ею.
  **ПОПРАВКА по гейту Задачи 5:** слова «предложения (5)» в этом перечне неверны — Задача 5 переводит
  только форму ПУНКТА предусловия (§А7-3), а сам ПАТЧ предложения расписан по другим задачам:
  Задача 12 владеет формой предложения (`routines/propose.ts`) и `snapshotDeferredUnit`
  (`dispatch.ts`), Задача 10b — `routines/edits.ts` (`fieldEditSchema`) и `updateRows`. Перевод
  патча внутри Задачи 5 сработал бы швом 4b: карточка молча показала бы пустой список правок; старая карта переводится на границе адаптером `fromLegacyInput()` (`legacy-form.ts` ИМПОРТИРУЕТ `legacyFieldToProperty`/`propertyToLegacyField` из `@orbis/shared` Задачи 2 — второй таблицы соответствий нет). LLM-контракты тулов (JSON Schema + zod) переходят на новую форму в Задаче 12; tRPC/UI-надмножества принимают старую карту до Задачи 13c (web-отправители) и 18 (`MemoryRuleCard`), где union и адаптер входа снимаются; проекции `projectLegacyAspects`/`projectLegacyRelationType` живут до Задачи 23.
...
- Изменить: `registry/load.ts` (`loadRegistry` → через кеш `(ownerId, ownerVersion, systemVersion)`;
  ...
  drift (`ops.ts check` — админский путь, runbook §1) и в системную заметку глобального треда.
  **`/health.registryDrift.conflicts` НЕ заводится (рулинг Р-14-1):** `registry_deltas` под RLS
  `owner_owns_row`, у стартовой проверки `/health` актора нет (`auth.uid()` = NULL), поэтому она
  видит НОЛЬ дельт — поле было бы вечно пустым, то есть мёртвым адресом (доказано пробой на живой
  базе). Условие снятия запрета — политика для сервисной роли, а это пятая миграция, которой в
  срезе нет. Запрет запиннен тестом, условие записано в докблок;
...
- Создать: `apps/server/src/seed/gardener.ts` (+test) — тело и параметры рутины «Садовник
  словаря»: `orbis/routine_mode: act` с `orbis/allowed_tools: ['property_merge']` — белым списком
  ровно из одного тула (**рулинг Р-16-1**: в `propose` из мутаций доступен РОВНО `orbis_propose`,
  `tools/registry.ts:205-207`, и отложенная единица §С8-11 садовнику недостижима; прав это не
  расширяет — `property_merge` от рутины классифицируется `behavior-delta` и до `execute` не
  доходит ни одним путём), раз в неделю (`orbis/routine_days: ['mo']`,
...
- [ ] **Шаг 1: падающие тесты:** сид садовника идемпотентен (детерминированный id); садовник
  посеян в режиме `act` с белым списком ровно из `property_merge` (Р-16-1), и его прогон над
  ВСТРОЕННЫМ аспектом даёт отказ по объекту (`routine_untouchable`), а не единицу; онбординг
...
- Содержание правок (текст описывает УЖЕ существующие тулы и грамматику): абзацы `v4.ts:54`
  ...
  остаток C, §Б7-1).
- **ДОБАВЛЕНО рулингом Р-19-1 (замер Задачи 17):** указание «свои свойства заводи со
  `status=proposed`» в промпт **v5** (и `routine-v3`). Основание — живой прогон: описание тула это
  уже велит дословно (`registry-tools.ts:100, :244` — «Заводя от себя, ставь proposed»), и модель
  всё равно завела **2 свойства из 2 со `status=active`**. Механизм §А2-7 (кап 20, разбор пачкой,
  порог П4), которым модель не пользуется, — мёртвый механизм; промпт остаётся единственным
  местом, где на это можно повлиять до части Б. Формулировку дать в политике-прозе, рядом с
  правилом «заводи только то, по чему фильтруют».
...
**Файлы:**
- Изменить: `apps/web/src/features/entity-editor/useBodySave.ts:677-729` (развилка при открытии,
  ДО сверки с телом записи: `draft.doc.v < DOC_SCHEMA_VERSION` → состав нод документа
  ⊆ `KNOWN_NODE_TYPES` → перештамповать версию и идти обычным потоком; иначе — предложение
  выбором; `draft.doc.v > DOC_SCHEMA_VERSION` (откат релиза) — всегда предложение, вниз не
  штампуем; молчаливый досыл `:715-724` — только при `v === DOC_SCHEMA_VERSION`; текст не
  теряется ни в одной ветке — черновик остаётся в `localStorage` до явного решения),
  `packages/shared/src/doc/types.ts` (+ `KNOWN_NODE_TYPES` — имена нод текущей схемы литералом,
  + `collectNodeTypes(doc)` — рекурсия по `content`, марки НЕ собираются; оба листовые),
  `packages/shared/package.json` (+ сабпат `"./doc/types"`: рантайм-импорт `@orbis/shared/doc`
  из `useBodySave.ts`/`draft-storage.ts` запрещён стражем чанка — `save.test.tsx:1466, :1491-1506`,
  якорь `$` предиката листовые сабпаты пропускает, прецедент `/diff`),
  `apps/web/src/features/entity-detail/DetailScreen.tsx:805-839` (`DraftBanner` — сегодня ровно две
  кнопки; третий вид баннера: «Сохранить в заметку» плоским текстом через `entity.create({body})` /
  «Открыть серверное тело»; `bodyDoc` чужой версии executor отвергает — `executor.ts:1884`),
  тесты `draft.test.tsx` (50), `save.test.tsx` (47), `detail.test.tsx` (`seedDraft:63`, 10 вызовов),
  новый `packages/shared/src/doc/schema.test.ts` (сверка `KNOWN_NODE_TYPES` с
  `Object.keys(getSchema(DOC_EXTENSIONS).nodes)` — 20 нод, `code` это марка).
- **Форма черновика на диске НЕ меняется** (рулинги Р-20-2/Р-20-3 леджера): версия в нём УЖЕ есть —
  `draft.doc.v` (`Draft.doc: BodyDoc`, `draft-storage.ts:21-33`), её же сверяют `executor.ts:1884`,
  `useEntityDetail.ts:135` и сама сверка с телом `useBodySave.ts:689`; состав нод — функция
  документа и считается обходом при чтении. Поля `v`/`nodeTypes` завели бы второй источник истины
  и сделали бы КАЖДЫЙ уже лежащий на диске черновик «составом неизвестным» — диалог на ровном
  месте; а ужесточение `parseDraft` до `null` уничтожило бы старые черновики молча
  (`sweepDrafts:117-119` сносит неразобранное при открытии ЛЮБОЙ записи).

- [ ] **Шаг 1: падающие тесты:** черновик `doc.v = 0` при живой `DOC_SCHEMA_VERSION = 1` (мока
  константы НЕТ — оба файла импортируют модуль только типом, мокать нечего) и известных нодах →
  перештампован, досыл штатный; черновик с нодой `unknownNode` → предложение выбором, ничего не
  отправлено; черновик `doc.v = 999` (откат релиза) → предложение выбором; «Сохранить в заметку» →
  `entity.create` с `body` плоским текстом, черновик снят с диска ТОЛЬКО по успеху мутации;
  «Открыть серверное тело» черновик не удаляет; `rejected` ветка прежняя; `collectNodeTypes` не
  собирает марки (`link`/`bold` в наборе отсутствуют); `KNOWN_NODE_TYPES` сверен со схемой.
- [ ] **Шаг 2:** реализация. — [ ] **Шаг 3:** PASS web + `bun run --filter @orbis/web build` +
  `bun scripts/check-lazy-chunks.ts` (у чанка `DetailScreen` сегодня семь статических импортов,
  `check-lazy-chunks.ts:207-208` — новый сабпат может стать восьмым, число обновлять осознанно);
  коммит
  `feat(editor): контракт офлайн-черновиков Т8-в — сверка версии и состава нод, перештамповка вместо молчаливого досыла (§А11-2)`.
...
- `docs/implementation/02-ops-runbook.md` §1 (чек-лист деплоя реформы — РП-7: автодеплой,
  ...
  вне CI);
- `docs/implementation/00-architecture.md` (найдено писателем Задачи 22, Р-22-17: `:125`/`:150`
  «шесть потребителей», `:148` диаграмма с несуществующими авто-чекинами, `:353` колонка `meta`,
  `:364` `relation_type` — те же ложные утверждения, что и в PRD; добирается тем же писателем и тем
  же гейтом, ложится тем же коммитом Задачи 23).
