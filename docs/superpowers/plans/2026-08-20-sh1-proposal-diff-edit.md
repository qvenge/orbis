...
```
  ...
  **Спаривание** removed/added в `changed` (Ш1.10): внутри одной замены, кандидаты по
  ...
  (∩/min) = 1.0 (ловит дописанный хвост. ЧИСЛО В ПРИМЕРЕ ИСПРАВЛЕНО по итогам гейт-ревью Задачи 6:
  «Спорт» → «Спорт — заменить на бассейн» даёт Дайс 2·1/(1+4) = **0.4** и порог проходит БЕЗ правила
  вложения — этот пример ничего не доказывал. Правило вложения нужно для более длинного хвоста:
  «Спорт» → «Спорт — заменить на бассейн, абонемент до пятницы» — Дайс 0.25 при вложении 1.0).
  **Слова** — `/[\p{L}\p{N}]+/gu` (одиночные цифры значимы: «18:00»;
...
**Файлы:**
**ПРЕФИКС-ШАГ (добавлен рулингом контроллёра по итогам гейт-ревью Задачи 9 — делать ПЕРВЫМ,
отдельным коммитом, до плашки и слоя):**
- вынести презентацию диффа из `apps/web/src/features/chat/cards/ProposalCard.tsx:67-203`
  (~130 строк: `BODY_DIFF_SKIP_NOTES`, `BodyDiffUnits` и их типы) в свой модуль
  `apps/web/src/features/chat/cards/BodyDiff.tsx`. Карточка выросла 273 → 550 строк, и слой стал бы
  ВТОРЫМ импортёром презентации ИЗ ФАЙЛА КОМПОНЕНТА чужой фичи; сейчас это чистый перенос, после
  двух импортёров он дорожает. `CollapsedBodyDiff`/`diffCounts` — политика карточки, ОСТАЮТСЯ в ней;
- подпись `ProposalCard.tsx:287` «живое предложение ниже» → **«правленое предложение ниже»**:
  нынешняя предписана планом дословно, но после принятия правленого она врёт (P1 говорит «живое», а
  ниже стоит «Принято (с правками)»);
- ужесточить типы словарей `ProposalCard.tsx:78` и `:90`: `Record<string, string>` →
  `Record<ReplacedReason, string>` / `Record<BodyDiffSkipReason, string>` — серверные собратья
  типизированы по enum и падают сборкой при новой причине, клиентские молча роняют её в фолбэк;
- дописать ассерцию инвалидации ленты для исхода `replaced` (`cards.test.tsx:1281-1334`) — главный
  сценарий приёмки 9 сегодня следует лишь из безусловности кода.
