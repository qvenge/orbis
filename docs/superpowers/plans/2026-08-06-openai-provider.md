...
> Устарели конкретно, полным перечнем (первая редакция этой врезки пропустила три места
> из семи — их дописало ревью 2026-08-08):
>
> 1. **Global Constraint «Транспорт OpenAI — только Chat Completions»** (раздел
>    «Global Constraints» ниже).
> 2. **Код-образец Task 2 Step 3**: там `createOpenAI(...).chat(modelId)` с комментарием
>    «отказ БЕЗУСЛОВЕН»; в коде сегодня голый `createOpenAI(...)(modelId)`. Образец
>    расходится с кодом и вторым способом: в нём нет ни `toolExtras`, ни `providerOptions` —
>    а без первого Responses отдаёт 400, без второго переписка остаётся у OpenAI.
> 3. **Формулировка D26 в Task 4 Step 5.**
> 4. **«Chat Completions» в тексте коммита Task 2** (Task 2 Step 7).
> 5. **Task 4 Step 4** — инструкция ПРЕДПИСЫВАЕТ «обязательно назвать транспортное
>    ограничение OpenAI (Chat Completions, причина — lookahead в `positiveDecimal`)»
>    в §7.7 PRD. Опаснее прочих пунктов перечня: остальные лишь описывают неверное,
>    а этот велит его написать. В самом PRD §7.7 транспорт уже приведён к факту
>    коммитом `72bdbc7` — шаг исполнять не надо.
> 6. **Контракт `AiSdkProvider`** — Task 1 «Interfaces → Produces» и Step 3, повторно
>    в «Interfaces → Consumes» Task 2 и в самопроверке: конструктор объявлен тремя полями
>    (`model`/`modelId`/`timeoutMs`), сегодня их пять — смена транспорта добавила
>    `toolExtras` и `providerOptions` (`apps/server/src/llm/ai-sdk.ts`).
> 7. **Самопроверка плана, «Покрытие спеки»** — строка «Р1 (транспорт) → Task 2 Step 3»
>    ведёт на решение спеки, которое отменено. Там же видна своя нумерация решений,
>    сдвинутая относительно спеки вставленным «расслоением»: в спеке их семь (Р1–Р7),
>    а список доходит до «Р8».
>
> Мест названо семь, потому что перечень объявлен закрытым: если он снова окажется
> неполным, читатель поверит именно ему, а не поиску по файлу.
>
> Действующее решение — **D29** в `docs/prd/04-decision-log.md`, оно же отменяет D26;
