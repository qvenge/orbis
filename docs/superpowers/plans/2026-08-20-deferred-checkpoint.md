...
| Р-17 | — (найдено разведкой) | `answerRoutineCheckpoint`/`readProposal` стоят на `runById` с `NOT archived` (`queries.ts:346`) — решения по единицам АРХИВНОГО (откаченного) прогона упёрлись бы в NOT_FOUND | Процедуры единиц адресуются `pendingId` и прогон через `runById` НЕ читают (спека §6) — это верно и реализовано. **Вторая половина ОПРОВЕРГНУТА пробоем 2026-08-21 (ревью Задачи 10):** бухгалтерский патч `undecided` по АРХИВНОМУ прогону НЕ отказывает — `NOT archived` стоит только на ЧТЕНИИ `runById` (`queries.ts:346`), а в конвейере executor'а запрета на `entity_update` архивной записи нет вовсе. Флажок с откаченного прогона снимается штатно; «признанной цены» здесь нет. Тест и докблок Задачи 10 переписаны на фактическое поведение |
...
7. **`decideAll` обходит единицы в детерминированном порядке** (`created_at, id`).
   **ОБОСНОВАНИЕ ОПРОВЕРГНУТО ПРОБОЕМ 2026-08-22 (ревью Задачи 11):** заявленной «защиты от
   взаимной блокировки advisory-замков» тут нет и быть не может — `acquirePendingLock` берёт
   ОДИН ключ на транзакцию (`pg_advisory_xact_lock`, `pending.ts:557-583`) и отпускает его на
   коммите, а каждая единица решается своей короткой транзакцией: две единицы разом не
   удерживаются, цикл непостроим. Решение остаётся в силе по ДРУГИМ доводам: предсказуемость
   сводки (она же экран владельца) и общая дисциплина обхода; порядок станет несущим, если
   лестница Ш1.5 когда-нибудь возьмёт замок одного pending, трогая другой.
...
//   run_id берётся из pending-записи; отказ патча глотается с логом (лестница §5). ПРИМЕЧАНИЕ
//   2026-08-21: ожидавшийся отказ по АРХИВНОМУ прогону не наступает — Р-17 в этой части
//   опровергнут пробоем, патч проходит; глотание остаётся страховкой от прочих отказов.
