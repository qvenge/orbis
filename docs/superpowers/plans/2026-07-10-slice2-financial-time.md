...
| **C. CSV-импорт** | Полный флоу импорта выписки: shared-дедуп, серверный роутер, экран ревью, undo | C1, C2a, C2–C4, C6, C5 |
...
```
- Инстанс: копия title/emoji/tags шаблона; `orbis/schedule` инстанса — `start_at` даты инстанса (время из `start_at` шаблона), без `recurrence`; при `orbis/financial` на шаблоне — тот же аспект с `occurred_on=<дата>`, `planned=true`, `recurring=true`; relation `derived_from` (source=шаблон, target=инстанс). **Привязка к конверту (sign-off владельца 2026-07-11): planned-инстанс привязывается A4-хуком сразу при материализации, если конверт периода уже существует** (а не только при переходе в факт) — семантика A4 самосогласованна (тот же селектор §2.3 для всех дат), и прогнозные секции Budget (Coming up, §3.1) сразу знают свой конверт. Следствия, принятые осознанно: Undo перехода planned→fact снимает привязку только если она была создана самим переходом (конверт появился после материализации); `children_of=<конверт>` включает будущие planned-инстансы (фаза B фильтрует отображение по `planned`/дате при необходимости). `spent` денежно не затронут — считает только факты (§2.2).
...
- **Обязательство (бэклог финального ревью фазы A):** нормализация currency NULL→defaultCurrency при создании конверта (решение по TOCTOU A7) — принять спек-решение и реализовать в этом таске, до появления новых путей создания конвертов.

- [ ] **Шаг 1: тесты**: пороги цвета по данным §3.1 (<60% → norm, 72% → warn(жёлтый), 91% → alert(⚠ оранжевый), ≥100% → 🔴); «—/день» при null-pace в active; «начнётся»/«завершён» по phase; carryover-бейдж; сабмит создания шлёт валидный `orbis/budget`-аспект.
- [ ] **Шаг 2:** FAIL → реализация → PASS.
...
- **Обязательство (бэклог финального ревью фазы A):** расширить `materializationWindow` абсолютными диапазонами дат вместе с грамматикой — иначе запрос `occurred_on=<от>..<до>` не материализует инстансы окна и экран покажет неполный месяц.
...
/** max(1 − levenshtein/maxLen, tokenJaccard, tokenContainment) на нормализованных
 *  строках, где tokenContainment = |A∩B| / min(|A|,|B|). Третий член добавлен
 *  sign-off'ом владельца 2026-07-25: без него банковский шум в имени мерчанта
 *  («ПЯТЕРОЧКА 843» против ручного «Пятёрочка») даёт 0.69/0.50 — ниже порога,
 *  и фикстура §3.4.1 не выполнялась. Известный класс ложных срабатываний
 *  (имя-подмножество: «OZON» ↔ «OZON TRAVEL») принят: ограничен точной суммой +
 *  направлением + датой ±1 день и переключаем пользователем в ревью. */
...
- [ ] **Шаг 1: тесты**: нормализация `«SBOL ПЯТЁРОЧКА 1234»` → `пятерочка 1234`; `ё→е`; пунктуация; similarity `ПЯТЕРОЧКА 843` vs `Пятёрочка` ≥ 0.85 (проходит третьим членом — containment) — и негативная пара < 0.85 (`OZON` vs `WILDBERRIES`); дубль: разница дат ровно 1 день — да, 2 дня — нет; bankTxnId перекрывает текст; external_id детерминирован (фикстура с точным hex), меняется от rowIndex. Плюс пиннящий тест принятого класса ложных срабатываний: `OZON` vs `OZON TRAVEL` = 1.0 (containment) — фиксирует осознанное решение, а не случайное поведение.
...
### Task C2a: Батчинг селектора конвертов (обязательство A → вход в массовый импорт)

Обязательство бэклога фазы A, вынесенное отдельным таском: N+1 в авто-привязке
разворачивается в квадрат ровно на пути импорта (batch из сотен `entity_create`).

**Files:**
- Modify: `apps/server/src/budget/binding.ts`, `apps/server/src/executor/executor.ts` (`applyBudgetFollowUps`)
- Test: `apps/server/src/budget/binding.test.ts` (или существующий тест привязки)

**Interfaces:**
- Батч-селектор: `selectEnvelopes(tx, { ownerId; defaultCurrency; rows: Array<{ key: string; categoryRef: string; currency: string; occurredOn: string }> }): Promise<Map<string, string | null>>` — один SQL на весь набор (тот же порядок выбора «самый узкий конверт», что и `selectEnvelope`: `ORDER BY (period_end − period_start) ASC, period_start DESC, id ASC`, по одному победителю на ключ).
- Батч-версия `budgetParentsOf` на множество `txnId` (один SQL) и кэш `defaultCurrencyOf` в пределах одного исполнения.
- Оба потребителя переводятся на батч: цикл `rebindForEnvelope` (названный в бэклоге) и `applyBudgetFollowUps` (N хуков одного batch — фактический горячий путь импорта).

- [ ] **Шаг 1: тесты**: (а) поведенческая эквивалентность — batch из N транзакций разных категорий/дат привязывается ровно к тем же конвертам, что N одиночных вызовов; (б) «самый узкий конверт» побеждает в батче так же, как в одиночном селекторе (пересечение месячного и отпускного, приёмка §7.3); (в) счётчик SQL-запросов на batch из 50 транзакций не растёт линейно (инструментировать через обёртку над `tx.execute` или пересчёт вызовов селектора).
- [ ] **Шаг 2:** FAIL → реализация → PASS; полный прогон существующих тестов привязки и агрегатов без изменений.
- [ ] **Шаг 3:** Коммит `perf(budget): батч-селектор конвертов вместо N+1 в привязке и ребиндинге (бэклог A)`.
...
- **Обязательство C1/C2 (бэклог финального ревью фазы A):** устранить N+1 `selectEnvelope` в `rebindForEnvelope` (`apps/server/src/budget/binding.ts`) до массового импорта — batch из сотен строк с конвертом в том же action иначе даёт квадрат селектора.
...
### Task C6: Пагинация списков транзакций (бэклог B, sign-off владельца 2026-07-25)

Импорт выписки на сотни строк делает молчаливое обрезание `limit=200` заметным:
пользователь импортирует 300 операций и видит 200. Взято в объём фазы C решением
владельца 2026-07-25.

**Files:**
- Modify: `apps/web/src/features/budget/txQuery.ts`, `apps/web/src/features/budget/TransactionsScreen.tsx`, `apps/web/src/features/budget/CategoryScreen.tsx`
- Test: `apps/web/src/features/budget/TransactionsScreen.test.tsx` (+ тест экрана категории, если есть)

**Interfaces:**
- Страница — существующая грамматика запросов (`limit`/`offset` в `entity.query`, если поддержаны; иначе — `limit` с инкрементом окна). Никакого расширения грамматики без нужды (глобальное ограничение плана): если `offset` в грамматике нет, окно растёт увеличением `limit` на клиенте.
- UX: кнопка `[Показать ещё]` под списком; видна только когда пришло ровно `limit` записей; счётчик показанных.

- [ ] **Шаг 1: тесты**: ровно `limit` записей → кнопка видна; меньше → скрыта; клик догружает следующее окно и не теряет фильтры.
- [ ] **Шаг 2:** FAIL → реализация → PASS.
- [ ] **Шаг 3:** Коммит `feat(web): пагинация списков транзакций — «показать ещё» (бэклог B)`.
...
**Обязательство фазы C (отложено осознанно, решение контроллера в C2):** задача C2
реализовала `suggestedCategoryRef` в `import.review` ТОЛЬКО по алиасам категорий, потому
что детерминированного формата memory-правила на тот момент не существовало — его задаёт
эта задача. Как только парсер правил появится, подключить его и к импорту: серверный
резолв в `apps/server/src/import/review.ts` обязан применять правила ПЕРЕД алиасами, тем же
кодом и тем же форматом, что fast-path. Второй реализации парсинга правил быть не должно.
Практический вес: на реальной выписке алиасы не покрывают имена мерчантов («ПЯТЁРОЧКА»,
«OZON», «YANDEX.TAXI» приходят без предложения — проверено смоуком фазы C), поэтому
именно правила делают категоризацию импорта полезной.
...
- [ ] **Шаг 2a:** тест сервера: `import.review` с правилом `пятерочка → Еда` предлагает Еду для строки «SBOL ПЯТЁРОЧКА 843», хотя ни один alias её не покрывает; без правила — предложения нет.
- [ ] **Шаг 3:** FAIL → реализация → PASS.
...
**Обязательство фазы C:** у «сегодня» нет тестового шва — `localToday`/`localTodayTx`
(`apps/server/src/budget/aggregates.ts`) читают таймзону и берут реальную дату, подменить
её нельзя. Задача C5a обошла это тем, что оба периода приёмки §7.1 закрыты, но **§7.2
(«в дату операции transition выполняется один раз») без шва недостижима**: сценарий требует,
чтобы «сегодня» совпало с датой инстанса. Решить ДО шага 1 — либо инъекцией clock в
`postDueInstances`/`localToday`, либо фикстурой, где дата инстанса и есть текущая дата
(хрупко: тест начнёт падать в полночь и зависит от таймзоны машины). Это решение об
архитектуре тестов, а не правка теста.

Приёмки §7.1 и §7.4 уже закрыты фазой C — автотестами
(`apps/server/src/import/import-acceptance.test.ts`, 13 проверок; `import.test.ts`) и
браузерным смоуком. Перепроверять их руками не нужно, достаточно сослаться.
