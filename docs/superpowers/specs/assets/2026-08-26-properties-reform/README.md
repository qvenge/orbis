...
| `p1-tasks.json` | П1, 2026-08-25 | `.superpowers/probe/p1/tasks.json` | то же | **Корпус 20 сценариев** пяти классов (А — «сведи к существующему», Б — вариант перечисления, В — действие, Г — подписка/правило, Д — новое свойство): нормативен как ВХОД замеров. На нём мерены П1 и П4 (§С8-12, приёмка порога Р14), и подмена корпуса обнулила бы сравнимость цифр |
...
`p1-tasks.json` перенесён Задачей 17 реформы — тогда, когда у него появился второй читатель:
проба П4 (`scripts/probe-p4.ts`) гоняет тот же корпус через живого провайдера и считает по нему
порог Р14 (§С9). До этого корпус лежал только в gitignored-пробе, то есть цифры П1 нечем было
воспроизвести.
